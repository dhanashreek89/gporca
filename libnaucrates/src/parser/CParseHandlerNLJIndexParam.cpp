...
// ctor
CParseHandlerNLJIndexParam::CParseHandlerNLJIndexParam(IMemoryPool *mp,
...
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenNLJIndexParam),
									  element_local_name))
	{
	...
	m_nest_param_colref_dxl = CDXLOperatorFactory::MakeDXLColRef(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenNLJIndexParam);
}
...
)
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenNLJIndexParam),
									  element_local_name))
	{
		CWStringDynamic *pstr = CDXLUtils::CreateDynamicStringFromXMLChArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), element_local_name);
