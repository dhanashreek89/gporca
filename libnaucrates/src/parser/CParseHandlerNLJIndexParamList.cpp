...
void
CParseHandlerNLJIndexParamList::StartElement(const XMLCh *const element_uri,
											 const XMLCh *const element_local_name,
											 const XMLCh *const element_qname,
											 const Attributes &attrs)
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenNLJIndexParamList),
									  element_local_name))
	{
	...
	else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenNLJIndexParam),
										   element_local_name))
	{
		...
		nest_param_parse_handler->startElement(
...
void
CParseHandlerNLJIndexParamList::EndElement(const XMLCh *const,  // element_uri,
										   const XMLCh *const element_local_name,
										   const XMLCh *const  // element_qname
)
{
