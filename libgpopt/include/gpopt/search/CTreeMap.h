...
namespace gpopt
{
	...
	private:
		...
		private:
			...
			// rehydrate tree
			R *
			PrUnrank(IMemoryPool *mp, PrFn prfn, U *pU, ULONG ulChild, ULLONG ullRank)
			{
				GPOS_CHECK_STACK_SIZE;
		...
		public:
			...
			R *
			PrUnrank(IMemoryPool *mp, PrFn prfn, U *pU, ULLONG ullRank)
			{
				GPOS_CHECK_STACK_SIZE;
		...
		// map of all nodes
		typedef gpos::CHashMap<T, CTreeNode, HashFn, EqFn, CleanupNULL, CleanupDelete<CTreeNode> >
			TMap;
		typedef gpos::
			CHashMapIter<T, CTreeNode, HashFn, EqFn, CleanupNULL, CleanupDelete<CTreeNode> >
				TMapIter;

		// map of created links
		typedef CHashMap<STreeLink,
						 BOOL,
						 STreeLink::HashValue,
						 STreeLink::Equals,
						 CleanupDelete<STreeLink>,
						 CleanupDelete<BOOL> >
			LinkMap;
	...
	public:
		...
		R *
		PrUnrank(IMemoryPool *mp, U *pU, ULLONG ullRank) const
		{
