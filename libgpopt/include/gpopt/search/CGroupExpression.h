...
namespace gpopt
{
	...
	private:
		...
		typedef CHashMap<CPartialPlan,
						 CCost,
						 CPartialPlan::HashValue,
						 CPartialPlan::Equals,
						 CleanupRelease<CPartialPlan>,
						 CleanupDelete<CCost> >
		...
		void PrintXform(IMemoryPool *mp,
						CXform *pxform,
						CExpression *pexpr,
						CXformResult *pxfres,
						ULONG ulNumResults);
		...
		void PreprocessTransform(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal, CXform *pxform);

		// postprocessing after applying transformation
		void PostprocessTransform(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal, CXform *pxform);
		...
		BOOL FValidContext(IMemoryPool *mp,
	...
	public:
		// ctor
		CGroupExpression(IMemoryPool *mp,
						 COperator *pop,
		...
		CCostContext *PccComputeCost(IMemoryPool *mp,
									 COptimizationContext *poc,
									 ULONG ulOptReq,
		...
		CCost CostLowerBound(IMemoryPool *mp,
		...
		static BOOL
		Equals(const CGroupExpression &gexprLeft, const CGroupExpression &gexprRight)
		{
