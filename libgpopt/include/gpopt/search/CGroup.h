...
namespace gpopt
{
	...
	private:
		...
		typedef CHashMap<SContextLink,
						 BOOL,
						 SContextLink::HashValue,
						 SContextLink::Equals,
						 CleanupDelete<SContextLink>,
						 CleanupDelete<BOOL> >
			LinkMap;

		// map of computed stats objects during costing
		typedef CHashMap<COptimizationContext,
						 IStatistics,
						 COptimizationContext::UlHashForStats,
						 COptimizationContext::FEqualForStats,
						 CleanupRelease<COptimizationContext>,
						 CleanupRelease<IStatistics> >
		...
		BOOL FBetterPromise(IMemoryPool *mp,
		...
		CLogical::EStatPromise EspDerive(IMemoryPool *pmpLocal,
		...
		void RecursiveBuildTreeMap(IMemoryPool *mp,
		...
		CGroupExpression *PgexprBestPromise(IMemoryPool *pmpLocal,
											IMemoryPool *pmpGlobal,
	...
	public:
		...
		IStatistics *PstatsRecursiveDerive(IMemoryPool *pmpLocal,
										   IMemoryPool *pmpGlobal,
										   CReqdPropRelational *prprel,
										   IStatisticsArray *stats_ctxt);
		...
		void BuildTreeMap(IMemoryPool *mp,
						  COptimizationContext *poc,
						  CCostContext *pccParent,
						  ULONG child_index,
						  CTreeMap<CCostContext,
