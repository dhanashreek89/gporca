...
namespace gpopt
{
	...
	private:
		...
		virtual CExpression *PexprConstructScalar(IMemoryPool *mp) const;

		virtual CExpression *PexprConstructArrayScalar(IMemoryPool *mp) const;

		// create interval from scalar comparison expression
		static CConstraintInterval *PciIntervalFromScalarCmp(IMemoryPool *mp,
															 CExpression *pexpr,
															 CColRef *colref);

		static CConstraintInterval *PciIntervalFromScalarIDF(IMemoryPool *mp,
															 CExpression *pexpr,
															 CColRef *colref);

		// create interval from scalar bool operator
		static CConstraintInterval *PciIntervalFromScalarBoolOp(IMemoryPool *mp,
																CExpression *pexpr,
																CColRef *colref);

		// create interval from scalar bool AND
		static CConstraintInterval *PciIntervalFromScalarBoolAnd(IMemoryPool *mp,
																 CExpression *pexpr,
																 CColRef *colref);

		// create interval from scalar bool OR
		static CConstraintInterval *PciIntervalFromScalarBoolOr(IMemoryPool *mp,
																CExpression *pexpr,
																CColRef *colref);

		// create interval from scalar null test
		static CConstraintInterval *PciIntervalFromScalarNullTest(IMemoryPool *mp,
																  CExpression *pexpr,
																  CColRef *colref);

		// creates a range like [x,x] where x is a constant
		static CRangeArray *PciRangeFromColConstCmp(IMemoryPool *mp,
													IMDType::ECmpType cmp_type,
													const CScalarConst *popScConst);

		// create an array IN or NOT IN expression
		CExpression *PexprConstructArrayScalar(IMemoryPool *mp, bool isIn) const;

	public:
		// ctor
		CConstraintInterval(IMemoryPool *mp,
		...
		static CConstraintInterval *PciUnbounded(IMemoryPool *mp,
												 const CColRef *colref,
												 BOOL fIncludesNull);
		...
		static CConstraintInterval *PciIntervalFromColConstCmp(IMemoryPool *mp,
															   CColRef *colref,
															   IMDType::ECmpType cmp_type,
															   CScalarConst *popScConst);

		// create interval from scalar expression
		static CConstraintInterval *PciIntervalFromScalarExpr(IMemoryPool *mp,
															  CExpression *pexpr,
															  CColRef *colref);

		// create interval from any general constraint that references
		// only one column
		static CConstraintInterval *PciIntervalFromConstraint(IMemoryPool *mp,
															  CConstraint *pcnstr,
															  CColRef *colref = NULL);

		// generate a ConstraintInterval from the given expression
		static CConstraintInterval *PcnstrIntervalFromScalarArrayCmp(IMemoryPool *mp,
																	 CExpression *pexpr,
																	 CColRef *colref);
