...
namespace gpopt
{
	...
	// of pointer values
	typedef CHashMap<CConstraint,
					 BOOL,
					 gpos::HashPtr<CConstraint>,
					 gpos::EqualPtr<CConstraint>,
					 CleanupNULL<CConstraint>,
					 CleanupNULL<BOOL> >
	...
	private:
		...
		static void AddColumnToEquivClasses(IMemoryPool *mp,
											const CColRef *colref,
											CColRefSetArray **ppdrgpcrs);

		// create constraint from scalar comparison
		static CConstraint *PcnstrFromScalarCmp(IMemoryPool *mp,
												CExpression *pexpr,
												CColRefSetArray **ppdrgpcrs);

		// create constraint from scalar boolean expression
		static CConstraint *PcnstrFromScalarBoolOp(IMemoryPool *mp,
												   CExpression *pexpr,
												   CColRefSetArray **ppdrgpcrs);

		// create conjunction/disjunction from array of constraints
		static CConstraint *PcnstrConjDisj(IMemoryPool *mp,
	...
	protected:
		...
		static CConstraint *PcnstrFromScalarArrayCmp(IMemoryPool *mp,
													 CExpression *pexpr,
													 CColRef *colref);

	public:
		// ctor
		explicit CConstraint(IMemoryPool *mp);
		...
		// scalar expression
		virtual CExpression *PexprScalar(IMemoryPool *mp) = 0;
		...
		virtual CConstraint *PcnstrRemapForColumn(IMemoryPool *mp, CColRef *colref) const = 0;
		...
		// equivalence classes
		static CConstraint *PcnstrFromScalarExpr(IMemoryPool *mp,
												 CExpression *pexpr,
												 CColRefSetArray **ppdrgpcrs);

		// create conjunction from array of constraints
		static CConstraint *PcnstrConjunction(IMemoryPool *mp, CConstraintArray *pdrgpcnstr);

		// create disjunction from array of constraints
		static CConstraint *PcnstrDisjunction(IMemoryPool *mp, CConstraintArray *pdrgpcnstr);

		// merge equivalence classes coming from children of a bool op
		static CColRefSetArray *PdrgpcrsMergeFromBoolOp(IMemoryPool *mp,
														CExpression *pexpr,
														CColRefSetArray *pdrgpcrsFst,
														CColRefSetArray *pdrgpcrsSnd);

		// subset of the given constraints, which reference the given column
		static CConstraintArray *PdrgpcnstrOnColumn(IMemoryPool *mp,
													CConstraintArray *pdrgpcnstr,
													CColRef *colref,
													BOOL fExclusive);
#ifdef GPOS_DEBUG
