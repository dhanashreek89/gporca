...
namespace gpopt
{
	...
	//
	//---------------------------------------------------------------------------
	template <class TEnumState,
			  TEnumState tenumstateSentinel,
			  class TEnumEvent,
...
#define GRAPHVIZ_BOX(x) GRAPHVIZ_SHAPE("box", x)
		...
		void
		Transitions(TEnumState tenumstateOld, TEnumState tenumstateNew, EsetEvents *peset) const
		{
		...
		typedef void (*PfWalker)(const CStateMachine *psm,
		...
		void
		Walk(IMemoryPool *mp, PfWalker Pfpv, void *pvContext) const
		{
			// retrieve all states
		...
		static void
		Diagram(const CStateMachine *psm,
				...
				void *pvContext)
		{
			...
			os << "\"" << psm->WszState(tenumstateSource) << "\" -> \""
			   << psm->WszState(tenumstateSink) << "\" [ label = \"" << psm->WszEvent(tenumevent)
			   << "\" ];" << std::endl;
		}
		...
		static void
		Unreachable(const CStateMachine *,
					TEnumState tenumstateSource,
					TEnumState tenumstateSink,
					TEnumEvent,
					void *pvContext)
		...
		BOOL
		FAttemptTransition(TEnumState tenumstateOld,
						   TEnumEvent tenumevent,
						   TEnumState &tenumstateNew) const
		{
			...
			for (ULONG ulOuter = 0; ulOuter < tenumstateSentinel; ulOuter++)
			{
				if (m_rgrgtenumeventTransitions[tenumstateOld][ulOuter] == tenumevent)
				{
					...
					{
						GPOS_ASSERT(m_rgrgtenumeventTransitions[tenumstateOld][ulInner] !=
									tenumevent);
					}
#endif  // GPOS_DEBUG
	...
	public:
		...
		void
		Init(const TEnumEvent rgrgtenumeventTransitions[tenumstateSentinel][tenumstateSentinel]
#ifdef GPOS_DEBUG
			 ,
		...
		OsHistory(IOstream &os) const
		{
			...
			for (ULONG ul = 0; ul < ulElems; ul++)
			{
				...
				os << ul << ": " << WszEvent(m_tenumeventHistory[ulPos]) << " (event) -> "
				   << WszState(m_tenumstateHistory[ulPos]) << " (state)" << std::endl;
			}
		...
		BOOL
		FReachable(IMemoryPool *mp) const
		{
		...
		void
		Unreachable(IMemoryPool *mp, TEnumState **ppestate, ULONG *pulSize) const
		{
		...
		IOstream &
		OsDiagramToGraphviz(IMemoryPool *mp, IOstream &os, const WCHAR *wszTitle) const
		{
