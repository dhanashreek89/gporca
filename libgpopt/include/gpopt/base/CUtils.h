...
namespace gpopt
{
	...
	public:
		...
		static IOstream &OsPrintDrgPcoldesc(IOstream &os,
											CColumnDescriptorArray *pdrgpcoldescIncludedCols,
											ULONG length);
#endif  // GPOS_DEBUG
		...
		static void ValidateCTEProducerConsumerLocality(IMemoryPool *mp,
														CExpression *pexpr,
		...
		static BOOL FCmpOrCastedCmpExists(IMDId *left_mdid,
										  IMDId *right_mdid,
										  IMDType::ECmpType cmp_type);

		// return the mdid of the given scalar comparison between the two types
		static IMDId *GetScCmpMdId(IMemoryPool *mp,
		...
		static CExpression *PexprScalarCmp(IMemoryPool *mp,
		...
		static CExpression *PexprScalarArrayCmp(IMemoryPool *mp,
		...
		static CExpression *PexprIDF(IMemoryPool *mp,
									 CExpression *pexprLeft,
									 CExpression *pexprRight);

		// generate an Is NOT Distinct From expression for two column references
		static CExpression *PexprINDF(IMemoryPool *mp,
									  const CColRef *pcrLeft,
									  const CColRef *pcrRight);
		...
		static CExpression *PexprIsNull(IMemoryPool *mp, CExpression *pexpr);

		// generate an Is NOT NULL expression
		static CExpression *PexprIsNotNull(IMemoryPool *mp, CExpression *pexpr);

		// generate an Is NOT FALSE expression
		static CExpression *PexprIsNotFalse(IMemoryPool *mp, CExpression *pexpr);

		// find if a scalar expression uses a nullable columns from the output of a logical expression
		static BOOL FUsesNullableCol(IMemoryPool *mp,
									 CExpression *pexprScalar,
									 CExpression *pexprLogical);

		// generate a scalar op expression for a column reference and an expression
		static CExpression *PexprScalarOp(IMemoryPool *mp,
										  const CColRef *pcrLeft,
										  CExpression *pexpr,
										  CWStringConst strOp,
										  IMDId *mdid_op,
		...
		static CExpression *PexprScalarBoolOp(IMemoryPool *mp,
											  CScalarBoolOp::EBoolOperator eboolop,
											  CExpressionArray *pdrgpexpr);

		// negate the given expression
		static CExpression *PexprNegate(IMemoryPool *mp, CExpression *pexpr);

		// generate a scalar ident expression
		static CExpression *PexprScalarIdent(IMemoryPool *mp, const CColRef *colref);

		// generate a scalar project element expression
		static CExpression *PexprScalarProjectElement(IMemoryPool *mp,
													  CColRef *colref,
													  CExpression *pexpr);
		...
		static CExpression *PexprAggFunc(IMemoryPool *mp,
										 IMDId *pmdidAggFunc,
										 const CWStringConst *pstrAggFunc,
										 const CColRef *colref,
										 BOOL is_distinct,
		...
		static CExpression *PexprCountStar(IMemoryPool *mp);

		// generate a GbAgg with count(*) function over the given expression
		static CExpression *PexprCountStar(IMemoryPool *mp, CExpression *pexprLogical);
		...
		static CExpression *PexprCountStarAndSum(IMemoryPool *mp,
												 const CColRef *colref,
												 CExpression *pexprLogical);

		// generate a sum(col) expression
		static CExpression *PexprSum(IMemoryPool *mp, const CColRef *colref);

		// generate a GbAgg with sum(col) expressions for all columns in the given array
		static CExpression *PexprGbAggSum(IMemoryPool *mp,
										  CExpression *pexprLogical,
										  CColRefArray *pdrgpcrSum);

		// generate a count(col) expression
		static CExpression *PexprCount(IMemoryPool *mp, const CColRef *colref, BOOL is_distinct);

		// generate a min(col) expression
		static CExpression *PexprMin(IMemoryPool *mp,
									 CMDAccessor *md_accessor,
									 const CColRef *colref);

		// generate an aggregate expression
		static CExpression *PexprAgg(IMemoryPool *mp,
									 CMDAccessor *md_accessor,
		...
		static CExpression *PexprLogicalProjectNulls(IMemoryPool *mp,
		...
		static CExpression *PexprScalarProjListConst(IMemoryPool *mp,
													 CColRefArray *colref_array,
													 IDatumArray *pdrgpdatum,
													 UlongToColRefMap *colref_mapping);

		// generate a project expression
		static CExpression *PexprAddProjection(IMemoryPool *mp,
											   CExpression *pexpr,
		...
		// generate an aggregate expression
		static CExpression *PexprLogicalGbAggGlobal(IMemoryPool *mp,
													CColRefArray *colref_array,
													CExpression *pexpr,
		...
		static CExpression *PexprScalarConstBool(IMemoryPool *mp, BOOL value, BOOL is_null = false);

		// generate an int4 expression
		static CExpression *PexprScalarConstInt4(IMemoryPool *mp, INT val);

		// generate an int8 expression
		static CExpression *PexprScalarConstInt8(IMemoryPool *mp, LINT val, BOOL is_null = false);

		// generate an oid constant expression
		static CExpression *PexprScalarConstOid(IMemoryPool *mp, OID oid_val);
		...
		static CColRef *PcrExtractPartKey(CColRefArrays *pdrgpdrgpcr, ULONG ulLevel);
		...
		static CExpressionArray *PdrgpexprDedup(IMemoryPool *mp, CExpressionArray *pdrgpexpr);
		...
		static CColRefArray *PdrgpcrNonSystemCols(IMemoryPool *mp, CColRefSet *pcrs);

		// create an array of output columns including a key for grouping
		static CColRefArray *PdrgpcrGroupingKey(IMemoryPool *mp,
												CExpression *pexpr,
		...
		static CColRefSetArray *PdrgpcrsAddEquivClass(IMemoryPool *mp,
													  CColRefSet *pcrsNew,
													  CColRefSetArray *pdrgpcrs);

		// merge 2 arrays of equivalence classes
		static CColRefSetArray *PdrgpcrsMergeEquivClasses(IMemoryPool *mp,
														  CColRefSetArray *pdrgpcrsFst,
														  CColRefSetArray *pdrgpcrsSnd);

		// intersect 2 arrays of equivalence classes
		static CColRefSetArray *PdrgpcrsIntersectEquivClasses(IMemoryPool *mp,
															  CColRefSetArray *pdrgpcrsFst,
															  CColRefSetArray *pdrgpcrsSnd);

		// return a copy of equivalence classes from all children
		static CColRefSetArray *PdrgpcrsCopyChildEquivClasses(IMemoryPool *mp,
															  CExpressionHandle &exprhdl);

		// return a copy of the given array of columns, excluding the columns
		// in the given colrefset
		static CColRefArray *PdrgpcrExcludeColumns(IMemoryPool *mp,
		...
		static CExpression *PScalarArrayExprChildAt(IMemoryPool *mp,
		...
		static CExpression *PexprCollapseConstArray(IMemoryPool *mp, CExpression *pexprArray);
		...
		static CColRefArray *PdrgpcrRedistributableSubset(IMemoryPool *mp,
														  CColRefArray *colref_array);
		...
		static CHAR *CreateMultiByteCharStringFromWCString(IMemoryPool *mp, WCHAR *wsz);
		...
		static BOOL FFunctionallyDependent(IMemoryPool *mp,
										   CDrvdPropRelational *pdprel,
										   CColRefSet *pcrsKey,
										   CColRef *colref);

		// construct an array of colids from the given array of column references
		static ULongPtrArray *Pdrgpul(IMemoryPool *mp, const CColRefArray *colref_array);

		// generate a timestamp-based file name
		static void GenerateFileName(CHAR *buf,
									 const CHAR *szPrefix,
									 const CHAR *szExt,
									 ULONG length,
									 ULONG ulSessionId,
		...
		static CColRefSet *PcrsRemap(IMemoryPool *mp,
									 CColRefSet *pcrs,
									 UlongToColRefMap *colref_mapping,
									 BOOL must_exist);
		...
		// and based on the given mapping
		static CColRefArray *PdrgpcrRemap(IMemoryPool *mp,
										  CColRefArray *colref_array,
										  UlongToColRefMap *colref_mapping,
										  BOOL must_exist);
		...
		static CColRefArray *PdrgpcrCopy(IMemoryPool *mp,
										 CColRefArray *colref_array,
		...
		static CColRefSet *PcrsCTEProducerColumns(IMemoryPool *mp,
		...
		static CExpression *PexprConjINDFCond(IMemoryPool *mp, CColRefArrays *pdrgpdrgpcrInput);
		...
		static CExpression *PexprCast(IMemoryPool *mp,
									  CMDAccessor *md_accessor,
									  CExpression *pexpr,
									  IMDId *mdid_dest);
		...
		static CExpressionArray *PdrgpexprScalarIdents(IMemoryPool *mp, CColRefArray *colref_array);

		// return the columns from the scalar ident expressions in the given array
		static CColRefSet *PcrsExtractColumns(IMemoryPool *mp, const CExpressionArray *pdrgpexpr);

		// create a new bitset of the given length, where all the bits are set
		static CBitSet *PbsAllSet(IMemoryPool *mp, ULONG size);

		// return a new bitset, setting the bits in the given array
		static CBitSet *Pbs(IMemoryPool *mp, ULongPtrArray *pdrgpul);
		...
		static UlongToConstraintMap *PhmulcnstrBoolConstOnPartKeys(
			IMemoryPool *mp, CColRefArrays *pdrgpdrgpcrPartKey, BOOL value);
		...
		static CExpression *PexprLogicalCTGDummy(IMemoryPool *mp);
		...
		static BOOL FMotionOverUnresolvedPartConsumers(IMemoryPool *mp,
													   CExpressionHandle &exprhdl,
		...
		static CExpression *PexprCollapseProjects(IMemoryPool *mp, CExpression *pexpr);
		...
		static BOOL FEquivalanceClassesDisjoint(IMemoryPool *mp, const CColRefSetArray *pdrgpcrs);

		// check if the equivalance classes are same
		static BOOL FEquivalanceClassesEqual(IMemoryPool *mp,
											 CColRefSetArray *pdrgpcrsFst,
											 CColRefSetArray *pdrgpcrsSnd);
		...
		static CExpression *PexprLimit(IMemoryPool *mp,
									   CExpression *pexpr,
									   ULONG ulOffSet,
									   ULONG count);
		...
		const static CColRef *PcrExtractFromScExpression(CExpression *pexpr);
	};  // class CUtils

	// hash set from expressions
	typedef CHashSet<CExpression,
	...
	template <class T>
	CExpression *
	CUtils::PexprLogicalJoin(IMemoryPool *mp,
							 CExpression *pexprLeft,
							 CExpression *pexprRight,
							 CExpression *pexprPredicate)
	{
		...
		return GPOS_NEW(mp)
			CExpression(mp, GPOS_NEW(mp) T(mp), pexprLeft, pexprRight, pexprPredicate);
	}
	...
	template <class T>
	CExpression *
	CUtils::PexprLogicalApply(IMemoryPool *mp,
							  CExpression *pexprLeft,
							  CExpression *pexprRight,
	...
	template <class T>
	CExpression *
	CUtils::PexprLogicalCorrelatedQuantifiedApply(IMemoryPool *mp,
												  CExpression *pexprLeft,
												  CExpression *pexprRight,
												  CColRefArray *pdrgpcrInner,
												  COperator::EOperatorId eopidOriginSubq,
	...
	CUtils::FScalarConstInt(CExpression *pexpr)
	{
		...
		GPOS_ASSERT(IMDType::EtiInt2 == type_info || IMDType::EtiInt4 == type_info ||
