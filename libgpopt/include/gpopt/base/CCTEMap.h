...
namespace gpopt
{
	...
	private:
		...
		public:
			...
			HashValue() const
			{
				return gpos::CombineHashes(gpos::HashValue<ULONG>(&m_id),
		...
		static void AddUnresolved(const CCTEMap &cmFirst,
								  const CCTEMap &cmSecond,
								  CCTEMap *pcmResult);

	public:
		// ctor
		explicit CCTEMap(IMemoryPool *mp);
		...
		static CCTEMap *PcmCombine(IMemoryPool *mp,
