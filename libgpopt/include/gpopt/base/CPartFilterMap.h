...
namespace gpopt
{
	...
	private:
		...
		public:
			// ctor
			CPartFilter(ULONG scan_id, CExpression *pexpr, IStatistics *stats = NULL);
	...
	public:
		// ctors
		explicit CPartFilterMap(IMemoryPool *mp);
		...
		Equals(CPartFilterMap *ppfm)
		{
			...
			return (m_phmulpf->Size() == ppfm->m_phmulpf->Size()) && this->FSubset(ppfm);
		}
		...
		void AddPartFilter(IMemoryPool *mp, ULONG scan_id, CExpression *pexpr, IStatistics *stats);
