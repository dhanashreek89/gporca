...
namespace gpopt
{
	...
	public:
		// ctor
		explicit CColRefSet(IMemoryPool *mp, ULONG ulSizeBits = GPOPT_COLREFSET_SIZE);

		explicit CColRefSet(IMemoryPool *mp, const CColRefSet &);

		// ctor, copy from col refs array
		CColRefSet(IMemoryPool *mp,
		...
		// debug print
		IOstream &OsPrint(IOstream &os, ULONG ulLenMax = gpos::ulong_max) const;
