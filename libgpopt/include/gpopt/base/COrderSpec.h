...
namespace gpopt
{
	...
	public:
		// ctor
		explicit COrderSpec(IMemoryPool *mp);
		...
		virtual COrderSpec *PosCopyWithRemappedColumns(IMemoryPool *mp,
													   UlongToColRefMap *colref_mapping,
													   BOOL must_exist);

		// return a copy of the order spec after excluding the given columns
		virtual COrderSpec *PosExcludeColumns(IMemoryPool *mp, CColRefSet *pcrs);
		...
		static CColRefSet *GetColRefSet(IMemoryPool *mp, COrderSpecArray *pdrgpos);

		// filter out array of order specs from order expressions using the passed columns
		static COrderSpecArray *PdrgposExclude(IMemoryPool *mp,
