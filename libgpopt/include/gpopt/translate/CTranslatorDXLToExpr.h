...
namespace gpopt
{
	...
	private:
		...
		CExpression *PexprCastPrjElem(IMDId *pmdidSource,
									  IMDId *mdid_dest,
		...
		CExpressionArray *PdrgpexprPreprocessSetOpInputs(const CDXLNode *dxlnode,
		...
		void ConstructDXLColId2ColRefMapping(const CDXLColDescrArray *dxl_col_descr_array,
	...
	public:
		// ctor
		CTranslatorDXLToExpr(IMemoryPool *mp,
		...
		CExpression *PexprTranslateQuery(const CDXLNode *dxlnode,
										 const CDXLNodeArray *query_output_dxlnode_array,
										 const CDXLNodeArray *cte_producers);
