...
namespace gpopt
{
	...
	protected:
		...
		static IMDFunction::EFuncStbl EfsDeriveFromChildren(CExpressionHandle &exprhdl,
															IMDFunction::EFuncStbl efsDefault);

		// derive function properties from children
		static CFunctionProp *PfpDeriveFromChildren(IMemoryPool *mp,
	...
	public:
		...
		// ctor
		explicit COperator(IMemoryPool *mp);
		...
		virtual DrvdPropArray *PdpCreate(IMemoryPool *mp) const = 0;

		// create container for required properties
		virtual CReqdProp *PrpCreate(IMemoryPool *mp) const = 0;

		// return empty container;
		// caller adds outer references using property derivation
		virtual CColRefSet *
		PcrsOuter(IMemoryPool *mp)
		{
