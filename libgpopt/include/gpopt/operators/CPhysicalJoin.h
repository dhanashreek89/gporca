...
namespace gpopt
{
	...
	protected:
		...
		public:
			...
			// equality function
			static BOOL Equals(const CPartPropReq *ppprFst, const CPartPropReq *ppprSnd);

		};  // class CPartPropReq

		// map partition propagation request to partition propagation spec
		typedef CHashMap<CPartPropReq,
						 CPartitionPropagationSpec,
						 CPartPropReq::HashValue,
						 CPartPropReq::Equals,
		...
		// ctor
		explicit CPhysicalJoin(IMemoryPool *mp);
		...
		CDistributionSpec *PdsRequiredCorrelatedJoin(IMemoryPool *mp,
		...
		CRewindabilitySpec *PrsRequiredCorrelatedJoin(IMemoryPool *mp,
													  CExpressionHandle &exprhdl,
		...
		CPartPropReq *PpprCreate(IMemoryPool *mp,
								 CExpressionHandle &exprhdl,
								 CPartitionPropagationSpec *pppsRequired,
								 ULONG child_index);

		// compute required partition propagation of the n-th child
		CPartitionPropagationSpec *PppsRequiredCompute(IMemoryPool *mp,
													   CExpressionHandle &exprhdl,
													   CPartitionPropagationSpec *pppsRequired,
													   ULONG child_index,
													   BOOL fNLJoin);
		...
		CPartitionPropagationSpec *PppsRequiredJoinChild(IMemoryPool *mp,
														 CExpressionHandle &exprhdl,
														 CPartitionPropagationSpec *pppsRequired,
														 ULONG child_index,
														 CDrvdPropArrays *pdrgpdpCtxt,
		...
		static COrderSpec *PosPropagateToOuter(IMemoryPool *mp,
											   CExpressionHandle &exprhdl,
											   COrderSpec *posRequired);

		// helper for checking if required sort columns come from outer child
		static BOOL FSortColsInOuterChild(IMemoryPool *mp,
										  CExpressionHandle &exprhdl,
										  COrderSpec *pos);
		...
		static void AddHashKeys(CExpression *pexprPred,
		...
		static void AddFilterOnPartKey(IMemoryPool *mp,
									   BOOL fNLJoin,
		...
		static CExpression *PexprJoinPredOnPartKeys(IMemoryPool *mp,
													CExpression *pexprScalar,
													CPartIndexMap *ppimSource,
													ULONG part_idx_id,
													CColRefSet *pcrsAllowedRefs);
		...
		static BOOL FHashJoinCompatible(CExpression *pexprOuter,
										CExpression *pexprInner,
	...
	public:
		...
		static void ExtractHashJoinExpressions(CExpression *pexprPred,
											   CExpression **ppexprLeft,
											   CExpression **ppexprRight);

		// can expression be implemented with hash join
		static BOOL FHashJoinPossible(
			IMemoryPool *mp,
			CExpression *pexpr,
			CExpressionArray *pdrgpexprOuter,
			CExpressionArray *pdrgpexprInner,
