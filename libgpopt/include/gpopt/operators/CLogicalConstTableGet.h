...
namespace gpopt
{
	...
	private:
		...
		CColumnDescriptorArray *PdrgpcoldescMapping(IMemoryPool *mp,
													CColRefArray *colref_array) const;

	public:
		// ctors
		explicit CLogicalConstTableGet(IMemoryPool *mp);
							...
							) const
		{
			...
			return GPOS_NEW(mp)
				CPropConstraint(mp, GPOS_NEW(mp) CColRefSetArray(mp), NULL /*pcnstr*/);
		}
