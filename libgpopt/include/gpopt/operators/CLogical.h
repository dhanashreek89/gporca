...
namespace gpopt
{
	...
	protected:
		...
		static CColRefSet *PcrsDeriveOutputCombineLogical(IMemoryPool *mp,
														  CExpressionHandle &exprhdl);

		// helper for common case of combining not nullable columns from all logical children
		static CColRefSet *PcrsDeriveNotNullCombineLogical(IMemoryPool *mp,
														   CExpressionHandle &exprhdl);

		// helper for common case of stat columns computation
		static CColRefSet *PcrsReqdChildStats(IMemoryPool *mp,
											  CExpressionHandle &exprhdl,
		...
		static CKeyCollection *PkcCombineKeys(IMemoryPool *mp, CExpressionHandle &exprhdl);

		// helper function for computing the keys in a base relation
		static CKeyCollection *PkcKeysBaseTable(IMemoryPool *mp,
		...
		static CPartInfo *PpartinfoDeriveCombine(IMemoryPool *mp, CExpressionHandle &exprhdl);

		// derive constraint property from a table/index get
		static CPropConstraint *PpcDeriveConstraintFromTable(IMemoryPool *mp,
		...
		static CPropConstraint *PpcDeriveConstraintPassThru(CExpressionHandle &exprhdl,
															ULONG ulChild);

		// derive constraint property only on the given columns
		static CPropConstraint *PpcDeriveConstraintRestrict(IMemoryPool *mp,
															CExpressionHandle &exprhdl,
		...
		static COrderSpec *PosFromIndex(IMemoryPool *mp,
										const IMDIndex *pmdindex,
										CColRefArray *colref_array,
										const CTableDescriptor *ptabdesc);

		// derive function properties using data access property of scalar child
		static CFunctionProp *PfpDeriveFromScalar(IMemoryPool *mp,
												  CExpressionHandle &exprhdl,
												  ULONG ulScalarIndex);

		// derive outer references
		static CColRefSet *PcrsDeriveOuter(IMemoryPool *mp,
										   CExpressionHandle &exprhdl,
										   CColRefSet *pcrsUsedAdditional);

	public:
		// ctor
		explicit CLogical(IMemoryPool *mp);
		...
		virtual CColRefSet *PcrsDeriveOutput(IMemoryPool *mp, CExpressionHandle &exprhdl) = 0;

		// derive outer references
		virtual CColRefSet *
		PcrsDeriveOuter(IMemoryPool *mp, CExpressionHandle &exprhdl)
		{
		...
		virtual CColRefSet *PcrsDeriveOuterIndexGet(IMemoryPool *mp, CExpressionHandle &exprhdl);
		...
		virtual CColRefSet *PcrsDeriveCorrelatedApply(IMemoryPool *mp,
		...
		// derive join depth
		virtual ULONG JoinDepth(IMemoryPool *mp, CExpressionHandle &exprhdl) const;

		// derive partition information
		virtual CPartInfo *PpartinfoDerive(IMemoryPool *mp, CExpressionHandle &exprhdl) const = 0;

		// derive constraint property
		virtual CPropConstraint *PpcDeriveConstraint(IMemoryPool *mp,
		...
		virtual CXformSet *PxfsCandidates(IMemoryPool *mp) const = 0;
		...
		static IStatistics *PstatsBaseTable(IMemoryPool *mp,
											CExpressionHandle &exprhdl,
		...
		static CColRefSet *PcrsDist(IMemoryPool *mp,
									const CTableDescriptor *ptabdesc,
									const CColRefArray *colref_array);

		// derive constraint property when expression has relational children and predicates
		static CPropConstraint *PpcDeriveConstraintFromPredicates(IMemoryPool *mp,
																  CExpressionHandle &exprhdl);
