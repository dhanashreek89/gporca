...
namespace gpopt
{
	...
	private:
		...
		typedef CHashMap<CColRef,
						 CExpressionArrays,
						 gpos::HashPtr<CColRef>,
						 gpos::EqualPtr<CColRef>,
						 CleanupNULL<CColRef>,
						 CleanupRelease<CExpressionArrays> >
			ColumnToArrayPosMap;

		// iterator for map of column to disjunctive form representation
		typedef CHashMapIter<CColRef,
							 CExpressionArrays,
							 gpos::HashPtr<CColRef>,
							 gpos::EqualPtr<CColRef>,
							 CleanupNULL<CColRef>,
		...
		static ExprMap *PexprmapFactors(IMemoryPool *mp, CExpression *pexpr);

		// factorize common expressions in Or tree
		static CExpression *PexprFactorizeDisj(IMemoryPool *mp,
											   CExpression *pexpr,
											   CExpression *  // pexprLowestLogicalAncestor
		);

		// visitor-like function to process descendents that are OR operators
		static CExpression *PexprProcessDisjDescendents(IMemoryPool *mp,
														CExpression *pexpr,
														CExpression *pexprLowestLogicalAncestor,
														PexprProcessDisj pexprorfun);

		// discover common factors in scalar expression
		static CExpression *PexprDiscoverFactors(IMemoryPool *mp, CExpression *pexpr);
		...
		static void StoreBaseOpToColumnExpr(IMemoryPool *mp,
											CExpression *pexpr,
											SourceToArrayPosMap *psrc2array,
											ColumnToArrayPosMap *pcol2array,
		...
		static void AddInferredFiltersFromArray(IMemoryPool *mp,
		...
		// of the given maps
		static CExpression *PexprAddInferredFilters(IMemoryPool *mp,
													CExpression *pexpr,
													SourceToArrayPosMap *psrc2array,
													ColumnToArrayPosMap *pcol2array);
		...
		//	expression
		static CColRefSet *PcrsColumnsProducedByChildren(IMemoryPool *mp, CExpression *pexpr);
		...
		static CExpression *PexprExtractInferredFiltersFromDisj(
			IMemoryPool *mp, CExpression *pexpr, CExpression *pexprLowestLogicalAncestor);

	public:
		// factorize common expressions
		static CExpression *PexprFactorize(IMemoryPool *mp, CExpression *pexpr);

		// compute disjunctive pre-filters that can be pushed to the column creators
		static CExpression *PexprExtractInferredFilters(IMemoryPool *mp, CExpression *pexpr);
	};
}  // namespace gpopt
