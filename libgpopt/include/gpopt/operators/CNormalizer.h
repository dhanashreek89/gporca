...
namespace gpopt
{
	...
	private:
		...
		typedef void(FnPushThru)(IMemoryPool *mp,
		...
		static BOOL FSimplifySelectOnOuterJoin(IMemoryPool *mp,
											   CExpression *pexprOuterJoin,
											   CExpression *pexprPred,
											   CExpression **ppexprResult);

		// call normalizer recursively on expression children
		static CExpression *PexprRecursiveNormalize(IMemoryPool *mp, CExpression *pexpr);
		...
		static void SplitConjunctForSeqPrj(IMemoryPool *mp,
										   CExpression *pexprSeqPrj,
										   CExpression *pexprConj,
										   CExpressionArray **ppdrgpexprPushable,
										   CExpressionArray **ppdrgpexprUnpushable);
		...
		static void PushThruOuterChild(IMemoryPool *mp,
									   CExpression *pexpr,
									   CExpression *pexprConj,
									   CExpression **ppexprResult);

		// push scalar expression through a join expression
		static void PushThruJoin(IMemoryPool *mp,
								 CExpression *pexprJoin,
								 CExpression *pexprConj,
								 CExpression **ppexprResult);

		// push scalar expression through logical expression
		static void PushThru(IMemoryPool *mp,
							 CExpression *pexprLogical,
							 CExpression *pexprConj,
							 CExpression **ppexprResult);
		...
		static CExpression *PexprPullUpAndCombineProjects(IMemoryPool *mp,
														  CExpression *pexpr,
														  BOOL *pfSuccess);

		// pull up project elements from the given projection expression that do not
		// exist in the given used columns set
		static CExpression *PexprPullUpProjectElements(IMemoryPool *mp,
													   CExpression *pexpr,
													   CColRefSet *pcrsUsed,
		...
		static BOOL FLocalColsSubsetOfInputCols(IMemoryPool *mp, CExpression *pexpr);
#endif  //GPOS_DEBUG

	public:
		// main driver
		static CExpression *PexprNormalize(IMemoryPool *mp, CExpression *pexpr);

		// normalize joins so that they are on colrefs
		static CExpression *PexprNormalizeJoins(IMemoryPool *mp, CExpression *pexpr);

		// pull logical projects as far up the logical tree as possible, and
		// combine consecutive projects if possible
		static CExpression *PexprPullUpProjections(IMemoryPool *mp, CExpression *pexpr);
