...
namespace gpopt
{
	...
	private:
		...
		static CExpression *PexprIndexLookupKeyOnLeft(IMemoryPool *mp,
		...
		static CExpression *PexprIndexLookupKeyOnRight(IMemoryPool *mp,
													   CMDAccessor *md_accessor,
													   CExpression *pexprScalar,
													   const IMDIndex *pmdindex,
													   CColRefArray *pdrgpcrIndex,
		...
		static CExpression *PexprReplaceColsWithNulls(IMemoryPool *mp,
													  CExpression *pexprScalar,
													  CColRefSet *pcrs);
	...
	public:
		...
		static BOOL FConjunctionOfEqComparisons(IMemoryPool *mp, CExpression *pexpr);
		...
		static CExpression *PexprINDFConjunction(IMemoryPool *mp,
		...
		static BOOL FRangeOrEqComp(CExpression *pexpr);
		...
		static void CollectChildren(CExpression *pexpr,
		...
		static void CollectGrandChildrenUnionUnionAll(IMemoryPool *mp,
													  CExpression *pexpr,
													  ULONG child_index,
		...
		static CExpressionArray *PdrgpexprConjuncts(IMemoryPool *mp, CExpression *pexpr);

		// extract disjuncts from a scalar tree
		static CExpressionArray *PdrgpexprDisjuncts(IMemoryPool *mp, CExpression *pexpr);

		// extract equality predicates on scalar identifier from a list of scalar expressions
		static CExpressionArray *PdrgpexprPlainEqualities(IMemoryPool *mp,
														  CExpressionArray *pdrgpexpr);

		// create conjunction/disjunction
		static CExpression *PexprConjDisj(IMemoryPool *mp,
										  CExpressionArray *Pdrgpexpr,
										  BOOL fConjunction);
		...
		// create conjunction
		static CExpression *PexprConjunction(IMemoryPool *mp, CExpressionArray *pdrgpexpr);

		// create conjunction of two expressions
		static CExpression *PexprConjunction(IMemoryPool *mp,
											 CExpression *pexprOne,
											 CExpression *pexprTwo);

		// create disjunction of two expressions
		static CExpression *PexprDisjunction(IMemoryPool *mp,
											 CExpression *pexprOne,
											 CExpression *pexprTwo);

		// expand disjuncts in the given array by converting ArrayComparison to AND/OR tree and deduplicating resulting disjuncts
		static CExpressionArray *PdrgpexprExpandDisjuncts(IMemoryPool *mp,
		...
		static CExpressionArray *PdrgpexprExpandConjuncts(IMemoryPool *mp,
		...
		static BOOL FDisjunctionOnColumn(IMemoryPool *mp,
										 CExpression *pexpr,
										 CColRef *colref,
		...
		// LT, GT, LEq, GEq, Eq
		static BOOL FRangeComparison(IMDType::ECmpType cmp_type);

		// create disjunction
		static CExpression *PexprDisjunction(IMemoryPool *mp, CExpressionArray *Pdrgpexpr);

		// find a predicate that can be used for partition pruning with the given part key
		static CExpression *PexprPartPruningPredicate(IMemoryPool *mp,
		...
		static CExpressionArray *PdrgpexprAppendConjunctsDedup(IMemoryPool *mp,
															   CExpressionArray *pdrgpexpr,
															   CExpression *pexpr);

		// extract interesting conditions involving the partitioning keys
		static CExpression *PexprExtractPredicatesOnPartKeys(IMemoryPool *mp,
															 CExpression *pexprScalar,
		...
		// scalar expression
		static CExpression *PexprPredicateCol(IMemoryPool *mp,
											  CConstraint *pcnstr,
											  CColRef *colref,
											  BOOL fUseConstraints);

		// extract components of a comparison expression on the given key
		static void ExtractComponents(CExpression *pexprScCmp,
									  CColRef *pcrKey,
		...
		static CExpression *PexprInverseComparison(IMemoryPool *mp, CExpression *pexprCmp);

		// prune unnecessary equality operations
		static CExpression *PexprPruneSuperfluosEquality(IMemoryPool *mp, CExpression *pexpr);

		// remove conjuncts that are implied based on child columns equivalence classes
		static CExpression *PexprRemoveImpliedConjuncts(IMemoryPool *mp,
														CExpression *pexprScalar,
														CExpressionHandle &exprhdl);

		//	check if given correlations are valid for semi join operator;
		static BOOL FValidSemiJoinCorrelations(IMemoryPool *mp,
		...
		static void SeparateOuterRefs(IMemoryPool *mp,
									  CExpression *pexprScalar,
									  CColRefSet *outer_refs,
		...
		static BOOL FNullRejecting(IMemoryPool *mp, CExpression *pexprScalar, CColRefSet *pcrs);
		...
		static BOOL FConvertToCNF(CExpression *pexpr,
								  CExpression *pexprOuter,
								  CExpression *pexprInner);
		...
		static BOOL FBitmapLookupSupportedPredicateOrConjunct(CExpression *pexpr);
