...
namespace gpopt
{
	...
	private:
		...
		typedef CHashMap<CReqdColsRequest,
						 CColRefSet,
						 CReqdColsRequest::HashValue,
						 CReqdColsRequest::Equals,
						 CleanupRelease<CReqdColsRequest>,
	...
	protected:
		...
		CCTEReq *PcterNAry(IMemoryPool *mp,
		...
		CColRefSet *PcrsChildReqd(IMemoryPool *mp,
								  CExpressionHandle &exprhdl,
		...
		static CDistributionSpec *PdsCompute(IMemoryPool *mp,
											 const CTableDescriptor *ptabdesc,
											 CColRefArray *pdrgpcrOutput);

		// helper for a simple case of computing child's required sort order
		static COrderSpec *PosPassThru(IMemoryPool *mp,
									   CExpressionHandle &exprhdl,
									   COrderSpec *posInput,
									   ULONG child_index);

		// helper for a simple case of computing child's required distribution
		static CDistributionSpec *PdsPassThru(IMemoryPool *mp,
											  CExpressionHandle &exprhdl,
		...
		static CDistributionSpec *PdsMasterOnlyOrReplicated(IMemoryPool *mp,
															CExpressionHandle &exprhdl,
															CDistributionSpec *pdsInput,
															ULONG child_index,
															ULONG ulOptReq);
		...
		static CDistributionSpec *PdsUnary(IMemoryPool *mp,
										   CExpressionHandle &exprhdl,
										   CDistributionSpec *pdsInput,
										   ULONG child_index,
										   ULONG ulOptReq);
		...
		static CRewindabilitySpec *PrsPassThru(IMemoryPool *mp,
											   CExpressionHandle &exprhdl,
		...
		static CCTEMap *PcmCombine(IMemoryPool *mp, CDrvdPropArrays *pdrgpdpCtxt);
		...
		static CRewindabilitySpec *PrsDerivePassThruOuter(CExpressionHandle &exprhdl);
		...
		static CPartFilterMap *PpfmDeriveCombineRelational(IMemoryPool *mp,
														   CExpressionHandle &exprhdl);

		// helper for common case of combining partition index maps of all relational children
		static CPartIndexMap *PpimDeriveCombineRelational(IMemoryPool *mp,
														  CExpressionHandle &exprhdl);

		// enforce an operator to be executed on the master
		static CDistributionSpec *PdsEnforceMaster(IMemoryPool *mp,
												   CExpressionHandle &exprhdl,
	...
	public:
		// ctor
		explicit CPhysical(IMemoryPool *mp);
		...
		// derive sort order
		virtual COrderSpec *PosDerive(IMemoryPool *mp, CExpressionHandle &exprhdl) const = 0;

		// dderive distribution
		virtual CDistributionSpec *PdsDerive(IMemoryPool *mp, CExpressionHandle &exprhdl) const = 0;

		// derived properties: derive rewindability
		virtual CRewindabilitySpec *PrsDerive(IMemoryPool *mp,
		...
		virtual CPartFilterMap *PpfmDerive(IMemoryPool *mp, CExpressionHandle &exprhdl) const = 0;

		// derive cte map
		virtual CCTEMap *PcmDerive(IMemoryPool *mp, CExpressionHandle &exprhdl) const;
		...
		virtual CEnfdRewindability::ERewindabilityMatching Erm(CReqdPropPlan *prppInput,
															   ULONG child_index,
															   CDrvdPropArrays *pdrgpdpCtxt,
															   ULONG ulOptReq);
		...
		static CDistributionSpecSingleton *PdssMatching(IMemoryPool *mp,
