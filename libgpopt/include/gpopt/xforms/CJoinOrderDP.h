...
namespace gpopt
{
	...
	private:
		...
		typedef CHashMap<SComponentPair,
						 CExpression,
						 SComponentPair::HashValue,
						 SComponentPair::Equals,
						 CleanupRelease<SComponentPair>,
						 CleanupRelease<CExpression> >
		...
		CExpression *PexprJoinCoveredSubsetWithUncoveredSubset(CBitSet *pbs,
															   CBitSet *pbsCovered,
		...
		static void GenerateSubsets(IMemoryPool *mp,
									CBitSet *pbsCurrent,
									ULONG *pulElems,
									ULONG size,
									ULONG ulIndex,
		...
		static CBitSetArray *PdrgpbsSubsets(IMemoryPool *mp, CBitSet *pbs);

	public:
		// ctor
		CJoinOrderDP(IMemoryPool *mp,
