...
namespace gpopt
{
	...
	template <class TJoin,
			  ...
			  BOOL fWithSelect,
			  BOOL is_partial,
			  IMDIndex::EmdindexType eidxtype>
	...
	private:
		...
		BOOL
		FCanLeftOuterIndexApply(IMemoryPool *mp,
	...
	protected:
		...
		PopLogicalJoin(IMemoryPool *mp) const
		{
		...
		PopLogicalApply(IMemoryPool *mp, CColRefArray *colref_array) const
		{
	...
	public:
		// ctor
		explicit CXformJoin2IndexApplyBase<TJoin, TApply, TGet, fWithSelect, is_partial, eidxtype>(
			IMemoryPool *mp)
			:  // pattern
			  CXformJoin2IndexApply(GPOS_NEW(mp) CExpression(
				  ...
				  fWithSelect ? GPOS_NEW(mp) CExpression  // inner child with Select operator
					  (mp,
					   ...
					   GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPatternTree(mp))  // predicate
							  ...
							  : GPOS_NEW(mp) CExpression(
									mp, GPOS_NEW(mp) TGet(mp)),  // inner child with Get operator,
		...
		// dtor
		virtual ~CXformJoin2IndexApplyBase<TJoin, TApply, TGet, fWithSelect, is_partial, eidxtype>()
		{
		}

		// actual transform
		virtual void
		Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
		{
			...
			if (fWithSelect)
			{
				...
				pexprAllPredicates =
					CPredicateUtils::PexprConjunction(mp, pexprScalar, (*pexprInner)[1]);
			}
			else
			{
				pexprScalar->AddRef();
			...
			if (is_partial)
			{
				CreatePartialIndexApplyAlternatives(mp,
													pexpr->Pop()->UlOpId(),
													pexprOuter,
													pexprInner,
													pexprAllPredicates,
			...
			{
				CreateHomogeneousIndexApplyAlternatives(mp,
														pexpr->Pop()->UlOpId(),
														pexprOuter,
														pexprGet,
														pexprAllPredicates,
