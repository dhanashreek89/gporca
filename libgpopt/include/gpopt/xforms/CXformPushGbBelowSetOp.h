...
namespace gpopt
{
	...
	public:
		// ctor
		explicit CXformPushGbBelowSetOp(IMemoryPool *mp)
			: CXformExploration(
				  // pattern
				  GPOS_NEW(mp) CExpression(
					  ...
					  GPOS_NEW(mp) CExpression(
						  mp, GPOS_NEW(mp) CPatternTree(mp))  // project list of group-by
					  ))
		{
		}
		...
		Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
		{
			...
			for (ULONG ulChild = 0; ulChild < arity; ulChild++)
			{
				...
				if (!pcrsChild->Equals(pcrsOutput))
				{
					...
					pdrgpcrChildGb =
						CUtils::PdrgpcrRemap(mp, pdrgpcrGb, colref_mapping, true /*must_exist*/);
				...
				fNewChild = true;
				pexprPrjList->AddRef();
				CExpression *pexprChildGb =
					CUtils::PexprLogicalGbAggGlobal(mp, pdrgpcrChildGb, pexprChild, pexprPrjList);
