...
namespace gpopt
{
	...
	private:
		...
		void CreateHomogeneousBtreeIndexApplyAlternatives(IMemoryPool *mp,
														  ULONG ulOriginOpId,
		...
		// helper to add IndexApply expression to given xform results container
		// for homogeneous b-tree indexes
		void CreateAlternativesForBtreeIndex(IMemoryPool *mp,
											 ULONG ulOriginOpId,
											 CExpression *pexprOuter,
											 CExpression *pexprInner,
		...
		void CreateHomogeneousBitmapIndexApplyAlternatives(IMemoryPool *mp,
														   ULONG ulOriginOpId,
														   CExpression *pexprOuter,
														   CExpression *pexprInner,
														   CExpression *pexprScalar,
		...
		void ComputeColumnSets(IMemoryPool *mp,
							   CExpression *pexprInner,
							   CExpression *pexprScalar,
		...
		void CreatePartialIndexApplyPlan(IMemoryPool *mp,
										 ULONG ulOriginOpId,
										 CExpression *pexprOuter,
										 CExpression *pexprScalar,
										 CColRefSet *outer_refs,
		...
		CExpression *PexprJoinOverCTEConsumer(IMemoryPool *mp,
											  ULONG ulOriginOpId,
											  CLogicalDynamicGet *popDynamicGet,
											  ULONG ulCTEId,
											  CExpression *pexprScalar,
		...
		CExpression *PexprConstructUnionAll(IMemoryPool *mp,
		...
		//	Xform result
		void AddUnionPlanForPartialIndexes(IMemoryPool *mp,
										   CLogicalDynamicGet *popDynamicGet,
										   ULONG ulCTEId,
		...
		BOOL FMatchDistKeyAndIndexKey(const IMDRelation *pmdrel, const IMDIndex *pmdindex) const;
	...
	protected:
		...
		// helper to add IndexApply expression to given xform results container
		// for partial indexes
		virtual void CreatePartialIndexApplyAlternatives(IMemoryPool *mp,
														 ULONG ulOriginOpId,
														 CExpression *pexprOuter,
														 CExpression *pexprInner,
														 CExpression *pexprScalar,
		...
		// the instance.
		virtual CLogicalJoin *PopLogicalJoin(IMemoryPool *mp) const = 0;
		...
		virtual CLogicalApply *PopLogicalApply(IMemoryPool *mp,
											   CColRefArray *pdrgpcrOuterRefs) const = 0;
