...
namespace gpopt
{
	...
	private:
		...
		static void
		CreateCorrelatedApply(IMemoryPool *mp, CExpression *pexprApply, CXformResult *pxfres)
		{
			...
			switch (op_id)
			{
				case COperator::EopLogicalInnerApply:
					pexprResult = CUtils::PexprLogicalApply<CLogicalInnerCorrelatedApply>(
						mp, pexprOuter, pexprInner, colref_array, eopidSubq, pexprScalar);
					break;

				case COperator::EopLogicalLeftOuterApply:
					pexprResult = CUtils::PexprLogicalApply<CLogicalLeftOuterCorrelatedApply>(
						mp, pexprOuter, pexprInner, colref_array, eopidSubq, pexprScalar);
					break;

				case COperator::EopLogicalLeftSemiApply:
					pexprResult = CUtils::PexprLogicalApply<CLogicalLeftSemiCorrelatedApply>(
						mp, pexprOuter, pexprInner, colref_array, eopidSubq, pexprScalar);
					break;
	...
	protected:
					 ...
					 CExpressionArray **ppdrgpexpr)
		{
			...
			*ppdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
			...
			if (!CDecorrelator::FProcess(
					mp, (*pexprApply)[1], false /*fEqualityOnly*/, ppexprInner, *ppdrgpexpr))
			{
				// decorrelation filed
				(*ppdrgpexpr)->Release();
				return false;
		...
		static void
		Decorrelate(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexprApply)
		{
		...
		static void
		CreateJoinAlternative(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexprApply)
		{
#ifdef GPOS_DEBUG
			CExpressionHandle exprhdl(pxfctxt->Pmp());
			exprhdl.Attach(pexprApply);
			GPOS_ASSERT_IMP(CUtils::HasOuterRefs((*pexprApply)[1]),
							!exprhdl.GetRelationalProperties(1)->PcrsOuter()->ContainsAll(
	...
	public:
		...
		explicit CXformApply2Join<TApply, TJoin>(IMemoryPool *mp, BOOL)
			:  // pattern
		...
		explicit CXformApply2Join<TApply, TJoin>(IMemoryPool *mp)
			:  // pattern
			  CXformExploration(GPOS_NEW(mp) CExpression(
				  mp,
				  GPOS_NEW(mp) TApply(mp),
