...
//---------------------------------------------------------------------------
CDXLMinidump *
CMinidumperUtils::PdxlmdLoad(IMemoryPool *mp, const CHAR *file_name)
{
	...
	CParseHandlerDXL *parse_handler_dxl =
		CDXLUtils::GetParseHandlerForDXLFile(mp, file_name, NULL /*xsd_file_path*/);
	...
	return GPOS_NEW(mp) CDXLMinidump(pbs,
									 optimizer_config,
									 query,
...
//---------------------------------------------------------------------------
CDXLNode *
CMinidumperUtils::PdxlnExecuteMinidump(IMemoryPool *mp,
									   ...
									   IConstExprEvaluator *pceeval)
{
	...
	CDXLNode *pdxlnPlan = PdxlnExecuteMinidump(
		mp, pdxlmd, file_name, ulSegments, ulSessionId, ulCmdId, optimizer_config, pceeval);
									   ...
									   IConstExprEvaluator *pceeval)
{
	...
	CDXLNode *result = CMinidumperUtils::PdxlnExecuteMinidump(mp,
															  factory.Pmda(),
															  pdxlmd,
															  file_name,
															  ulSegments,
									   ...
									   IConstExprEvaluator *pceeval)
{
	...
	GPOS_ASSERT(NULL != pdxlmd->GetQueryDXLRoot() && NULL != pdxlmd->PdrgpdxlnQueryOutput() &&
				NULL != pdxlmd->GetCTEProducerDXLArray() && "No query found in Minidump");
	GPOS_ASSERT(NULL != pdxlmd->GetMdIdCachedObjArray() && NULL != pdxlmd->GetSysidPtrArray() &&
	...
	GPOS_TRY_HDL(&errhdl)
	{
		pdxlnPlan = COptimizer::PdxlnOptimize(mp,
											  md_accessor,
