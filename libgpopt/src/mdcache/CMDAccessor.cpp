...
const CMDAccessor::SMDProviderElem CMDAccessor::SMDProviderElem::m_mdpelemInvalid(
...
//---------------------------------------------------------------------------
BOOL
CMDAccessor::SMDAccessorElem::Equals(const MdidPtr &left_mdid, const MdidPtr &right_mdid)
{
...
//
//---------------------------------------------------------------------------
CMDAccessor::SMDProviderElem::SMDProviderElem(CSystemId sysid, IMDProvider *pmdp)
...
//
//---------------------------------------------------------------------------
CMDAccessor::CMDAccessor(IMemoryPool *mp, MDCache *pcache)
...
//
//---------------------------------------------------------------------------
CMDAccessor::CMDAccessor(IMemoryPool *mp, MDCache *pcache, CSystemId sysid, IMDProvider *pmdp)
	: m_mp(mp), m_pcache(pcache), m_dLookupTime(0.0), m_dFetchTime(0.0)
{
	GPOS_ASSERT(NULL != m_mp);
	GPOS_ASSERT(NULL != m_pcache);
...
//
//---------------------------------------------------------------------------
CMDAccessor::CMDAccessor(IMemoryPool *mp,
						 MDCache *pcache,
...
//---------------------------------------------------------------------------
void
CMDAccessor::InitHashtables(IMemoryPool *mp)
{
	// initialize Cache accessors hash table
	m_shtCacheAccessors.Init(mp,
...
CMDAccessor::~CMDAccessor()
{
	...
	if (GPOS_FTRACE(EopttracePrintOptimizationStatistics))
	{
		...
		at.Os() << "[OPT]: Total metadata lookup time (including fetch time): " << m_dLookupTime
				<< "ms" << std::endl;
	}
}
...
//---------------------------------------------------------------------------
void
CMDAccessor::RegisterProviders(const CSystemIdArray *pdrgpsysid, const CMDProviderArray *pdrgpmdp)
{
...
CMDAccessor::GetImdObj(IMDId *mdid)
{
	...
	if (NULL == pimdobj)
	{
		...
		if (NULL == pmdobjNew)
		{
			...
			pmdobjNew =
				gpdxl::CDXLUtils::ParseDXLToIMDIdCacheObj(mp, a_pstr.Value(), NULL /* XSD path */);
...
CMDAccessor::Pmdcast(IMDId *mdid_src, IMDId *mdid_dest)
{
	...
	a_pmdidCast =
		GPOS_NEW(m_mp) CMDIdCast(CMDIdGPDB::CastMdid(mdid_src), CMDIdGPDB::CastMdid(mdid_dest));
...
//---------------------------------------------------------------------------
const IMDScCmp *
CMDAccessor::Pmdsccmp(IMDId *left_mdid, IMDId *right_mdid, IMDType::ECmpType cmp_type)
{
	...
	a_pmdidScCmp = GPOS_NEW(m_mp)
		CMDIdScCmp(CMDIdGPDB::CastMdid(left_mdid), CMDIdGPDB::CastMdid(right_mdid), cmp_type);
...
//---------------------------------------------------------------------------
void
CMDAccessor::RecordColumnStats(IMemoryPool *mp,
							   IMDId *rel_mdid,
							   ULONG colid,
							   ULONG ulPos,
							   BOOL fSystemCol,
							   ...
							   CStatisticsConfig *stats_config)
{
	...
	BOOL fRecordMissingStats = !fEmptyTable && fGuc && !fSystemCol && (NULL != stats_config) &&
...
const IMDColStats *
CMDAccessor::Pmdcolstats(IMemoryPool *mp, IMDId *rel_mdid, ULONG ulPos)
{
	rel_mdid->AddRef();
...
//---------------------------------------------------------------------------
IStatistics *
CMDAccessor::Pstats(IMemoryPool *mp,
					IMDId *rel_mdid,
					CColRefSet *pcrsHist,
					...
					CStatisticsConfig *stats_config)
{
	...
	return GPOS_NEW(mp)
		CStatistics(mp, col_histogram_mapping, colid_width_mapping, rows, fEmptyTable);
}
...
//---------------------------------------------------------------------------
CHistogram *
CMDAccessor::GetHistogram(IMemoryPool *mp, IMDId *mdid_type, const IMDColStats *pmdcolstats)
{
	...
	CHistogram *histogram = GPOS_NEW(mp) CHistogram(buckets,
...
//---------------------------------------------------------------------------
CBucket *
CMDAccessor::Pbucket(IMemoryPool *mp, IMDId *mdid_type, const CDXLBucket *dxl_bucket)
{
...
//---------------------------------------------------------------------------
IDatum *
CMDAccessor::GetDatum(IMemoryPool *mp, IMDId *mdid_type, const CDXLDatum *dxl_datum)
{
