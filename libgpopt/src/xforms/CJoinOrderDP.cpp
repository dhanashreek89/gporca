...
//
//---------------------------------------------------------------------------
CJoinOrderDP::CJoinOrderDP(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
void
CJoinOrderDP::InsertExpressionCost(CExpression *pexpr,
								   CDouble dCost,
...
//---------------------------------------------------------------------------
void
CJoinOrderDP::GenerateSubsets(IMemoryPool *mp,
							  CBitSet *pbsCurrent,
							  ULONG *pulElems,
							  ULONG size,
							  ULONG ulIndex,
...
//---------------------------------------------------------------------------
CBitSetArray *
CJoinOrderDP::PdrgpbsSubsets(IMemoryPool *mp, CBitSet *pbs)
{
														...
														CBitSet *pbsUncovered)
{
	...
	CExpression *pexprResult = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
		m_mp, pexprJoin, pexprCross, CPredicateUtils::PexprConjunction(m_mp, NULL));
	pbs->AddRef();
#ifdef GPOS_DEBUG
	BOOL fInserted =
#endif  // GPOS_DEBUG
...
CJoinOrderDP::PexprBuildPred(CBitSet *pbsFst, CBitSet *pbsSnd)
{
	...
	{
		SEdge *pedge = m_rgpedge[ul];
		if (pbs->ContainsAll(pedge->m_pbs) && !pbsFst->IsDisjoint(pedge->m_pbs) &&
