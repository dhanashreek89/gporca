...
CXformLeftOuter2InnerUnionAllLeftAntiSemiJoin::Exfp(CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
void
CXformLeftOuter2InnerUnionAllLeftAntiSemiJoin::Transform(CXformContext *pxfctxt,
														 CXformResult *pxfres,
														 CExpression *pexpr) const
{
	...
	CExpression *pexprProjectAppendNulls = PexprProjectOverLeftAntiSemiJoin(mp,
																			pdrgpcrOuter,
																			pcrsScalar,
																			pcrsInner,
																			pdrgpcrJoinOutput,
	...
	CExpression *pexprJoinAnchor = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalCTEAnchor(mp, ulCTEJoinId), pexprUnionAll);
	CExpression *pexprOuterAnchor = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalCTEAnchor(mp, ulCTEOuterId), pexprJoinAnchor);
...
//---------------------------------------------------------------------------
BOOL
CXformLeftOuter2InnerUnionAllLeftAntiSemiJoin::FValidInnerExpr(CExpression *pexprInner)
{
	...
	ULONG ulCTEOuterId)
{
	...
	CExpression *pexprGby = GPOS_NEW(mp)
		CExpression(mp,
	...
	CColRefArray **ppdrgpcrProjectOutput)
{
	...
	CExpression *pexprLeftAntiSemi = PexprLeftAntiSemiJoinWithInnerGroupBy(mp,
																		   pdrgpcrOuter,
																		   pdrgpcrOuterCopy,
																		   pcrsScalar,
																		   pcrsInner,
