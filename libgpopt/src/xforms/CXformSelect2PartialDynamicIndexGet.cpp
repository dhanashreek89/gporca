...
//
//---------------------------------------------------------------------------
CXformSelect2PartialDynamicIndexGet::CXformSelect2PartialDynamicIndexGet(IMemoryPool *mp)
	:  // pattern
	...
	CXformResult *pxfres) const
{
	...
	for (ULONG ul = 0; ul < ulPartialIndexes; ul++)
	{
		...
		{
			pexprDynamicScan =
				CXformUtils::PexprPartialDynamicIndexGet(mp,
														 popGet,
														 pexpr->Pop()->UlOpId(),
														 pdrgpexprIndex,
														 pdrgpexprResidual,
		...
		{
			pexprDynamicScan =
				PexprSelectOverDynamicGet(mp, popGet, pexprScalar, pdrgpcrNew, ppartcnstr);
		}
	...
	if (0 < ulInput)
	{
		...
		if (NULL != pexprPredOnPartKey)
		{
			pexprResult = GPOS_NEW(mp)
				CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexprResult, pexprPredOnPartKey);
		}
...
//---------------------------------------------------------------------------
CExpression *
CXformSelect2PartialDynamicIndexGet::PexprSelectOverDynamicGet(IMemoryPool *mp,
