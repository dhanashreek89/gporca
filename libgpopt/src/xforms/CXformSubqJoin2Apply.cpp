...
//
//---------------------------------------------------------------------------
CXformSubqJoin2Apply::CXformSubqJoin2Apply(IMemoryPool *mp)
	:  // pattern
...
)
{
	...
	if (CUtils::FSubquery(pop))
	{
		...
		CColRefSet *outer_refs = GPOS_NEW(mp) CColRefSet(
			mp,
			*CDrvdPropRelational::GetRelationalProperties((*pexpr)[0]->PdpDerive())->PcrsOuter());
...
//---------------------------------------------------------------------------
CExpression *
CXformSubqJoin2Apply::PexprReplaceSubqueries(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CExpression *
CXformSubqJoin2Apply::PexprSubqueryPushDown(IMemoryPool *mp,
											CExpression *pexpr,
	...
	for (ULONG ulChild = 0; ulChild < arity - 1; ulChild++)
	{
		...
		if (0 < ulSubqs)
		{
			...
			for (ULONG ulSubq = 0; ulSubq < ulSubqs; ulSubq++)
			{
				...
				pexprSubq->AddRef();
				CColRef *colref =
					CScalarProjectElement::PopConvert((*pexprPrjList)[ulSubq]->Pop())->Pcr();
#ifdef GPOS_DEBUG
				BOOL fInserted =
#endif  // GPOS_DEBUG
...
//---------------------------------------------------------------------------
void
CXformSubqJoin2Apply::Transform(CXformContext *pxfctxt,
								CXformResult *pxfres,
								CExpression *pexpr,
								BOOL fEnforceCorrelatedApply) const
{
	...
	CColRefSet *pcrsUsed =
		CDrvdPropScalar::GetDrvdScalarProps(pexprJoinCondition->PdpDerive())->PcrsUsed();
	CColRefSet *pcrsJoinOutput =
		CDrvdPropRelational::GetRelationalProperties(pexprJoin->PdpDerive())->PcrsOutput();
	...
	BOOL fHasSubquery = CDrvdPropScalar::GetDrvdScalarProps((*pexprSubqsPushedDown)[1]->PdpDerive())
							->FHasSubquery();
	if (fHasSubquery)
	{
