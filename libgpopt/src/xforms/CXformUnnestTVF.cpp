...
//
//---------------------------------------------------------------------------
CXformUnnestTVF::CXformUnnestTVF(IMemoryPool *mp)
	: CXformExploration(
		  // pattern
...
//---------------------------------------------------------------------------
CColRefArray *
CXformUnnestTVF::PdrgpcrSubqueries(IMemoryPool *mp,
								   ...
								   CExpression *pexprCTECons)
{
	...
	CColRefArray *pdrgpcrConsOutput =
		CDrvdPropRelational::GetRelationalProperties(pexprCTECons->PdpDerive())
			->PcrsOutput()
			->Pdrgpcr(mp);
	...
	for (ULONG ulOuter = 0; ulOuter < ulPrjElems; ulOuter++)
	{
		...
		if (CDrvdPropScalar::GetDrvdScalarProps((*pexprPrjElem)[0]->PdpDerive())->FHasSubquery())
		{
			...
			CColRef *pcrConsumer =
				CUtils::PcrMap(pcrProducer, pdrgpcrProdOutput, pdrgpcrConsOutput);
...
//---------------------------------------------------------------------------
CExpression *
CXformUnnestTVF::PexprProjectSubqueries(IMemoryPool *mp, CExpression *pexprTVF)
{
...
//---------------------------------------------------------------------------
void
CXformUnnestTVF::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
{
	...
	CExpression *pexprCTEProd =
		CXformUtils::PexprAddCTEProducer(mp, ulCTEId, pdrgpcrOutput, pexprProject);
	...
	CExpression *pexprCorrApply = CUtils::PexprLogicalApply<CLogicalLeftOuterCorrelatedApply>(
		...
		CPredicateUtils::PexprConjunction(mp,
										  NULL /*pdrgpexpr*/)  // scalar expression is const True
	);

	CExpression *pexprAlt =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalCTEAnchor(mp, ulCTEId), pexprCorrApply);
