...
//
//---------------------------------------------------------------------------
CXformDifference2LeftAntiSemiJoin::CXformDifference2LeftAntiSemiJoin(IMemoryPool *mp)
	:  // pattern
											 ...
											 CExpression *pexpr) const
{
	...
	CExpression *pexprProjList = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), GPOS_NEW(mp) CExpressionArray(mp));

	CExpression *pexprAgg =
		CUtils::PexprLogicalGbAggGlobal(mp, pdrgpcrOutput, pexprLASJ, pexprProjList);
