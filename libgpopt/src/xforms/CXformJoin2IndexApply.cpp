...
CXformJoin2IndexApply::Exfp(CExpressionHandle &exprhdl) const
{
	...
	if (0 == pdpscalar->PcrsUsed()->Size() || pdpscalar->FHasSubquery() || exprhdl.HasOuterRefs())
	{
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::ComputeColumnSets(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::CreateHomogeneousIndexApplyAlternatives(IMemoryPool *mp,
															   ULONG ulOriginOpId,
															   ...
															   IMDIndex::EmdindexType emdtype) const
{
	...
	{
		CreateHomogeneousBtreeIndexApplyAlternatives(mp,
													 ulOriginOpId,
													 pexprOuter,
													 pexprInner,
													 pexprScalar,
	...
	{
		CreateHomogeneousBitmapIndexApplyAlternatives(mp,
													  ulOriginOpId,
													  pexprOuter,
													  pexprInner,
													  pexprScalar,
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::CreateAlternativesForBtreeIndex(IMemoryPool *mp,
													   ULONG ulOriginOpId,
													   CExpression *pexprOuter,
													   CExpression *pexprInner,
	...
	CXformResult *pxfres) const
{
	...
	CExpression *pexprLogicalIndexGet = CXformUtils::PexprBitmapTableGet(
		mp, popGet, ulOriginOpId, ptabdescInner, pexprScalar, outer_refs, pcrsReqd);
	if (NULL != pexprLogicalIndexGet)
	{
		// second child has residual predicates, create an apply of outer and inner
		// and add it to xform results
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::CreatePartialIndexApplyAlternatives(IMemoryPool *mp,
														   ULONG ulOriginOpId,
														   CExpression *pexprOuter,
														   CExpression *pexprInner,
														   CExpression *pexprScalar,
														   ...
														   CXformResult *pxfres) const
{
	...
	SPartDynamicIndexGetInfoArrays *pdrgpdrgppartdig =
		CXformUtils::PdrgpdrgppartdigCandidates(mp,
												COptCtxt::PoctxtFromTLS()->Pmda(),
												pdrgpexpr,
												popDynamicGet->PdrgpdrgpcrPart(),
												pmdrel,
	...
	for (ULONG ul = 0; ul < ulCandidates; ul++)
	{
		...
		{
			CreatePartialIndexApplyPlan(mp,
										ulOriginOpId,
										pexprOuter,
										pexprScalar,
										outer_refs,
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::CreatePartialIndexApplyPlan(IMemoryPool *mp,
												   ULONG ulOriginOpId,
												   CExpression *pexprOuter,
												   CExpression *pexprScalar,
												   CColRefSet *outer_refs,
												   ...
												   CXformResult *pxfres) const
{
	...
	CColRefArray *pdrgpcrOuter =
		CDrvdPropRelational::GetRelationalProperties(pexprOuter->PdpDerive())
			->PcrsOutput()
			->Pdrgpcr(mp);
	...
	for (ULONG ul = 0; ul < ulPartialIndexes; ul++)
	{
		...
		else
		{
			...
			pdrgpcrOuterNew =
				CUtils::PdrgpcrCopy(mp, pdrgpcrOuter, false /*fAllComputed*/, colref_mapping);
			pdrgpcrIndexGet =
				CUtils::PdrgpcrCopy(mp, pdrgpcrGet, false /*fAllComputed*/, colref_mapping);
		}

		CExpression *pexprUnionAllChild = NULL;
		if (NULL != pmdindex)
		{
			pexprUnionAllChild = PexprIndexApplyOverCTEConsumer(mp,
																ulOriginOpId,
																popDynamicGet,
																pdrgpexprIndex,
																pdrgpexprResidual,
		...
		{
			pexprUnionAllChild = PexprJoinOverCTEConsumer(mp,
														  ulOriginOpId,
														  popDynamicGet,
														  ulCTEId,
														  pexprScalar,
		...
		{
			pdrgpcrNew =
				CDrvdPropRelational::GetRelationalProperties(pexprUnionAllChild->PdpDerive())
					->PcrsOutput()
					->Pdrgpcr(mp);
		}
		else
		{
			...
			pdrgpcrNew =
				CUtils::PdrgpcrRemap(mp, pdrgpcrOutput, colref_mapping, true /*must_exist*/);
		}
	...
	if (2 <= pdrgpexprInput->Size())
	{
		...
		pexprResult = GPOS_NEW(mp) CExpression(
			mp,
			GPOS_NEW(mp)
				CLogicalUnionAll(mp, pdrgpcrOutput, pdrgpdrgpcrInput, popDynamicGet->ScanId()),
			pdrgpexprInput);
	}
	else
	{
...
//---------------------------------------------------------------------------
CExpression *
CXformJoin2IndexApply::PexprJoinOverCTEConsumer(IMemoryPool *mp,
												ULONG,  //  ulOriginOpId
												CLogicalDynamicGet *popDynamicGet,
												ULONG ulCTEId,
												CExpression *pexprScalar,
...
{
	CExpression *pexprDynamicScan = CXformUtils::PexprPartialDynamicIndexGet(mp,
																			 popDynamicGet,
																			 ulOriginOpId,
																			 pdrgpexprIndex,
																			 pdrgpexprResidual,
...
//---------------------------------------------------------------------------
CExpression *
CXformJoin2IndexApply::PexprConstructUnionAll(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::AddUnionPlanForPartialIndexes(IMemoryPool *mp,
													 CLogicalDynamicGet *popDynamicGet,
													 ULONG ulCTEId,
													 ...
													 CXformResult *pxfres) const
{
	...
	CExpression *pexprPredOnPartKey =
		CPredicateUtils::PexprExtractPredicatesOnPartKeys(mp,
														  pexprScalar,
														  pdrgpdrgpcrPartKeys,
	...
	{
		pexprUnion = GPOS_NEW(mp)
			CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexprUnion, pexprPredOnPartKey);
	}

	CExpression *pexprAnchor =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalCTEAnchor(mp, ulCTEId), pexprUnion);
