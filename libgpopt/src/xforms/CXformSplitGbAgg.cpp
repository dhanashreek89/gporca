...
//
//---------------------------------------------------------------------------
CXformSplitGbAgg::CXformSplitGbAgg(IMemoryPool *mp)
	: CXformExploration(
		  // pattern
...
//---------------------------------------------------------------------------
void
CXformSplitGbAgg::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
{
	...
	CExpression *local_expr = GPOS_NEW(mp)
		CExpression(mp,
					GPOS_NEW(mp) CLogicalGbAgg(
						mp, pdrgpcrLocal, pdrgpcrMinimal, COperator::EgbaggtypeLocal /*egbaggtype*/
						),
					pexprRelational,
	...
	CExpression *pexprGlobal = GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp) CLogicalGbAgg(
			mp, pdrgpcrGlobal, pdrgpcrMinimal, COperator::EgbaggtypeGlobal /*egbaggtype*/
			),
		local_expr,
...
//---------------------------------------------------------------------------
void
CXformSplitGbAgg::PopulateLocalGlobalProjectList(IMemoryPool *mp,
												 ...
												 CExpression **ppexprProjListGlobal)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		CExpression *pexprProjElemGlobal =
			CUtils::PexprScalarProjectElement(mp, pcrGlobal, pexprAggFuncGlobal);
	...
	*ppexprProjListLocal =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprProjElemLocal);

	*ppexprProjListGlobal =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprProjElemGlobal);
}
