	...
	CColRefSet **ppcrsInnerUsed)
{
	...
	CColRefSet *pcrsUsed =
		GPOS_NEW(mp) CColRefSet(mp);  // set of inner columns used in external correlations
	BOOL fSuccess = true;
	for (ULONG ul = 0; fSuccess && ul < ulCorrs; ul++)
	{
		CExpression *pexprCurrent = (*pdrgpexprAllCorr)[ul];
		CColRefSet *pcrsCurrent = GPOS_NEW(mp) CColRefSet(
			mp, *CDrvdPropScalar::GetDrvdScalarProps(pexprCurrent->PdpDerive())->PcrsUsed());
...
//---------------------------------------------------------------------------
BOOL
CXformLeftSemiApplyWithExternalCorrs2InnerJoin::FDecorrelate(IMemoryPool *mp,
															 CExpression *pexpr,
															 ...
															 CExpressionArray **ppdrgpexprCorr)
{
	...
	if (!CDecorrelator::FProcess(
			mp, pexprInner, true /* fEqualityOnly */, ppexprInnerNew, pdrgpexpr))
	{
...
//---------------------------------------------------------------------------
CExpression *
CXformLeftSemiApplyWithExternalCorrs2InnerJoin::PexprDecorrelate(IMemoryPool *mp,
																 CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
	GPOS_ASSERT(COperator::EopLogicalLeftSemiApply == pexpr->Pop()->Eopid() ||
	...
	if (NULL == exprhdl.GetRelationalProperties(0 /*child_index*/)->Pkc() ||
	...
	pdrgpcrUsed->Release();
	CExpression *pexprGb =
		CUtils::PexprLogicalGbAggGlobal(mp,
										pdrgpcrGrpCols,
										pexprInnerJoin,
										GPOS_NEW(mp)
...
CXformLeftSemiApplyWithExternalCorrs2InnerJoin::Exfp(CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
void
CXformLeftSemiApplyWithExternalCorrs2InnerJoin::Transform(CXformContext *pxfctxt,
														  CXformResult *pxfres,
