...
//
//---------------------------------------------------------------------------
CXformPushDownLeftOuterJoin::CXformPushDownLeftOuterJoin(IMemoryPool *mp)
	: CXformExploration(
		  // pattern
		  GPOS_NEW(mp) CExpression(
			  ...
			  (mp,
			   ...
			   GPOS_NEW(mp)
				   CExpression(mp, GPOS_NEW(mp) CPatternTree(mp))  // NAry-join predicate tree
			   ),
									   ...
									   CExpression *pexpr) const
{
	...
	CColRefSet *pcrsLOJUsed =
		CDrvdPropScalar::GetDrvdScalarProps(pexprLOJScalarChild->PdpDerive())->PcrsUsed();
	...
	if (1 < pdrgpexprLOJChildren->Size())
	{
		...
		pexprLOJOuterChild =
			GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalNAryJoin(mp), pdrgpexprLOJChildren);
	...
	// build new LOJ
	CExpression *pexprLOJNew =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), pdrgpexprLOJChildren);
