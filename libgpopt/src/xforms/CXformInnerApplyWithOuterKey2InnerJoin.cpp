...
//
//---------------------------------------------------------------------------
CXformInnerApplyWithOuterKey2InnerJoin::CXformInnerApplyWithOuterKey2InnerJoin(IMemoryPool *mp)
	: CXformExploration(
		  // pattern
		  GPOS_NEW(mp) CExpression(
			  ...
			  GPOS_NEW(mp) CExpression(
				  ...
				  GPOS_NEW(mp)
					  CExpression(mp, GPOS_NEW(mp) CPatternTree(mp)),  // relational child of Gb
				  GPOS_NEW(mp)
					  CExpression(mp, GPOS_NEW(mp) CPatternLeaf(mp))  // scalar project list of Gb
				  ),												  // right child
												  ...
												  CExpression *pexpr) const
{
	...
	if (!CDecorrelator::FProcess(
			mp, (*pexprGb)[0], false /*fEqualityOnly*/, &pexprInner, pdrgpexpr))
	{
		pdrgpexpr->Release();
		return;
	}
	...
	pexprOuter->AddRef();
	CExpression *pexprInnerJoin = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalInnerJoin(mp), pexprOuter, pexprInner, pexprPredicate);
