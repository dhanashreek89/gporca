...
//
//---------------------------------------------------------------------------
CXformSimplifyGbAgg::CXformSimplifyGbAgg(IMemoryPool *mp)
	: CXformExploration(
		  // pattern
...
//---------------------------------------------------------------------------
BOOL
CXformSimplifyGbAgg::FDropGbAgg(IMemoryPool *mp, CExpression *pexpr, CXformResult *pxfres)
{
	...
	CKeyCollection *pkc =
		CDrvdPropRelational::GetRelationalProperties(pexprRelational->PdpDerive())->Pkc();
	if (NULL == pkc)
	{
	...
	for (ULONG ul = 0; !fDrop && ul < ulKeys; ul++)
	{
		...
		if (fGrpColsHasKey)
		{
			...
			CExpression *pexprResult = CUtils::PexprLogicalSelect(
				mp, pexprRelational, CPredicateUtils::PexprConjunction(mp, NULL));
...
//---------------------------------------------------------------------------
void
CXformSimplifyGbAgg::Transform(CXformContext *pxfctxt,
							   CXformResult *pxfres,
							   CExpression *pexpr) const
{
	...
	CColRefSet *pcrsMinimal =
		GPOS_NEW(mp) CColRefSet(mp);  // a set of minimal grouping columns based on FD's
	CFunctionalDependencyArray *pdrgpfd =
		CDrvdPropRelational::GetRelationalProperties(pexpr->PdpDerive())->Pdrgpfd();
	...
	CLogicalGbAgg *popAggNew = GPOS_NEW(mp)
		CLogicalGbAgg(mp, colref_array, pcrsMinimal->Pdrgpcr(mp), popAgg->Egbaggtype());
	pcrsMinimal->Release();
	...
	CExpression *pexprResult =
		GPOS_NEW(mp) CExpression(mp, popAggNew, pexprRelational, pexprProjectList);
	pxfres->Add(pexprResult);
}
