									  ...
									  CReqdPropPlan *prppCTEProducer)
{
	...
	m_fOptimizeCTESequence = (COperator::EopPhysicalSequence == pgexpr->Pop()->Eopid() &&
...
//---------------------------------------------------------------------------
void
CJobGroupExpressionOptimization::InitChildGroupsOptimization(CSchedulerContext *psc)
{
	...
	m_pexprhdlPlan =
		GPOS_NEW(psc->GetGlobalMemoryPool()) CExpressionHandle(psc->GetGlobalMemoryPool());
	...
	CGroupExpression *pgexprForStats =
		m_pgexpr->Pgroup()->PgexprBestPromise(psc->GetGlobalMemoryPool(), m_pgexpr);
	...
	m_pdrgpstatCurrentCtxt =
		GPOS_NEW(psc->GetGlobalMemoryPool()) IStatisticsArray(psc->GetGlobalMemoryPool());
...
CJobGroupExpressionOptimization::EevtInitialize(CSchedulerContext *psc, CJob *pjOwner)
{
	...
	if (psc->Peng()->FSafeToPrune(pjgeo->m_pgexpr,
								  ...
								  &costLowerBound))
	{
		(void) pjgeo->m_pgexpr->PccComputeCost(psc->GetGlobalMemoryPool(),
											   pjgeo->m_poc,
											   pjgeo->m_ulOptReq,
											   NULL /*pdrgpoc*/,
											   true /*fPruned*/,
...
CJobGroupExpressionOptimization::DerivePrevChildProps(CSchedulerContext *psc)
{
	...
	COptimizationContext *pocChild =
		pgroupChild->PocLookupBest(psc->GetGlobalMemoryPool(),
	...
	if (psc->Peng()->FSafeToPrune(
			m_pgexpr, m_poc->Prpp(), pccChildBest, ulPrevChildIndex, &costLowerBound))
	{
		// failed to optimize child due to cost bounding
		(void) m_pgexpr->PccComputeCost(psc->GetGlobalMemoryPool(),
										m_poc,
										m_ulOptReq,
										NULL /*pdrgpoc*/,
										true /*fPruned*/,
...
//---------------------------------------------------------------------------
void
CJobGroupExpressionOptimization::ComputeCurrentChildRequirements(CSchedulerContext *psc)
{
...
CJobGroupExpressionOptimization::ScheduleChildGroupsJobs(CSchedulerContext *psc)
{
	...
	COptimizationContext *pocChild = GPOS_NEW(psc->GetGlobalMemoryPool())
		COptimizationContext(psc->GetGlobalMemoryPool(),
							 pgroupChild,
...
CJobGroupExpressionOptimization::EevtAddEnforcers(CSchedulerContext *psc, CJob *pjOwner)
{
	...
	pjgeo->m_pdrgpoc =
		psc->Peng()->PdrgpocChildren(psc->GetGlobalMemoryPool(), *pjgeo->m_pexprhdlPlan);

	// enforce physical properties
	BOOL fCheckEnfdProps = psc->Peng()->FCheckEnfdProps(psc->GetGlobalMemoryPool(),
														pjgeo->m_pgexpr,
														pjgeo->m_poc,
														pjgeo->m_ulOptReq,
														pjgeo->m_pdrgpoc);
...
CJobGroupExpressionOptimization::EevtOptimizeSelf(CSchedulerContext *psc, CJob *pjOwner)
{
	...
	CCostContext *pcc = pgexpr->PccComputeCost(
		psc->GetGlobalMemoryPool(), poc, ulOptReq, pdrgpoc, false /*fPruned*/, CCost(0.0));
...
//---------------------------------------------------------------------------
BOOL
CJobGroupExpressionOptimization::FScheduleCTEOptimization(CSchedulerContext *psc,
														  CGroupExpression *pgexpr,
														  COptimizationContext *poc,
														  ULONG ulOptReq,
														  CJob *pjParent)
