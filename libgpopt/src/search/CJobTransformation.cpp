...
// +-----------------+
//
const CJobTransformation::EEvent rgeev[CJobTransformation::estSentinel]
									  [CJobTransformation::estSentinel] = {
										  {// estInitialized
...
const WCHAR rgwszStates[CJobTransformation::estSentinel][GPOPT_FSM_NAME_LENGTH] = {
	GPOS_WSZ_LIT("initialized"), GPOS_WSZ_LIT("completed")};
...
CJobTransformation::EevtTransform(CSchedulerContext *psc, CJob *pjOwner)
{
	...
	psc->Peng()->InsertXformResult(
		pgexpr->Pgroup(), pxfres, pxform->Exfid(), pgexpr, ulElapsedTime);
	pxfres->Release();
