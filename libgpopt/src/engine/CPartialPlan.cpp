...
//---------------------------------------------------------------------------
void
CPartialPlan::ExtractChildrenCostingInfo(IMemoryPool *mp,
										 ICostModel *pcm,
...
CPartialPlan::RaiseExceptionIfStatsNull(IStatistics *stats)
{
	if (NULL == stats)
	{
		GPOS_RAISE(gpopt::ExmaGPOPT,
				   ...
				   GPOS_WSZ_LIT("Could not compute cost of partial plan since statistics for the "
								"group not derived"));
	}
}
...
//---------------------------------------------------------------------------
CCost
CPartialPlan::CostCompute(IMemoryPool *mp)
{
	...
	ICostModel::SCostingInfo ci(
		mp, exprhdl.UlNonScalarChildren(), GPOS_NEW(mp) ICostModel::CCostingStats(stats));
