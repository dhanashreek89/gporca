...
//
//---------------------------------------------------------------------------
CPhysicalPartitionSelector::CPhysicalPartitionSelector(IMemoryPool *mp,
													   ULONG scan_id,
													   IMDId *mdid,
...
//---------------------------------------------------------------------------
BOOL
CPhysicalPartitionSelector::FMatchPartCnstr(UlongToPartConstraintMap *ppartcnstrmap) const
{
...
//---------------------------------------------------------------------------
BOOL
CPhysicalPartitionSelector::FSubsetPartCnstr(UlongToPartConstraintMap *ppartcnstrmapFst,
...
CPhysicalPartitionSelector::Matches(COperator *pop) const
{
	...
	return fScanIdCmp && fMdidCmp && fPartCnstrMapCmp && fColRefCmp && fPartCnstrEquiv &&
		   fEqPredCmp && fResPredCmp && fPredCmp;
}
...
//---------------------------------------------------------------------------
CExpression *
CPhysicalPartitionSelector::PexprPartPred(IMemoryPool *mp, ULONG ulPartLevel) const
{
...
//---------------------------------------------------------------------------
CExpression *
CPhysicalPartitionSelector::PexprCombinedPartPred(IMemoryPool *mp) const
{
...
//---------------------------------------------------------------------------
CPartFilterMap *
CPhysicalPartitionSelector::PpfmDerive(IMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	if (!FHasFilter())
	{
...
//---------------------------------------------------------------------------
CColRefSet *
CPhysicalPartitionSelector::PcrsRequired(IMemoryPool *mp,
...
)
{
	...
	pcrs->Union(
		CDrvdPropScalar::GetDrvdScalarProps(m_pexprCombinedPredicate->PdpDerive())->PcrsUsed());
...
//---------------------------------------------------------------------------
COrderSpec *
CPhysicalPartitionSelector::PosRequired(IMemoryPool *mp,
										CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalPartitionSelector::PdsRequired(IMemoryPool *mp,
										CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CRewindabilitySpec *
CPhysicalPartitionSelector::PrsRequired(IMemoryPool *mp,
										CExpressionHandle &exprhdl,
...
CPhysicalPartitionSelector::PppsRequired(IMemoryPool *mp,
										 CExpressionHandle &exprhdl,
...
)
{
	...
	for (ULONG ul = 0; ul < ulScanIds; ul++)
	{
		...
		ppim->Insert(
			scan_id, ppartcnstrmap, epim, ulExpectedPropagators, mdid, pdrgppartkeys, ppartcnstr);
...
//---------------------------------------------------------------------------
CCTEReq *
CPhysicalPartitionSelector::PcteRequired(IMemoryPool *,		   //mp,
										 CExpressionHandle &,  //exprhdl,
										 CCTEReq *pcter,
										 ULONG
#ifdef GPOS_DEBUG
...
//---------------------------------------------------------------------------
COrderSpec *
CPhysicalPartitionSelector::PosDerive(IMemoryPool *,  // mp
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalPartitionSelector::PdsDerive(IMemoryPool *,  // mp
									  CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
CPartIndexMap *
CPhysicalPartitionSelector::PpimDerive(IMemoryPool *mp,
									   CExpressionHandle &exprhdl,
									   CDrvdPropCtxt *pdpctxt) const
{
	...
	CPartIndexMap *ppim =
		ppimInput->PpimPartitionSelector(mp, m_scan_id, ulExpectedPartitionSelectors);
	if (!ppim->Contains(m_scan_id))
	{
		...
		ppim->Insert(m_scan_id,
					 m_ppartcnstrmap,
					 CPartIndexMap::EpimPropagator,
...
//---------------------------------------------------------------------------
CRewindabilitySpec *
CPhysicalPartitionSelector::PrsDerive(IMemoryPool *,  // mp
									  CExpressionHandle &exprhdl) const
{
