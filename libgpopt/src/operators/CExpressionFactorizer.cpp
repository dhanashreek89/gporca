...
//---------------------------------------------------------------------------
CExpression *
CExpressionFactorizer::PexprProcessDisjDescendents(IMemoryPool *mp,
												   CExpression *pexpr,
												   ...
												   PexprProcessDisj pfnpepdFunction)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		CExpression *pexprChild =
			PexprProcessDisjDescendents(mp, (*pexpr)[ul], pexprLogicalAncestor, pfnpepdFunction);
...
//---------------------------------------------------------------------------
void
CExpressionFactorizer::AddFactor(IMemoryPool *mp,
								 CExpression *pexpr,
...
CExpressionFactorizer::PexprmapFactors(IMemoryPool *mp, CExpression *pexpr)
{
...
//---------------------------------------------------------------------------
CExpression *
CExpressionFactorizer::PexprFactorizeDisj(IMemoryPool *mp,
										  CExpression *pexpr,
...
)
{
	...
	for (ULONG ulOuter = 0; ulOuter < ulDisjuncts; ulOuter++)
	{
		...
		{
			AddFactor(
				mp, pexprDisj, pdrgpexprFactors, pdrgpexprResidual, pexprmapFactors, ulDisjuncts);
		}
	}
...
//---------------------------------------------------------------------------
CExpression *
CExpressionFactorizer::PexprDiscoverFactors(IMemoryPool *mp, CExpression *pexpr)
{
...
//---------------------------------------------------------------------------
CExpression *
CExpressionFactorizer::PexprFactorize(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != pexpr);
...
CExpressionFactorizer::PcrsUsedByPushableScalar(CExpression *pexpr)
{
	...
	if (0 < pdpscalar->PcrsDefined()->Size() || pdpscalar->FHasSubquery() ||
...
//---------------------------------------------------------------------------
CExpressionArrays *
CExpressionFactorizer::PdrgPdrgpexprDisjunctArrayForSourceId(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CExpressionArrays *
CExpressionFactorizer::PdrgPdrgpexprDisjunctArrayForColumn(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
void
CExpressionFactorizer::StoreBaseOpToColumnExpr(IMemoryPool *mp,
											   CExpression *pexpr,
											   SourceToArrayPosMap *psrc2array,
											   ColumnToArrayPosMap *pcol2array,
											   ...
											   ULONG ulPosition)
{
	...
	{
		pdrgpdrgpexpr =
			PdrgPdrgpexprDisjunctArrayForSourceId(mp, psrc2array, fAllowNewSources, ulOpSourceId);
	}
	else
	{
		...
		pdrgpdrgpexpr =
			PdrgPdrgpexprDisjunctArrayForColumn(mp, pcol2array, fAllowNewSources, pcrComputed);
	}
...
//---------------------------------------------------------------------------
CExpression *
CExpressionFactorizer::PexprAddInferredFilters(IMemoryPool *mp,
											   CExpression *pexpr,
											   SourceToArrayPosMap *psrc2array,
											   ColumnToArrayPosMap *pcol2array)
{
...
//---------------------------------------------------------------------------
void
CExpressionFactorizer::AddInferredFiltersFromArray(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CColRefSet *
CExpressionFactorizer::PcrsColumnsProducedByChildren(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
	const ULONG arity = pexpr->Arity();
		...
		for (ULONG ulBelowChild = 0; ulBelowChild < ulChildArity; ulBelowChild++)
		{
			...
			if (pexprGrandChild->Pop()->FScalar())
			{
				CColRefSet *pcrsChildDefined =
					CDrvdPropScalar::GetDrvdScalarProps(pexprGrandChild->PdpDerive())
						->PcrsDefined();
...
//---------------------------------------------------------------------------
CExpression *
CExpressionFactorizer::PexprExtractInferredFiltersFromDisj(IMemoryPool *mp,
														   CExpression *pexpr,
														   CExpression *pexprLowestLogicalAncestor)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CExpressionFactorizer::PexprExtractInferredFilters(IMemoryPool *mp, CExpression *pexpr)
{
	return PexprProcessDisjDescendents(
		mp, pexpr, NULL /*pexprLowestLogicalAncestor*/, PexprExtractInferredFiltersFromDisj);
}
