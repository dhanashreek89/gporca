...
//
//---------------------------------------------------------------------------
CScalarArrayCmp::CScalarArrayCmp(IMemoryPool *mp,
								 IMDId *mdid_op,
	  ...
	  m_returns_null_on_null_input(false)
{
	...
	m_returns_null_on_null_input =
		CMDAccessorUtils::FScalarOpReturnsNullOnNullInput(md_accessor, m_mdid_op);
}
...
{
	return gpos::CombineHashes(gpos::CombineHashes(COperator::HashValue(), m_mdid_op->HashValue()),
							   m_earrccmpt);
}
...
//---------------------------------------------------------------------------
CExpression *
CScalarArrayCmp::PexprExpand(IMemoryPool *mp, CExpression *pexprArrayCmp)
{
	...
	for (ULONG ul = 0; ul < ulArrayElems; ul++)
	{
		...
		CExpression *pexprCmp =
			CUtils::PexprScalarCmp(mp, pexprIdent, pexprArrayElem, *str_opname, mdid_op);
