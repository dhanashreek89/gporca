...
//---------------------------------------------------------------------------
BOOL
CNormalizer::FPushableThruSeqPrjChild(CExpression *pexprSeqPrj, CExpression *pexprPred)
{
...
//---------------------------------------------------------------------------
CExpression *
CNormalizer::PexprRecursiveNormalize(IMemoryPool *mp, CExpression *pexpr)
{
...
//---------------------------------------------------------------------------
void
CNormalizer::SplitConjunct(IMemoryPool *mp,
						   CExpression *pexpr,
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThruOuterChild(IMemoryPool *mp,
								CExpression *pexpr,
								CExpression *pexprConj,
								CExpression **ppexprResult)
{
	...
	if (0 < pdrgpexprPushable->Size())
	{
		...
		CExpression *pexprNew =
			GPOS_NEW(mp) CExpression(mp, pop, pexprNewOuter, pexprInner, pexprPred);
...
//---------------------------------------------------------------------------
BOOL
CNormalizer::FSimplifySelectOnOuterJoin(IMemoryPool *mp,
										...
										CExpression **ppexprResult)
{
	...
	CColRefSet *pcrsOutput =
		CDrvdPropRelational::GetRelationalProperties(pexprOuterJoinInnerChild->PdpDerive())
			->PcrsOutput();
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThruSelect(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CExpression *
CNormalizer::PexprSelect(IMemoryPool *mp, CExpression *pexpr, CExpressionArray *pdrgpexpr)
{
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThruUnaryWithoutScalarChild(IMemoryPool *mp,
											 ...
											 CExpression **ppexprResult)
{
	...
	CExpressionArray *pdrgpexprUnpushable = NULL;
	PushThru(
		mp, pexprLogicalChild, pdrgpexprConjuncts, &pexprNewLogicalChild, &pdrgpexprUnpushable);
	pdrgpexprConjuncts->Release();
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThruUnaryWithScalarChild(IMemoryPool *mp,
										  CExpression *pexprLogical,
										  CExpression *pexprConj,
										  CExpression **ppexprResult)
{
	...
	CExpression *pexprNewLogical =
		GPOS_NEW(mp) CExpression(mp, pop, pexprNewLogicalChild, pexprScalarChild);
	*ppexprResult = PexprSelect(mp, pexprNewLogical, pdrgpexprUnpushable);
}
...
//---------------------------------------------------------------------------
void
CNormalizer::SplitConjunctForSeqPrj(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThruSeqPrj(IMemoryPool *mp,
							CExpression *pexprSeqPrj,
							CExpression *pexprConj,
							CExpression **ppexprResult)
{
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThruSetOp(IMemoryPool *mp,
						   ...
						   CExpression **ppexprResult)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (!pcrsChild->Equals(pcrsOutput))
		{
			...
			pexprRemappedConj =
				pexprConj->PexprCopyWithRemappedColumns(mp, colref_mapping, true /*must_exist*/);
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThruJoin(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
void
CNormalizer::PushThru(IMemoryPool *mp,
					  CExpression *pexprLogical,
					  CExpression *pexprConj,
					  CExpression **ppexprResult)
{
...
//---------------------------------------------------------------------------
CExpression *
CNormalizer::PexprNormalize(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(NULL != pexpr);
...
)
{
	...
	if (0 < pdrgpexprPrElPullUp->Size() && COperator::EopLogicalProject == pop->Eopid())
	{
		...
		*pfSuccess = true;
		...
		CExpression *pexprPrjList =
			GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprPrElPullUp);
		GPOS_ASSERT(
			CDrvdPropRelational::GetRelationalProperties(pexprRelational->PdpDerive())
				->PcrsOutput()
				->ContainsAll(
	...
	*pfSuccess = true;
	CExpression *pexprPrjList =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprPrElPullUp);
	GPOS_ASSERT(
		CDrvdPropRelational::GetRelationalProperties(pexprOutput->PdpDerive())
			->PcrsOutput()
			->ContainsAll(
				CDrvdPropScalar::GetDrvdScalarProps(pexprPrjList->PdpDerive())->PcrsUsed()));

	return GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalProject(mp), pexprOutput, pexprPrjList);
}
...
)
{
	...
	else
	{
		...
		CExpression *pexprPrjList = GPOS_NEW(mp)
			CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprPrElNoPullUp);
		pexprNew =
			GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalProject(mp), pexprNew, pexprPrjList);
	}
...
//---------------------------------------------------------------------------
CExpression *
CNormalizer::PexprPullUpProjections(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
BOOL
CNormalizer::FLocalColsSubsetOfInputCols(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
	GPOS_CHECK_STACK_SIZE;
