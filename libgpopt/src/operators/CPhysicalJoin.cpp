...
//
//---------------------------------------------------------------------------
CPhysicalJoin::CPhysicalJoin(IMemoryPool *mp) : CPhysical(mp)
{
...
//---------------------------------------------------------------------------
COrderSpec *
CPhysicalJoin::PosPropagateToOuter(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CColRefSet *
CPhysicalJoin::PcrsRequired(IMemoryPool *mp,
							CExpressionHandle &exprhdl,
...
CPhysicalJoin::PppsRequired(IMemoryPool *mp,
							CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CCTEReq *
CPhysicalJoin::PcteRequired(IMemoryPool *mp,
							CExpressionHandle &exprhdl,
							CCTEReq *pcter,
							ULONG child_index,
							CDrvdPropArrays *pdrgpdpCtxt,
...
//----------------------------------------------------------------------------
BOOL
CPhysicalJoin::FSortColsInOuterChild(IMemoryPool *mp, CExpressionHandle &exprhdl, COrderSpec *pos)
{
...
//---------------------------------------------------------------------------
BOOL
CPhysicalJoin::FOuterProvidesReqdCols(CExpressionHandle &exprhdl, CColRefSet *pcrsRequired)
{
	GPOS_ASSERT(NULL != pcrsRequired);
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalJoin::PdsRequired(IMemoryPool *mp,
						   CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalJoin::PdsDerive(IMemoryPool *,  // mp,
...
//---------------------------------------------------------------------------
CRewindabilitySpec *
CPhysicalJoin::PrsDerive(IMemoryPool *mp, CExpressionHandle &exprhdl) const
{
...
CPhysicalJoin::EpetRewindability(CExpressionHandle &exprhdl, const CEnfdRewindability *per) const
{
...
//---------------------------------------------------------------------------
BOOL
CPhysicalJoin::FHashJoinCompatible(CExpression *pexprOuter,		 // outer child of the join
...
)
{
	...
	CColRefSet *outer_refs =
		CDrvdPropRelational::GetRelationalProperties(pexprOuter->Pdp(DrvdPropArray::EptRelational))
			->PcrsOutput();
	CColRefSet *pcrsInner =
		CDrvdPropRelational::GetRelationalProperties(pexprInner->Pdp(DrvdPropArray::EptRelational))
			->PcrsOutput();
	...
	BOOL fPredOuterUsesJoinOuterChild =
		(0 < pcrsUsedPredOuter->Size()) && outer_refs->ContainsAll(pcrsUsedPredOuter);
	BOOL fPredOuterUsesJoinInnerChild =
		(0 < pcrsUsedPredOuter->Size()) && pcrsInner->ContainsAll(pcrsUsedPredOuter);
	BOOL fPredInnerUsesJoinOuterChild =
		(0 < pcrsUsedPredInner->Size()) && outer_refs->ContainsAll(pcrsUsedPredInner);
	BOOL fPredInnerUsesJoinInnerChild =
		(0 < pcrsUsedPredInner->Size()) && pcrsInner->ContainsAll(pcrsUsedPredInner);

	BOOL fHashJoinCompatiblePred = (fPredOuterUsesJoinOuterChild && fPredInnerUsesJoinInnerChild) ||
								   (fPredOuterUsesJoinInnerChild && fPredInnerUsesJoinOuterChild);

	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();

	return fHashJoinCompatiblePred && md_accessor->RetrieveType(pmdidTypeOuter)->IsHashable() &&
...
//---------------------------------------------------------------------------
BOOL
CPhysicalJoin::FHashJoinCompatible(CExpression *pexprPred,   // predicate in question
...
//---------------------------------------------------------------------------
void
CPhysicalJoin::AddFilterOnPartKey(IMemoryPool *mp,
								  BOOL fNLJoin,
								  ...
								  CColRefSet *pcrsAllowedRefs)
{
	...
	CExpression *pexprCmp =
		PexprJoinPredOnPartKeys(mp, pexprScalar, ppimSource, part_idx_id, pcrsAllowedRefs);
		...
		if (fOuterPartConsumerTest)
		{
			if (ulChildIndexToTestFirst == child_index)
			{
				...
				ppimResult->AddRequiredPartPropagation(
					ppimSource, part_idx_id, CPartIndexMap::EppraIncrementPropagators);
				pexprCmp->Release();
			}
			else
			{
				...
				ppimResult->AddRequiredPartPropagation(
					ppimSource, part_idx_id, CPartIndexMap::EppraZeroPropagators);
	...
	else if (FProcessingChildWithPartConsumer(fOuterPartConsumerTest,
											  ulChildIndexToTestFirst,
											  ulChildIndexToTestSecond,
											  child_index))
	{
...
//---------------------------------------------------------------------------
CExpression *
CPhysicalJoin::PexprJoinPredOnPartKeys(IMemoryPool *mp,
									   CExpression *pexprScalar,
									   CPartIndexMap *ppimSource,
									   ULONG part_idx_id,
									   CColRefSet *pcrsAllowedRefs)
{
	...
	for (ULONG ulKey = 0; NULL == pexprPred && ulKey < ulKeysets; ulKey++)
	{
		...
		pexprPred = CPredicateUtils::PexprExtractPredicatesOnPartKeys(mp,
																	  pexprScalar,
																	  pdrgpdrgpcrPartKeys,
																	  pcrsAllowedRefs,
...
//---------------------------------------------------------------------------
CRewindabilitySpec *
CPhysicalJoin::PrsRequiredCorrelatedJoin(IMemoryPool *mp,
										 CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalJoin::PdsRequiredCorrelatedJoin(IMemoryPool *mp,
										 CExpressionHandle &exprhdl,
										 CDistributionSpec *pdsRequired,
										 ULONG child_index,
										 CDrvdPropArrays *pdrgpdpCtxt,
...
// Equality function
BOOL
CPhysicalJoin::CPartPropReq::Equals(const CPartPropReq *ppprFst, const CPartPropReq *ppprSnd)
{
...
CPhysicalJoin::PpprCreate(IMemoryPool *mp,
						  CExpressionHandle &exprhdl,
						  CPartitionPropagationSpec *pppsRequired,
						  ULONG child_index)
{
	...
	pppsRequired->AddRef();
	return GPOS_NEW(mp)
		CPartPropReq(pppsRequired, child_index, ulOuterChild, ulInnerChild, ulScalarChild);
}


// Compute required partition propagation of the n-th child
CPartitionPropagationSpec *
CPhysicalJoin::PppsRequiredCompute(IMemoryPool *mp,
								   CExpressionHandle &exprhdl,
								   CPartitionPropagationSpec *pppsRequired,
								   ULONG child_index,
								   BOOL fNLJoin)
	...
	for (ULONG ul = 0; ul < ulPartIndexIds; ul++)
	{
		...
		if (fNLJoin)
		{
			if (0 == child_index && fOuterPartConsumer)
			{
				...
				ppimResult->AddRequiredPartPropagation(
					ppim, part_idx_id, CPartIndexMap::EppraPreservePropagators, pdrgppartkeys);
			}
			else
			{
		...
		else
		{
			if (1 == child_index && !fOuterPartConsumer)
			{
				...
				CPartKeysArray *pdrgppartkeys = exprhdl.GetRelationalProperties(1 /*child_index*/)
													->Ppartinfo()
...
// Compute required partition propagation of the n-th child
CPartitionPropagationSpec *
CPhysicalJoin::PppsRequiredJoinChild(IMemoryPool *mp,
									 CExpressionHandle &exprhdl,
									 CPartitionPropagationSpec *pppsRequired,
									 ULONG child_index,
									 CDrvdPropArrays *,  //pdrgpdpCtxt,
