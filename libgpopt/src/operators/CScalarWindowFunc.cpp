...
//
//---------------------------------------------------------------------------
CScalarWindowFunc::CScalarWindowFunc(IMemoryPool *mp,
									 IMDId *mdid_func,
...
CScalarWindowFunc::HashValue() const
{
	return gpos::CombineHashes(
		CombineHashes(
			CombineHashes(CombineHashes(gpos::CombineHashes(
											COperator::HashValue(),
...
CScalarWindowFunc::Matches(COperator *pop) const
{
	if (pop->Eopid() == Eopid())
	{
		...
		return ((popFunc->IsDistinct() == m_is_distinct) &&
				...
				m_return_type_mdid->Equals(popFunc->MDIdType()) && (popFunc->Ews() == m_ewinstage));
	}
