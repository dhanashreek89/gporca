...
BOOL
CPredicateUtils::FValidRefsOnly(CExpression *pexprScalar, CColRefSet *pcrsAllowedRefs)
{
	...
	return CUtils::FVarFreeExpr(pexprScalar) && IMDFunction::EfsVolatile != pdpscalar->Pfp()->Efs();
}

// is the given expression a conjunction of equality comparisons
BOOL
CPredicateUtils::FConjunctionOfEqComparisons(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
...
void
CPredicateUtils::CollectChildren(CExpression *pexpr,
...
void
CPredicateUtils::CollectConjuncts(CExpression *pexpr, CExpressionArray *pdrgpexpr)
{
...
void
CPredicateUtils::CollectDisjuncts(CExpression *pexpr, CExpressionArray *pdrgpexpr)
{
	GPOS_CHECK_STACK_SIZE;
...
CExpressionArray *
CPredicateUtils::PdrgpexprConjuncts(IMemoryPool *mp, CExpression *pexpr)
{
...
CExpressionArray *
CPredicateUtils::PdrgpexprDisjuncts(IMemoryPool *mp, CExpression *pexpr)
{
	CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
...
CExpressionArray *
CPredicateUtils::PdrgpexprExpandDisjuncts(IMemoryPool *mp, CExpressionArray *pdrgpexprDisjuncts)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		if (COperator::EopScalarArrayCmp == pexpr->Pop()->Eopid())
		{
			...
			if (FOr(pexprExpanded))
			{
				...
				CUtils::AddRefAppend<CExpression, CleanupRelease>(pdrgpexprExpanded,
...
CExpressionArray *
CPredicateUtils::PdrgpexprExpandConjuncts(IMemoryPool *mp, CExpressionArray *pdrgpexprConjuncts)
{
...
void
CPredicateUtils::ExtractComponents(CExpression *pexprScCmp,
								   CColRef *pcrKey,
...
BOOL
CPredicateUtils::FIdentCompare(CExpression *pexpr, IMDType::ECmpType pecmpt, CColRef *colref)
{
	GPOS_ASSERT(NULL != pexpr);
...
CExpression *
CPredicateUtils::PexprConjDisj(IMemoryPool *mp, CExpressionArray *pdrgpexpr, BOOL fConjunction)
{
...
CExpression *
CPredicateUtils::PexprConjunction(IMemoryPool *mp, CExpressionArray *pdrgpexpr)
{
...
CExpression *
CPredicateUtils::PexprDisjunction(IMemoryPool *mp, CExpressionArray *pdrgpexpr)
{
...
CExpression *
CPredicateUtils::PexprConjDisj(IMemoryPool *mp,
...
CExpression *
CPredicateUtils::PexprConjunction(IMemoryPool *mp, CExpression *pexprOne, CExpression *pexprTwo)
{
...
CExpression *
CPredicateUtils::PexprDisjunction(IMemoryPool *mp, CExpression *pexprOne, CExpression *pexprTwo)
{
...
CExpressionArray *
CPredicateUtils::PdrgpexprPlainEqualities(IMemoryPool *mp, CExpressionArray *pdrgpexpr)
{
...
CExpression *
CPredicateUtils::PexprEliminateSelfComparison(IMemoryPool *mp, CExpression *pexpr)
{
...
CExpression *
CPredicateUtils::PexprINDFConjunction(IMemoryPool *mp,
...
BOOL
CPredicateUtils::FIdentCompareConstIgnoreCast(CExpression *pexpr, COperator::EOperatorId op_id)
{
	COperator *pop = pexpr->Pop();
...
BOOL
CPredicateUtils::FCompareScalarIdentToConstAndScalarIdentArray(CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
	...
	CColRefSet *pcrsAllowedRefs)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		if (FBoolPredicateOnColumn(pexpr, pcrPartKey) || FNullCheckOnColumn(pexpr, pcrPartKey) ||
...
CExpressionArray *
CPredicateUtils::PdrgpexprAppendConjunctsDedup(IMemoryPool *mp,
...
CPredicateUtils::FBoolPredicateOnColumn(CExpression *pexpr, CColRef *colref)
{
	...
	if (fBoolean && (CUtils::FScalarIdent(pexpr, colref) ||
...
// given column
BOOL
CPredicateUtils::FScArrayCmpOnColumn(CExpression *pexpr, CColRef *colref, BOOL fConstOnly)
{
	GPOS_ASSERT(NULL != pexpr);
...
// on the given column
BOOL
CPredicateUtils::FDisjunctionOnColumn(IMemoryPool *mp,
									  CExpression *pexpr,
									  CColRef *colref,
									  CColRefSet *pcrsAllowedRefs)
{
	...
	for (ULONG ulDisj = 0; ulDisj < ulDisjuncts; ulDisj++)
	{
		...
		if (!FComparison(pexprDisj, colref, pcrsAllowedRefs) ||
			!FRangeComparison(CScalarCmp::PopConvert(pexprDisj->Pop())->ParseCmpType()))
		{
			pdrgpexprDisjuncts->Release();
			return false;
		}
...
// LT, GT, LEq, GEq, Eq
BOOL
CPredicateUtils::FRangeComparison(IMDType::ECmpType cmp_type)
{
...
// constant filters.
CExpression *
CPredicateUtils::PexprExtractPredicatesOnPartKeys(IMemoryPool *mp,
												  ...
												  BOOL fUseConstraints)
{
	...
	if (pdpScalar->FHasScalarArrayCmp() && !GPOS_FTRACE(EopttraceArrayConstraints))
	{
...
CExpression *
CPredicateUtils::PexprPredicateCol(IMemoryPool *mp,
								   CConstraint *pcnstr,
								   CColRef *colref,
								   BOOL fUseConstraints)
{
...
BOOL
CPredicateUtils::FConstColumn(CConstraint *pcnstr,
							  const CColRef *
#ifdef GPOS_DEBUG
								  colref
#endif  // GPOS_DEBUG
...
BOOL
CPredicateUtils::FColumnDisjunctionOfConst(CConstraint *pcnstr, const CColRef *colref)
{
...
CExpression *
CPredicateUtils::PexprIndexLookupKeyOnLeft(IMemoryPool *mp,
										   ...
										   CColRefSet *outer_refs)
{
	...
	if ((CUtils::FScalarIdent(pexprLeft) &&
		 pcrsIndex->FMember(CScalarIdent::PopConvert(pexprLeft->Pop())->Pcr())) ||
		 ...
		 pcrsIndex->FMember(CScalarIdent::PopConvert((*pexprLeft)[0]->Pop())->Pcr())))
	{
		...
		CColRefSet *pcrsUsedRight =
			CDrvdPropScalar::GetDrvdScalarProps(pexprRight->PdpDerive())->PcrsUsed();
		BOOL fSuccess = true;
		...
		fSuccess = (fSuccess &&
					FCompatibleIndexPredicate(pexprScalar, pmdindex, pdrgpcrIndex, md_accessor));
...
CExpression *
CPredicateUtils::PexprIndexLookupKeyOnRight(IMemoryPool *mp,
											CMDAccessor *md_accessor,
											CExpression *pexprScalar,
											const IMDIndex *pmdindex,
											CColRefArray *pdrgpcrIndex,
											CColRefSet *outer_refs)
{
	...
	if (CUtils::FScalarCmp(pexprScalar))
	{
		...
		if (popScCmpCommute)
		{
			...
			pexprRight->AddRef();
			pexprLeft->AddRef();
			CExpression *pexprCommuted =
				GPOS_NEW(mp) CExpression(mp, popScCmpCommute, pexprRight, pexprLeft);
			CExpression *pexprIndexCond = PexprIndexLookupKeyOnLeft(
				mp, md_accessor, pexprCommuted, pmdindex, pdrgpcrIndex, outer_refs);
...
CExpression *
CPredicateUtils::PexprIndexLookup(IMemoryPool *mp,
								  CMDAccessor *md_accessor,
								  CExpression *pexprScalar,
								  const IMDIndex *pmdindex,
								  CColRefArray *pdrgpcrIndex,
								  CColRefSet *outer_refs)
{
	...
	if (cmptype == IMDType::EcmptNEq || cmptype == IMDType::EcmptIDF ||
		(cmptype == IMDType::EcmptOther &&
		 pmdindex->IndexType() !=
			 IMDIndex::EmdindGist) ||  // only GiST indexes with a comparison type other are ok
		(pmdindex->IndexType() == IMDIndex::EmdindGist &&
		 pexprScalar->Arity() < 2))  // we do not support index expressions for GiST indexes
	{
		return NULL;
	}

	CExpression *pexprIndexLookupKeyOnLeft =
		PexprIndexLookupKeyOnLeft(mp, md_accessor, pexprScalar, pmdindex, pdrgpcrIndex, outer_refs);
...
)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CColRefSet *pcrsUsed = GPOS_NEW(mp) CColRefSet(
			mp, *CDrvdPropScalar::GetDrvdScalarProps(pexprCond->PdpDerive())->PcrsUsed());
		...
		if (CUtils::FScalarIdentBoolType(pexprCond))
		{
			...
			pexprCond = CUtils::PexprScalarEqCmp(
				mp, pexprCond, CUtils::PexprScalarConstBool(mp, true /*value*/, false /*is_null*/));
		}
		else if (FNot(pexprCond) && CUtils::FScalarIdentBoolType((*pexprCond)[0]))
		{
			...
			pexprCond = CUtils::PexprScalarEqCmp(
				mp,
				pexprScId,
				CUtils::PexprScalarConstBool(mp, false /*value*/, false /*is_null*/));
		}
		else
		{
			// attempt building index lookup predicate
			CExpression *pexprLookupPred = PexprIndexLookup(
				mp, md_accessor, pexprCond, pmdindex, pdrgpcrIndex, pcrsAcceptedOuterRefs);
...
void
CPredicateUtils::SeparateOuterRefs(IMemoryPool *mp,
								   CExpression *pexprScalar,
								   CColRefSet *outer_refs,
...
CExpression *
CPredicateUtils::PexprInverseComparison(IMemoryPool *mp, CExpression *pexprCmp)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
CExpression *
CPredicateUtils::PexprPruneSuperfluosEquality(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(pexpr->Pop()->FScalar());
...
BOOL
CPredicateUtils::FImpliedPredicate(CExpression *pexprPred, CColRefSetArray *pdrgpcrsEquivClasses)
{
...
CExpression *
CPredicateUtils::PexprRemoveImpliedConjuncts(IMemoryPool *mp,
											 CExpression *pexprScalar,
...
BOOL
CPredicateUtils::FValidSemiJoinCorrelations(IMemoryPool *mp,
...
BOOL
CPredicateUtils::FSimpleEqualityUsingCols(IMemoryPool *mp,
										  CExpression *pexprScalar,
										  CColRefSet *pcrs)
{
	GPOS_ASSERT(NULL != pexprScalar);
...
CExpression *
CPredicateUtils::PexprReplaceColsWithNulls(IMemoryPool *mp,
										   CExpression *pexprScalar,
										   CColRefSet *pcrs)
{
	GPOS_CHECK_STACK_SIZE;
...
// are set to NULL
BOOL
CPredicateUtils::FNullRejecting(IMemoryPool *mp, CExpression *pexprScalar, CColRefSet *pcrs)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(NULL != pexprScalar);
	GPOS_ASSERT(pexprScalar->Pop()->FScalar());
...
BOOL
CPredicateUtils::FConvertToCNF(CExpression *pexprOuter,
							   CExpression *pexprInner,
							   CExpression *pexprScalar)
{
	GPOS_ASSERT(NULL != pexprScalar);
...
void
CPredicateUtils::CollectGrandChildrenUnionUnionAll(IMemoryPool *mp,
												   CExpression *pexpr,
												   ULONG child_index,
...
BOOL
CPredicateUtils::FCollapsibleChildUnionUnionAll(CExpression *pexpr, ULONG child_index)
{
	GPOS_ASSERT(NULL != pexpr);
...
//		+--CScalarConstArray
BOOL
CPredicateUtils::FBitmapLookupSupportedPredicateOrConjunct(CExpression *pexpr)
{
	if (CPredicateUtils::FAnd(pexpr))
	{
		...
		{
			result =
				result && CPredicateUtils::FBitmapLookupSupportedPredicateOrConjunct((*pexpr)[ul]);
		}
