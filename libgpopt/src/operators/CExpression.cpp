...
//
//---------------------------------------------------------------------------
CExpression::CExpression(IMemoryPool *mp, COperator *pop, CGroupExpression *pgexpr)
	: m_mp(mp),
	  m_pop(pop),
	  m_pdrgpexpr(NULL),
	  m_pdprel(NULL),
...
//
//---------------------------------------------------------------------------
CExpression::CExpression(IMemoryPool *mp, COperator *pop, CExpression *pexpr)
	: m_mp(mp),
	  m_pop(pop),
	  m_pdrgpexpr(NULL),
	  m_pdprel(NULL),
...
//
//---------------------------------------------------------------------------
CExpression::CExpression(IMemoryPool *mp,
						 COperator *pop,
...
//
//---------------------------------------------------------------------------
CExpression::CExpression(IMemoryPool *mp, COperator *pop, CExpressionArray *pdrgpexpr)
	: m_mp(mp),
	  m_pop(pop),
...
//---------------------------------------------------------------------------
DrvdPropArray *
CExpression::Pdp(const DrvdPropArray::EPropType ept) const
{
	switch (ept)
	{
...
//---------------------------------------------------------------------------
void
CExpression::SetPdp(DrvdPropArray *pdp, const DrvdPropArray::EPropType ept)
{
	GPOS_ASSERT(NULL != pdp);
...
//---------------------------------------------------------------------------
void
CExpression::AssertValidPropDerivation(const DrvdPropArray::EPropType ept)
{
	COperator *pop = Pop();
	...
	GPOS_ASSERT_IMP(pop->FPhysical(),
					DrvdPropArray::EptRelational == ept || DrvdPropArray::EptPlan == ept);

	GPOS_ASSERT_IMP(pop->FPhysical() && DrvdPropArray::EptRelational == ept,
...
//---------------------------------------------------------------------------
void
CExpression::ResetDerivedProperty(DrvdPropArray::EPropType ept)
{
	switch (ept)
	{
		case DrvdPropArray::EptRelational:
...
CExpression::ResetDerivedProperties()
{
	...
	DrvdPropArray::EPropType rgept[] = {
		DrvdPropArray::EptRelational, DrvdPropArray::EptScalar, DrvdPropArray::EptPlan};
...
//---------------------------------------------------------------------------
CReqdPropPlan *
CExpression::PrppCompute(IMemoryPool *mp, CReqdPropPlan *prppInput)
{
...
//---------------------------------------------------------------------------
CReqdPropPlan *
CExpression::PrppDecorate(IMemoryPool *mp, CReqdPropPlan *prppInput)
{
...
//---------------------------------------------------------------------------
CExpression *
CExpression::PexprCopyWithRemappedColumns(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CExpression *
CExpression::PexprRehydrate(IMemoryPool *mp,
							CCostContext *pcc,
							CExpressionArray *pdrgpexpr,
...
CExpression::FValidPlan(const CReqdPropPlan *prpp, CDrvdPropCtxtPlan *pdpctxtplan)
{
	...
	CDrvdPropRelational *pdprel =
		CDrvdPropRelational::GetRelationalProperties(Pdp(DrvdPropArray::EptRelational));
