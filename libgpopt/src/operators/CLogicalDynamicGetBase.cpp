...
//
//---------------------------------------------------------------------------
CLogicalDynamicGetBase::CLogicalDynamicGetBase(IMemoryPool *mp)
	: CLogical(mp),
	  m_pnameAlias(NULL),
	  m_ptabdesc(NULL),
	  m_scan_id(0),
	  ...
	  m_pcrsDist(NULL)
{
	...
	UlongToConstraintMap *phmulcnstr =
		CUtils::PhmulcnstrBoolConstOnPartKeys(mp, m_pdrgpdrgpcrPart, true /*value*/);
	CBitSet *pbsDefaultParts = CUtils::PbsAllSet(mp, m_pdrgpdrgpcrPart->Size());
	m_pdrgpdrgpcrPart->AddRef();
	m_part_constraint = GPOS_NEW(mp)
		CPartConstraint(mp, phmulcnstr, pbsDefaultParts, true /*is_unbounded*/, m_pdrgpdrgpcrPart);
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalDynamicGetBase::PcrsDeriveOutput(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CKeyCollection *
CLogicalDynamicGetBase::PkcDeriveKeys(IMemoryPool *mp,
									  CExpressionHandle &  // exprhdl
									  ) const
{
...
//---------------------------------------------------------------------------
CPropConstraint *
CLogicalDynamicGetBase::PpcDeriveConstraint(IMemoryPool *mp,
											CExpressionHandle &  // exprhdl
											) const
{
...
//---------------------------------------------------------------------------
CPartInfo *
CLogicalDynamicGetBase::PpartinfoDerive(IMemoryPool *mp,
										CExpressionHandle &  // exprhdl
										) const
{
...
//---------------------------------------------------------------------------
IStatistics *
CLogicalDynamicGetBase::PstatsDeriveFilter(IMemoryPool *mp,
										   CExpressionHandle &exprhdl,
										   CExpression *pexprFilter) const
{
	...
	if (m_is_partial && NULL != pcnstr && !pcnstr->IsConstraintUnbounded())
	{
		...
		{
			pexprFilterNew =
				CPredicateUtils::PexprConjunction(mp, pexprFilter, pcnstr->PexprScalar(mp));
		}
	}
	...
	CStatistics *pstatsFullTable =
		dynamic_cast<CStatistics *>(PstatsBaseTable(mp, exprhdl, m_ptabdesc, pcrsStat));
	...
	CStatsPred *pred_stats =
		CStatsPredUtils::ExtractPredStats(mp, pexprFilterNew, NULL /*outer_refs*/
		);
