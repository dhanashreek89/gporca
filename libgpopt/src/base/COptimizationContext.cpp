...
//---------------------------------------------------------------------------
BOOL
COptimizationContext::FOptimize(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
BOOL
COptimizationContext::FOptimizeMotion(IMemoryPool *,	   // mp
									  CGroupExpression *,  // pgexprParent
...
//---------------------------------------------------------------------------
BOOL
COptimizationContext::FOptimizeSort(IMemoryPool *,		 // mp
									CGroupExpression *,  // pgexprParent
...
//---------------------------------------------------------------------------
BOOL
COptimizationContext::FOptimizeAgg(IMemoryPool *mp,
								   CGroupExpression *,  // pgexprParent
								   ...
								   ULONG ulSearchStages)
{
	...
	if (GPOS_FTRACE(EopttraceForceExpandedMDQAs))
	{
		BOOL fHasMultipleDistinctAggs =
			CDrvdPropScalar::GetDrvdScalarProps((*pgexprAgg)[1]->Pdp())->FHasMultipleDistinctAggs();
...
//---------------------------------------------------------------------------
BOOL
COptimizationContext::FOptimizeNLJoin(IMemoryPool *mp,
									  CGroupExpression *,  // pgexprParent
...
)
{
	...
	CColRefSet *pcrsOuterChild =
		CDrvdPropRelational::GetRelationalProperties((*pgexprJoin)[0]->Pdp())->PcrsOutput();
...
//---------------------------------------------------------------------------
CReqdPropPlan *
COptimizationContext::PrppCTEProducer(IMemoryPool *mp,
									  COptimizationContext *poc,
									  ULONG ulSearchStages)
{
	GPOS_ASSERT(NULL != poc);
	...
	BOOL fOptimizeCTESequence = (COperator::EopPhysicalSequence == pgexpr->Pop()->Eopid() &&
	...
	COptimizationContext *pocProducer =
		(*pgexpr)[0]->PocLookupBest(mp, ulSearchStages, (*pccBest->Pdrgpoc())[0]->Prpp());
	if (NULL == pocProducer)
	{
		return NULL;
	}
	...
	COptimizationContext *pocConsumer =
		(*pgexpr)[1]->PocLookupBest(mp, ulSearchStages, (*pccBest->Pdrgpoc())[1]->Prpp());
	if (NULL == pocConsumer)
	{
		return NULL;
	}
	...
	CColRefSet *pcrsInnerOutput =
		CDrvdPropRelational::GetRelationalProperties((*pgexpr)[1]->Pdp())->PcrsOutput();
	...
	CReqdPropPlan *prppProducer =
		CReqdPropPlan::PrppRemap(mp, pocProducer->Prpp(), pccConsumer->Pdpplan(), colref_mapping);
...
{
	os << szPrefix << m_id << " (stage " << m_ulSearchStageIndex << "): (" << *m_prpp
	   << ") => Best Expr:";
	if (NULL != PgexprBest())
	{
		os << PgexprBest()->Id();
