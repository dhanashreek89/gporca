...
IOstream &
CUtils::OsPrintDrgPcoldesc(IOstream &os, CColumnDescriptorArray *pdrgpcoldesc, ULONG ulLengthMax)
{
...
CExpression *
CUtils::PexprScalarIdent(IMemoryPool *mp, const CColRef *colref)
{
...
CExpression *
CUtils::PexprScalarProjectElement(IMemoryPool *mp, CColRef *colref, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != colref);
...
IMDId *
CUtils::GetScCmpMdId(IMemoryPool *mp,
					 ...
					 IMDType::ECmpType cmp_type)
{
	...
	else
	{
		...
		str->AppendFormat(
			GPOS_WSZ_LIT(
				"Cannot generate metadata id for scaler comparison operator between %ls and %ls"),
			left_mdid->GetBuffer(),
			right_mdid->GetBuffer());
...
CExpression *
CUtils::PexprScalarCmp(IMemoryPool *mp,
					   ...
					   IMDId *mdid_op)
{
	...
	return GPOS_NEW(mp) CExpression(
		...
		GPOS_NEW(mp) CScalarCmp(
			mp, mdid_op, GPOS_NEW(mp) CWStringConst(mp, strOp.GetBuffer()), ParseCmpType(mdid_op)),
...
BOOL
CUtils::FCmpOrCastedCmpExists(IMDId *left_mdid, IMDId *right_mdid, IMDType::ECmpType cmp_type)
{
					   ...
					   IMDId *mdid_op)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp)
			CScalarCmp(mp, mdid_op, GPOS_NEW(mp) CWStringConst(mp, strOp.GetBuffer()), cmp_type),
		pexprLeft,
		PexprScalarIdent(mp, pcrRight));
}
					   ...
					   IMDType::ECmpType cmp_type)
{
	...
	else
	{
		CWStringDynamic *str = GPOS_NEW(mp) CWStringDynamic(mp);
		str->AppendFormat(
			GPOS_WSZ_LIT("Cannot generate a comparison expression between %ls and %ls"),
			left_mdid->GetBuffer(),
			right_mdid->GetBuffer());
		GPOS_RAISE(gpopt::ExmaGPOPT, gpopt::ExmiUnexpectedOp, str->GetBuffer());
	}
	...
	CExpression *pexprResult = GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp) CScalarCmp(
			mp, pmdidCmpOp, GPOS_NEW(mp) CWStringConst(mp, strCmpOpName.GetBuffer()), cmp_type),
		pexprNewLeft,
		pexprNewRight);
...
CExpression *
CUtils::PexprScalarEqCmp(IMemoryPool *mp, const CColRef *pcrLeft, const CColRef *pcrRight)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pcrRight);
...
CExpression *
CUtils::PexprScalarEqCmp(IMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
CExpression *
CUtils::PexprScalarEqCmp(IMemoryPool *mp, const CColRef *pcrLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
CExpression *
CUtils::PexprScalarEqCmp(IMemoryPool *mp, CExpression *pexprLeft, const CColRef *pcrRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pcrRight);
...
CExpression *
CUtils::PScalarArrayExprChildAt(IMemoryPool *mp, CExpression *pexprArray, ULONG ul)
{
	ULONG arity = pexprArray->Arity();
	if (0 == arity)
	{
...
CExpression *
CUtils::PexprCollapseConstArray(IMemoryPool *mp, CExpression *pexprArray)
{
	...
	if (FScalarConstArray(pexprArray) && !FScalarArrayCollapsed(pexprArray))
	{
		...
		CScalarArray *pConstArray = GPOS_NEW(mp) CScalarArray(
			mp, elem_type_mdid, array_type_mdid, psArray->FMultiDimensional(), pdrgPconst);
...
CExpression *
CUtils::PexprScalarArrayCmp(IMemoryPool *mp,
							...
							const CColRef *colref)
{
	...
	return GPOS_NEW(mp) CExpression(
		...
		GPOS_NEW(mp) CScalarArrayCmp(
			mp, pmdidCmpOp, GPOS_NEW(mp) CWStringConst(mp, strOp.GetBuffer()), earrcmptype),
...
CExpression *
CUtils::PexprCmpWithZero(IMemoryPool *mp,
						 CExpression *pexprLeft,
						 ...
						 IMDType::ECmpType ecmptype)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp) CScalarCmp(
			mp, mdid_op, GPOS_NEW(mp) CWStringConst(mp, strOpName.GetBuffer()), ecmptype),
		pexprLeft,
...
CExpression *
CUtils::PexprIDF(IMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
	...
	return GPOS_NEW(mp)
		CExpression(mp,
					GPOS_NEW(mp) CScalarIsDistinctFrom(
						mp, pmdidEqOp, GPOS_NEW(mp) CWStringConst(mp, strEqOpName.GetBuffer())),
					pexprNewLeft,
					pexprNewRight);
}
...
CExpression *
CUtils::PexprINDF(IMemoryPool *mp, const CColRef *pcrLeft, const CColRef *pcrRight)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pcrRight);
...
CExpression *
CUtils::PexprINDF(IMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
CExpression *
CUtils::PexprIsNull(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
...
CExpression *
CUtils::PexprIsNotNull(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
...
CExpression *
CUtils::PexprIsNotFalse(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
...
BOOL
CUtils::FUsesNullableCol(IMemoryPool *mp, CExpression *pexprScalar, CExpression *pexprLogical)
{
	GPOS_ASSERT(pexprScalar->Pop()->FScalar());
	GPOS_ASSERT(pexprLogical->Pop()->FLogical());

	CColRefSet *pcrsNotNull =
		CDrvdPropRelational::GetRelationalProperties(pexprLogical->PdpDerive())->PcrsNotNull();
...
CColRef *
CUtils::PcrExtractPartKey(CColRefArrays *pdrgpdrgpcr, ULONG ulLevel)
{
...
CUtils::FHasSubqueryOrApply(CExpression *pexpr, BOOL fCheckRoot)
{
	...
	if (fCheckRoot)
	{
		...
		if (pop->FScalar() &&
			CDrvdPropScalar::GetDrvdScalarProps(pexpr->PdpDerive())->FHasSubquery())
		{
			return true;
		}
	}
...
CUtils::FSubquery(COperator *pop)
{
	...
	COperator::EOperatorId op_id = pop->Eopid();
	return pop->FScalar() &&
		   (COperator::EopScalarSubquery == op_id || COperator::EopScalarSubqueryExists == op_id ||
...
CExpressionArray *
CUtils::PdrgpexprDedup(IMemoryPool *mp, CExpressionArray *pdrgpexpr)
{
...
BOOL
CUtils::Equals(const CExpressionArray *pdrgpexprLeft, const CExpressionArray *pdrgpexprRight)
{
	GPOS_CHECK_STACK_SIZE;
...
BOOL
CUtils::FMatchChildrenUnordered(const CExpression *pexprLeft, const CExpression *pexprRight)
{
	BOOL fEqual = true;
...
BOOL
CUtils::FMatchChildrenOrdered(const CExpression *pexprLeft, const CExpression *pexprRight)
{
	BOOL fEqual = true;
	const ULONG arity = pexprLeft->Arity();
	GPOS_ASSERT(pexprRight->Arity() == arity);
...
BOOL
CUtils::Contains(const CExpressionArray *pdrgpexprFst, const CExpressionArray *pdrgpexprSnd)
{
...
CExpression *
CUtils::PexprNegate(IMemoryPool *mp, CExpression *pexpr)
{
	CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
...
CExpression *
CUtils::PexprScalarOp(IMemoryPool *mp,
					  const CColRef *pcrLeft,
					  CExpression *pexprRight,
					  ...
					  IMDId *return_type_mdid)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp) CScalarOp(
			mp, mdid_op, return_type_mdid, GPOS_NEW(mp) CWStringConst(mp, strOp.GetBuffer())),
		PexprScalarIdent(mp, pcrLeft),
		pexprRight);
}
...
CExpression *
CUtils::PexprScalarBoolOp(IMemoryPool *mp,
...
CExpression *
CUtils::PexprScalarConstBool(IMemoryPool *mp, BOOL fval, BOOL is_null)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
CExpression *
CUtils::PexprScalarConstInt4(IMemoryPool *mp, INT val)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
CExpression *
CUtils::PexprScalarConstInt8(IMemoryPool *mp, LINT val, BOOL is_null)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
CExpression *
CUtils::PexprScalarConstOid(IMemoryPool *mp, OID oid_val)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
)
{
	...
	return GPOS_NEW(mp) CScalarAggFunc(
		mp, pmdidAggFunc, pmdidResolvedReturnType, pstrAggFunc, is_distinct, eaggfuncstage, fSplit);
}

// generate an aggregate function
CExpression *
CUtils::PexprAggFunc(IMemoryPool *mp,
					 IMDId *pmdidAggFunc,
					 const CWStringConst *pstrAggFunc,
					 const CColRef *colref,
					 BOOL is_distinct,
					 ...
					 BOOL fSplit)
{
	...
	CScalarAggFunc *popScAggFunc =
		PopAggFunc(mp, pmdidAggFunc, pstrAggFunc, is_distinct, eaggfuncstage, fSplit);
...
CExpression *
CUtils::PexprCountStar(IMemoryPool *mp)
{
	...
	CScalarAggFunc *popScAggFunc = PopAggFunc(mp,
											  mdid,
											  str,
											  false /*is_distinct*/,
...
CExpression *
CUtils::PexprCountStar(IMemoryPool *mp, CExpression *pexprLogical)
{
...
CExpression *
CUtils::PexprCountStarAndSum(IMemoryPool *mp, const CColRef *colref, CExpression *pexprLogical)
{
	GPOS_ASSERT(CDrvdPropRelational::GetRelationalProperties(pexprLogical->PdpDerive())
					->PcrsOutput()
					->FMember(colref));
	...
	CExpression *pexprPrjList = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprPrjElemCount, pexprPrjElemSum);
...
BOOL
CUtils::FCountAggProjElem(CExpression *pexprPrjElem,
...
CExpression *
CUtils::PexprSum(IMemoryPool *mp, const CColRef *colref)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
CExpression *
CUtils::PexprGbAggSum(IMemoryPool *mp, CExpression *pexprLogical, CColRefArray *pdrgpcrSum)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();
	...
	CExpression *pexprPrjList =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexpr);
...
CExpression *
CUtils::PexprCount(IMemoryPool *mp, const CColRef *colref, BOOL is_distinct)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
CExpression *
CUtils::PexprMin(IMemoryPool *mp, CMDAccessor *md_accessor, const CColRef *colref)
{
...
CExpression *
CUtils::PexprAgg(IMemoryPool *mp,
				 CMDAccessor *md_accessor,
				 ...
				 BOOL is_distinct)
{
	...
	const IMDAggregate *pmdagg =
		md_accessor->RetrieveAgg(colref->RetrieveType()->GetMdidForAggType(agg_type));
	...
	return PexprAggFunc(
		mp, agg_mdid, str, colref, is_distinct, EaggfuncstageGlobal /*fGlobal*/, false /*fSplit*/);
}

// generate a select expression
CExpression *
CUtils::PexprLogicalSelect(IMemoryPool *mp, CExpression *pexpr, CExpression *pexprPredicate)
{
	GPOS_ASSERT(NULL != pexpr);
...
CExpression *
CUtils::PexprSafeSelect(IMemoryPool *mp, CExpression *pexprLogical, CExpression *pexprPred)
{
...
CExpression *
CUtils::PexprCollapseSelect(IMemoryPool *mp, CExpression *pexpr, CExpression *pexprPredicate)
{
	GPOS_ASSERT(NULL != pexpr);
	GPOS_ASSERT(NULL != pexprPredicate);
	...
	)
	{
		...
		return GPOS_NEW(mp)
			CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexprChild, pexprScalar);
	}
...
CExpression *
CUtils::PexprLogicalProject(IMemoryPool *mp,
							CExpression *pexpr,
							CExpression *pexprPrjList,
...
CExpression *
CUtils::PexprLogicalSequenceProject(IMemoryPool *mp,
									...
									CExpression *pexprPrjList)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalSequenceProject(mp, pds, pdrgpos, pdrgpwf), pexpr, pexprPrjList);
}

// construct a projection of NULL constants using the given column
// names and types on top of the given expression
CExpression *
CUtils::PexprLogicalProjectNulls(IMemoryPool *mp,
...
CExpression *
CUtils::PexprScalarProjListConst(IMemoryPool *mp,
								 CColRefArray *colref_array,
								 ...
								 UlongToColRefMap *colref_mapping)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		CColRef *new_colref =
			col_factory->PcrCreate(colref->RetrieveType(), colref->TypeModifier(), colref->Name());
...
CExpression *
CUtils::PexprAddProjection(IMemoryPool *mp, CExpression *pexpr, CExpression *pexprProjected)
{
	GPOS_ASSERT(pexpr->Pop()->FLogical());
...
CExpression *
CUtils::PexprAddProjection(IMemoryPool *mp,
						   CExpression *pexpr,
						   CExpressionArray *pdrgpexprProjected)
{
	...
	CExpression *pexprPrjList =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprPrjElem);
...
CExpression *
CUtils::PexprLogicalGbAgg(IMemoryPool *mp,
						  CColRefArray *colref_array,
...
CExpression *
CUtils::PexprLogicalGbAggGlobal(IMemoryPool *mp,
								CColRefArray *colref_array,
								CExpression *pexprRelational,
								CExpression *pexprProjList)
{
	return PexprLogicalGbAgg(
		mp, colref_array, pexprRelational, pexprProjList, COperator::EgbaggtypeGlobal);
}
...
CColRefArray *
CUtils::PdrgpcrNonSystemCols(IMemoryPool *mp, CColRefSet *pcrs)
{
...
CColRefSetArray *
CUtils::PdrgpcrsAddEquivClass(IMemoryPool *mp, CColRefSet *pcrsNew, CColRefSetArray *pdrgpcrs)
{
...
CColRefSetArray *
CUtils::PdrgpcrsMergeEquivClasses(IMemoryPool *mp,
...
CColRefSetArray *
CUtils::PdrgpcrsIntersectEquivClasses(IMemoryPool *mp,
									  CColRefSetArray *pdrgpcrsFst,
									  CColRefSetArray *pdrgpcrsSnd)
{
...
CColRefSetArray *
CUtils::PdrgpcrsCopyChildEquivClasses(IMemoryPool *mp, CExpressionHandle &exprhdl)
{
	CColRefSetArray *pdrgpcrs = GPOS_NEW(mp) CColRefSetArray(mp);
...
CColRefArray *
CUtils::PdrgpcrExcludeColumns(IMemoryPool *mp,
...
IOstream &
CUtils::OsPrintDrgPcr(IOstream &os, const CColRefArray *colref_array, ULONG ulLenMax)
{
...
BOOL
CUtils::FOpExists(const COperator *pop, const COperator::EOperatorId *peopid, ULONG ulOps)
{
	GPOS_ASSERT(NULL != pop);
...
BOOL
CUtils::FHasOp(const CExpression *pexpr, const COperator::EOperatorId *peopid, ULONG ulOps)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(NULL != pexpr);
	GPOS_ASSERT(NULL != peopid);
...
CUtils::UlInlinableCTEs(CExpression *pexpr, ULONG ulDepth)
{
	...
	if (COperator::EopLogicalCTEConsumer == pop->Eopid())
	{
		...
		CExpression *pexprProducer =
			COptCtxt::PoctxtFromTLS()->Pcteinfo()->PexprCTEProducer(popConsumer->UlCTEId());
...
CUtils::FScalarIdentBoolType(CExpression *pexpr)
{
	return FScalarIdent(pexpr) &&
		   IMDType::EtiBool ==
			   CScalarIdent::PopConvert(pexpr->Pop())->Pcr()->RetrieveType()->GetDatumType();
}
...
{
	COperator *pop = pexpr->Pop();
	return pop->FScalar() && (FScalarCmp(pexpr) || FScalarArrayCmp(pexpr) || FScalarBoolOp(pexpr) ||
...
CUtils::FInnerUsesExternalColsOnly(CExpressionHandle &exprhdl)
{
	return FInnerUsesExternalCols(exprhdl) &&
		   exprhdl.GetRelationalProperties(1)->PcrsOuter()->IsDisjoint(
			   exprhdl.GetRelationalProperties(0)->PcrsOutput());
}

// check if given columns have available comparison operators
BOOL
CUtils::FComparisonPossible(CColRefArray *colref_array, IMDType::ECmpType cmp_type)
{
	GPOS_ASSERT(NULL != colref_array);
...
CColRefArray *
CUtils::PdrgpcrRedistributableSubset(IMemoryPool *mp, CColRefArray *colref_array)
{
	GPOS_ASSERT(NULL != colref_array);
	GPOS_ASSERT(0 < colref_array->Size());
...
CHAR *
CUtils::CreateMultiByteCharStringFromWCString(IMemoryPool *mp, WCHAR *wsz)
{
...
BOOL
CUtils::FFunctionallyDependent(IMemoryPool *mp,
...
ULongPtrArray *
CUtils::Pdrgpul(IMemoryPool *mp, const CColRefArray *colref_array)
{
...
void
CUtils::GenerateFileName(CHAR *buf,
						 const CHAR *szPrefix,
						 const CHAR *szExt,
						 ULONG length,
						 ULONG ulSessionId,
						 ULONG ulCmdId)
{
	...
	str.AppendFormat(wszFileNameTemplate,
					 tm.tm_year + 1900,
					 tm.tm_mon + 1,
					 tm.tm_mday,
					 tm.tm_hour,
...
CColRefSet *
CUtils::PcrsRemap(IMemoryPool *mp,
				  CColRefSet *pcrs,
				  UlongToColRefMap *colref_mapping,
				  BOOL must_exist)
{
...
CColRefArray *
CUtils::PdrgpcrRemap(IMemoryPool *mp,
					 CColRefArray *colref_array,
					 UlongToColRefMap *colref_mapping,
					 BOOL must_exist)
{
...
CColRefArray *
CUtils::PdrgpcrRemapAndCreate(IMemoryPool *mp,
							  CColRefArray *colref_array,
							  UlongToColRefMap *colref_mapping)
{
	GPOS_ASSERT(NULL != colref_array);
...
// and based on the given mapping
CColRefArrays *
CUtils::PdrgpdrgpcrRemap(IMemoryPool *mp,
...
CExpressionArray *
CUtils::PdrgpexprRemap(IMemoryPool *mp,
					   CExpressionArray *pdrgpexpr,
					   UlongToColRefMap *colref_mapping)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
...
UlongToColRefMap *
CUtils::PhmulcrMapping(IMemoryPool *mp, CColRefArray *pdrgpcrFrom, CColRefArray *pdrgpcrTo)
{
...
void
CUtils::AddColumnMapping(IMemoryPool *mp,
						 UlongToColRefMap *colref_mapping,
...
CColRefArray *
CUtils::PdrgpcrExactCopy(IMemoryPool *mp, CColRefArray *colref_array)
{
	CColRefArray *pdrgpcrNew = GPOS_NEW(mp) CColRefArray(mp);
...
CColRefArray *
CUtils::PdrgpcrCopy(IMemoryPool *mp,
					CColRefArray *colref_array,
					BOOL fAllComputed,
					UlongToColRefMap *colref_mapping)
{
...
CColRefSet *
CUtils::PcrsCTEProducerColumns(IMemoryPool *mp,
							   CColRefSet *pcrsInput,
							   CLogicalCTEConsumer *popCTEConsumer)
{
	CExpression *pexprProducer =
		COptCtxt::PoctxtFromTLS()->Pcteinfo()->PexprCTEProducer(popCTEConsumer->UlCTEId());
	GPOS_ASSERT(NULL != pexprProducer);
...
CExpression *
CUtils::PexprConjINDFCond(IMemoryPool *mp, CColRefArrays *pdrgpdrgpcrInput)
{
...
CExpression *
CUtils::PexprCast(IMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexpr, IMDId *mdid_dest)
{
	...
	{
		CScalarCast *popCast = GPOS_NEW(mp)
			CScalarCast(mp, mdid_dest, pmdcast->GetCastFuncMdId(), pmdcast->IsBinaryCoercible());
...
CExpression *
CUtils::PexprLogicalJoin(IMemoryPool *mp, EdxlJoinType edxljointype, CExpressionArray *pdrgpexpr)
{
	COperator *pop = NULL;
	switch (edxljointype)
	{
...
CExpressionArray *
CUtils::PdrgpexprScalarIdents(IMemoryPool *mp, CColRefArray *colref_array)
{
	GPOS_ASSERT(NULL != colref_array);
...
CColRefSet *
CUtils::PcrsExtractColumns(IMemoryPool *mp, const CExpressionArray *pdrgpexpr)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
	CColRefSet *pcrs = GPOS_NEW(mp) CColRefSet(mp);
...
CUtils::PhmulcnstrBoolConstOnPartKeys(IMemoryPool *mp,
	...
	for (ULONG ul = 0; ul < ulLevels; ul++)
	{
		...
		else
		{
			...
			pcnstr = GPOS_NEW(mp) CConstraintInterval(
				mp, pcrPartKey, GPOS_NEW(mp) CRangeArray(mp), false /*is_null*/);
		}
...
CBitSet *
CUtils::PbsAllSet(IMemoryPool *mp, ULONG size)
{
...
CBitSet *
CUtils::Pbs(IMemoryPool *mp, ULongPtrArray *pdrgpul)
{
...
CPartConstraint *
CUtils::PpartcnstrFromMDPartCnstr(IMemoryPool *mp,
								  CMDAccessor *md_accessor,
								  CColRefArrays *pdrgpdrgpcrPartKey,
								  ...
								  BOOL fDummyConstraint)
{
	...
	CExpression *pexprPartCnstr =
		mdpart_constraint->GetPartConstraintExpr(mp, md_accessor, pdrgpcrOutput);
	...
	return GPOS_NEW(mp) CPartConstraint(mp,
										phmulcnstr,
										pbsDefaultParts,
										mdpart_constraint->IsConstraintUnbounded(),
										pdrgpdrgpcrPartKey);
}
...
CExpression *
CUtils::PexprLogicalCTGDummy(IMemoryPool *mp)
{
	CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	...
	return GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalConstTableGet(mp, pdrgpcrCTG, pdrgpdrgpdatum));
}

// map a column from source array to destination array based on position
CColRef *
CUtils::PcrMap(CColRef *pcrSource, CColRefArray *pdrgpcrSource, CColRefArray *pdrgpcrTarget)
{
...
BOOL
CUtils::FMotionOverUnresolvedPartConsumers(IMemoryPool *mp,
...
CUtils::FDuplicateHazardMotion(CExpression *pexprMotion)
{
	...
	BOOL fReplicatedInput =
		CDistributionSpec::EdtReplicated == edtChild || CDistributionSpec::EdtUniversal == edtChild;
...
//				 +--CScalarConst ()
CExpression *
CUtils::PexprCollapseProjects(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
	...
	CColRefSet *pcrsDefinedChild = GPOS_NEW(mp) CColRefSet(
		mp, *CDrvdPropScalar::GetDrvdScalarProps(pexprChildScalar->PdpDerive())->PcrsDefined());
	...
	BOOL fChildProjElHasSetReturn =
		CDrvdPropScalar::GetDrvdScalarProps(pexprChildScalar->PdpDerive())->FHasNonScalarFunction();
	...
	for (ULONG ul1 = 0; ul1 < ulLenPr; ul1++)
	{
		...
		CColRefSet *pcrsUsed = GPOS_NEW(mp)
			CColRefSet(mp, *CDrvdPropScalar::GetDrvdScalarProps(pexprPrE->PdpDerive())->PcrsUsed());

		pexprPrE->AddRef();

		BOOL fHasSetReturn =
			CDrvdPropScalar::GetDrvdScalarProps(pexprPrE->PdpDerive())->FHasNonScalarFunction();
...
void
CUtils::AppendArrayExpr(CExpressionArray *pdrgpexprSrc, CExpressionArray *pdrgpexprDest)
{
...
BOOL
CUtils::FEquivalanceClassesDisjoint(IMemoryPool *mp, const CColRefSetArray *pdrgpcrs)
{
	const ULONG length = pdrgpcrs->Size();
...
BOOL
CUtils::FEquivalanceClassesEqual(IMemoryPool *mp,
								 CColRefSetArray *pdrgpcrsFst,
								 CColRefSetArray *pdrgpcrsSnd)
{
...
CExpression *
CUtils::PexprLimit(IMemoryPool *mp, CExpression *pexpr, ULONG ulOffSet, ULONG count)
{
	GPOS_ASSERT(pexpr);

	COrderSpec *pos = GPOS_NEW(mp) COrderSpec(mp);
	CLogicalLimit *popLimit = GPOS_NEW(mp) CLogicalLimit(
		mp, pos, true /* fGlobal */, true /* fHasCount */, false /*fTopLimitUnderDML*/);
...
CUtils::FGeneratePartOid(IMDId *mdid)
{
	...
	BOOL fInsertSortOnParquet =
		(!GPOS_FTRACE(EopttraceDisableSortForDMLOnParquet) &&
		 pmdrel->RetrieveRelStorageType() == IMDRelation::ErelstorageAppendOnlyParquet);

	COptimizerConfig *optimizer_config = COptCtxt::PoctxtFromTLS()->GetOptimizerConfig();
	BOOL fInsertSortOnRows =
		(pmdrel->RetrieveRelStorageType() == IMDRelation::ErelstorageAppendOnlyRows) &&
