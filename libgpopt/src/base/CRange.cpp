...
//---------------------------------------------------------------------------
CExpression *
CRange::PexprScalar(IMemoryPool *mp, const CColRef *colref)
{
	...
	CExpression *pexprLeft =
		PexprScalarCompEnd(mp, m_pdatumLeft, m_eriLeft, IMDType::EcmptGEq, IMDType::EcmptG, colref);

	CExpression *pexprRight = PexprScalarCompEnd(
		mp, m_pdatumRight, m_eriRight, IMDType::EcmptLEq, IMDType::EcmptL, colref);
...
//---------------------------------------------------------------------------
CExpression *
CRange::PexprEquality(IMemoryPool *mp, const CColRef *colref)
{
	if (NULL == m_pdatumLeft || NULL == m_pdatumRight ||
		!m_pcomp->Equals(m_pdatumLeft, m_pdatumRight) || EriExcluded == m_eriLeft ||
		EriExcluded == m_eriRight)
	{
	...
	CExpression *pexprVal =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarConst(mp, m_pdatumLeft));
...
//---------------------------------------------------------------------------
CExpression *
CRange::PexprScalarCompEnd(IMemoryPool *mp,
						   IDatum *datum,
						   ERangeInclusion eri,
...
CRange::PrngIntersect(IMemoryPool *mp, CRange *prange)
{
	...
	if (FOverlapsLeft(prange))
	{
		...
		return GPOS_NEW(mp)
			CRange(m_mdid, m_pcomp, pdatumLeft, prange->EriLeft(), m_pdatumRight, m_eriRight);
	}
	...
	if (FOverlapsRight(prange))
	{
		...
		return GPOS_NEW(mp)
			CRange(m_mdid, m_pcomp, m_pdatumLeft, m_eriLeft, pdatumRight, prange->EriRight());
	}
...
CRange::PrngDifferenceLeft(IMemoryPool *mp, CRange *prange)
{
	...
	if (FStartsBefore(prange) && NULL != prange->PdatumLeft())
	{
		...
		return GPOS_NEW(mp) CRange(m_mdid,
								   m_pcomp,
								   m_pdatumLeft,
								   m_eriLeft,
								   pdatumRight,
...
{
	if (FDisjointLeft(prange) && m_pcomp->Equals(prange->PdatumLeft(), m_pdatumRight) &&
