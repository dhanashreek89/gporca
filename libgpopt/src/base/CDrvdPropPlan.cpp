...
//---------------------------------------------------------------------------
void
CDrvdPropPlan::Derive(IMemoryPool *mp, CExpressionHandle &exprhdl, CDrvdPropCtxt *pdpctxt)
{
...
//---------------------------------------------------------------------------
void
CDrvdPropPlan::CopyCTEProducerPlanProps(IMemoryPool *mp, CDrvdPropCtxt *pdpctxt, COperator *pop)
{
...
CDrvdPropPlan::FSatisfies(const CReqdPropPlan *prpp) const
{
	...
	return m_pos->FSatisfies(prpp->Peo()->PosRequired()) &&
		   ...
		   m_ppim->FSatisfies(prpp->Pepp()->PppsRequired()) && m_pcm->FSatisfies(prpp->Pcter());
}
...
{
	return m_pos->Matches(pdpplan->Pos()) && m_pds->Matches(pdpplan->Pds()) &&
...
{
	os << "Drvd Plan Props ("
	   << "ORD: " << (*m_pos) << ", DIST: " << (*m_pds) << ", REWIND: " << (*m_prs) << ")"
