...
const CHAR rgszFrameExclusionStrategy[][20] = {
	"None", "Nulls", "Current", "MatchingOthers", "Ties"};
...
//
//---------------------------------------------------------------------------
CWindowFrame::CWindowFrame(IMemoryPool *mp,
						   EFrameSpec efs,
	...
	: m_efs(efs),
	  ...
	  m_pcrsUsed(NULL)
{
	GPOS_ASSERT_IMP(EfbBoundedPreceding == m_efbLeading || EfbBoundedFollowing == m_efbLeading,
					NULL != pexprLeading);
	...
	{
		m_pcrsUsed->Include(
			CDrvdPropScalar::GetDrvdScalarProps(pexprLeading->PdpDerive())->PcrsUsed());
	}

	if (NULL != pexprTrailing)
	{
		m_pcrsUsed->Include(
			CDrvdPropScalar::GetDrvdScalarProps(pexprTrailing->PdpDerive())->PcrsUsed());
	}
}
...
//---------------------------------------------------------------------------
CWindowFrame *
CWindowFrame::PwfCopyWithRemappedColumns(IMemoryPool *mp,
										 ...
										 BOOL must_exist)
{
	...
	{
		pexprTrailing =
			m_pexprTrailing->PexprCopyWithRemappedColumns(mp, colref_mapping, must_exist);
	}

	return GPOS_NEW(mp)
		CWindowFrame(mp, m_efs, m_efbLeading, m_efbTrailing, pexprLeading, pexprTrailing, m_efes);
}
...
//---------------------------------------------------------------------------
BOOL
CWindowFrame::Equals(const CWindowFrameArray *pdrgpwfFirst, const CWindowFrameArray *pdrgpwfSecond)
{
