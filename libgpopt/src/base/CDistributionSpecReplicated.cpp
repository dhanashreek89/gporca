...
CDistributionSpecReplicated::FSatisfies(const CDistributionSpec *pdss) const
{
	...
	if (EdtNonSingleton == pdss->Edt())
	{
		...
		return CDistributionSpecNonSingleton::PdsConvert(const_cast<CDistributionSpec *>(pdss))
			->FAllowReplicated();
	}
...
//---------------------------------------------------------------------------
void
CDistributionSpecReplicated::AppendEnforcers(IMemoryPool *mp,
											 ...
											 CExpression *pexpr)
{
	...
	CExpression *pexprMotion =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPhysicalMotionBroadcast(mp), pexpr);
