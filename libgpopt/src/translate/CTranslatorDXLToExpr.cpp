...
//
//---------------------------------------------------------------------------
CTranslatorDXLToExpr::CTranslatorDXLToExpr(IMemoryPool *mp,
							...
							const CDXLNodeArray *cte_producers)
{
	...
	for (ULONG ul = 0; ul < ulCTEs; ul++)
	{
		...
		BOOL fres =
#endif  // GPOS_DEBUG
			m_phmulpdxlnCTEProducer->Insert(GPOS_NEW(m_mp) ULONG(pdxlopCTEProducer->Id()),
											pdxlnCTE);
		GPOS_ASSERT(fres);
	}
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprTranslateQuery(const CDXLNode *dxlnode,
										  const CDXLNodeArray *query_output_dxlnode_array,
										  const CDXLNodeArray *cte_producers)
{
...
CTranslatorDXLToExpr::PexprLogicalTVF(const CDXLNode *dxlnode)
{
	...
	for (ULONG ul = 0; ul < ulColumns; ul++)
	{
		...
		CColumnDescriptor *pcoldesc = GPOS_NEW(m_mp) CColumnDescriptor(m_mp,
																	   pmdtype,
	...
	CLogicalTVF *popTVF = GPOS_NEW(m_mp)
		CLogicalTVF(m_mp,
					mdid_func,
					mdid_return_type,
					GPOS_NEW(m_mp) CWStringConst(m_mp, dxl_op->MdName()->GetMDName()->GetBuffer()),
					pdrgpcoldesc);
...
CTranslatorDXLToExpr::PexprLogicalGet(const CDXLNode *dxlnode)
{
	...
	if (pmdrel->IsPartitioned())
	{
		...
		CPartConstraint *ppartcnstr =
			CUtils::PpartcnstrFromMDPartCnstr(m_mp,
											  m_pmda,
...
CTranslatorDXLToExpr::PexprLogicalSetOp(const CDXLNode *dxlnode)
{
	...
	switch (dxl_op->GetSetOpType())
	{
		...
		default:
			GPOS_RAISE(
				gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp, dxl_op->GetOpNameStr()->GetBuffer());
	}
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprCastPrjElem(IMDId *pmdidSource,
									   IMDId *mdid_dest,
									   ...
									   CColRef *pcrToReturn)
{
	...
	else
	{
		pexprCast = GPOS_NEW(m_mp) CExpression(
			m_mp,
			GPOS_NEW(m_mp) CScalarCast(
				m_mp, mdid_dest, pmdcast->GetCastFuncMdId(), pmdcast->IsBinaryCoercible()),
			GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarIdent(m_mp, pcrToCast)));
	}

	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectElement(m_mp, pcrToReturn), pexprCast);
}
...
)
{
	...
	*ppexprChild = PexprLogical(child_dxlnode);
	...
	CColRefSet *pcrsChildOutput =
		CDrvdPropRelational::GetRelationalProperties((*ppexprChild)->PdpDerive())->PcrsOutput();
	for (ULONG ulColPos = 0; ulColPos < ulInputCols; ulColPos++)
	{
		...
		BOOL fUnionOrUnionAll = ((EdxlsetopUnionAll == dxl_op->GetSetOpType()) ||
		...
		if (!pcrsChildOutput->FMember(colref))
		{
			...
			CColRef *new_colref =
				PcrCreate(colref, pmdtype, type_modifier, fFirstChild, pdxlcdOutput->Id());
...
//---------------------------------------------------------------------------
CExpressionArray *
CTranslatorDXLToExpr::PdrgpexprPreprocessSetOpInputs(const CDXLNode *dxlnode,
													 ...
													 ULongPtrArray *pdrgpulOutput)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		{
			CExpression *pexprChildProject = GPOS_NEW(m_mp)
				CExpression(m_mp,
...
//---------------------------------------------------------------------------
CColRef *
CTranslatorDXLToExpr::LookupColRef(UlongToColRefMap *colref_mapping, ULONG colid)
{
...
//---------------------------------------------------------------------------
CColRef *
CTranslatorDXLToExpr::PcrCreate(const CColRef *colref,
...
//---------------------------------------------------------------------------
void
CTranslatorDXLToExpr::ConstructDXLColId2ColRefMapping(const CDXLColDescrArray *dxl_col_descr_array,
...
CTranslatorDXLToExpr::PexprLogicalCTEAnchor(const CDXLNode *dxlnode)
{
	...
	CColRefSet *pcrsProducerOuter =
		CDrvdPropRelational::GetRelationalProperties(pexprProducer->PdpDerive())->PcrsOuter();
	if (0 < pcrsProducerOuter->Size())
	{
		GPOS_RAISE(
			gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp, GPOS_WSZ_LIT("CTE with outer references"));
	}
...
CTranslatorDXLToExpr::PexprLogicalCTEProducer(const CDXLNode *dxlnode)
{
	...
	if (0 < pdrgpexprPrEl->Size())
	{
		...
		CExpression *pexprPr = GPOS_NEW(m_mp) CExpression(
			...
			pexprChild,
			GPOS_NEW(m_mp)
				CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectList(m_mp), pdrgpexprPrEl));
		pexprChild = pexprPr;
	}
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CLogicalCTEProducer(m_mp, id, colref_array), pexprChild);
}
...
CTranslatorDXLToExpr::PexprLogicalCTEConsumer(const CDXLNode *dxlnode)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CLogicalCTEConsumer(m_mp, id, pdrgpcrConsumer));
}
...
CTranslatorDXLToExpr::PexprLogicalInsert(const CDXLNode *dxlnode)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CLogicalInsert(m_mp, ptabdesc, colref_array), pexprChild);
}
...
CTranslatorDXLToExpr::PexprLogicalUpdate(const CDXLNode *dxlnode)
{
	...
	return GPOS_NEW(m_mp) CExpression(
		m_mp,
		GPOS_NEW(m_mp) CLogicalUpdate(
			m_mp, ptabdesc, pdrgpcrDelete, pdrgpcrInsert, pcrCtid, pcrSegmentId, pcrTupleOid),
		pexprChild);
}
...
CTranslatorDXLToExpr::PexprLogicalGroupBy(const CDXLNode *dxlnode)
{
	...
	return GPOS_NEW(m_mp) CExpression(
		m_mp,
		GPOS_NEW(m_mp)
			CLogicalGbAgg(m_mp, pdrgpcrGroupingCols, COperator::EgbaggtypeGlobal /*egbaggtype*/
						  ),
		pexprChild,
		pexprProjList);
}
...
CTranslatorDXLToExpr::PexprLogicalLimit(const CDXLNode *dxlnode)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, popLimit, pexprChild, pexprLimitOffset, pexprLimitCount);
}
...
CTranslatorDXLToExpr::PexprLogicalSeqPr(const CDXLNode *dxlnode)
{
	...
	while (hmiterulpdrgexpr.Advance())
	{
		...
		CExpression *pexprProjList =
			GPOS_NEW(m_mp) CExpression(m_mp, popPrL, const_cast<CExpressionArray *>(pdrgpexpr));
		...
		if (0 < colref_array->Size())
		{
			...
			pds = GPOS_NEW(m_mp)
				CDistributionSpecHashed(pdrgpexprScalarIdents, true /* fNullsCollocated */);
		}
		else
		{
		...
		pexprLgSequence =
			GPOS_NEW(m_mp) CExpression(m_mp, popLgSequence, pexprWindowChild, pexprProjList);
...
//---------------------------------------------------------------------------
CColRefArray *
CTranslatorDXLToExpr::PdrgpcrPartitionByCol(const ULongPtrArray *partition_by_colid_array)
{
...
CTranslatorDXLToExpr::Pwf(const CDXLWindowFrame *window_frame)
{
	...
	CWindowFrame *pwf =
		GPOS_NEW(m_mp) CWindowFrame(m_mp, efs, efbLead, efbTrail, pexprLead, pexprTrail, efes);
...
CTranslatorDXLToExpr::Efb(EdxlFrameBoundary frame_boundary) const
{
	...
	CWindowFrame::EFrameBoundary efb = (CWindowFrame::EFrameBoundary)
		window_frame_boundary_to_frame_boundary_mapping[(ULONG) frame_boundary][1];
...
#endif
	CWindowFrame::EFrameExclusionStrategy efeb = (CWindowFrame::EFrameExclusionStrategy)
...
CTranslatorDXLToExpr::Ptabdesc(CDXLTableDescr *table_descr)
{
	...
	for (ULONG ulPos = 0; ulPos < ulAllColumns; ulPos++)
	{
		...
		(void) phmiulAttnoColMapping->Insert(GPOS_NEW(m_mp) INT(pmdcol->AttrNum()),
											 GPOS_NEW(m_mp) ULONG(ulPosNonDropped));
		(void) phmiulAttnoPosMapping->Insert(GPOS_NEW(m_mp) INT(pmdcol->AttrNum()),
											 GPOS_NEW(m_mp) ULONG(ulPos));
	...
	mdid->AddRef();
	CTableDescriptor *ptabdesc = GPOS_NEW(m_mp) CTableDescriptor(m_mp,
																 mdid,
																 CName(m_mp, &strName),
...
CTranslatorDXLToExpr::RegisterMDRelationCtas(CDXLLogicalCTAS *pdxlopCTAS)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CMDColumn *pmdcol =
			GPOS_NEW(m_mp) CMDColumn(GPOS_NEW(m_mp) CMDName(m_mp, pdxlcd->MdName()->GetMDName()),
									 pdxlcd->AttrNum(),
									 pdxlcd->MDIdType(),
									 pdxlcd->TypeModifier(),
									 true,  // is_nullable,
	...
	CMDRelationCtasGPDB *pmdrel = GPOS_NEW(m_mp)
		CMDRelationCtasGPDB(m_mp,
							pdxlopCTAS->MDId(),
							mdname_schema,
...
CTranslatorDXLToExpr::PtabdescFromCTAS(CDXLLogicalCTAS *pdxlopCTAS)
{
	...
	CTableDescriptor *ptabdesc =
		GPOS_NEW(m_mp) CTableDescriptor(m_mp,
										mdid,
										CName(m_mp, &strName),
										pmdrel->ConvertHashToRandom(),
										rel_distr_policy,
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprScalarSubqueryExistential(Edxlopid edxlopid, CDXLNode *pdxlnLogicalChild)
{
...
CTranslatorDXLToExpr::PexprLogicalConstTableGet(const CDXLNode *pdxlnConstTable)
{
	...
	CLogicalConstTableGet *popConstTableGet =
		GPOS_NEW(m_mp) CLogicalConstTableGet(m_mp, pdrgpcoldesc, pdrgpdrgpdatum);
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprScalarSubqueryQuantified(Edxlopid edxlopid,
													...
													CDXLNode *pdxlnScalarChild)
{
	...
	if (EdxlopScalarSubqueryAny == edxlopid)
	{
		popScalarSubquery = GPOS_NEW(m_mp) CScalarSubqueryAny(
			m_mp, scalar_op_mdid, GPOS_NEW(m_mp) CWStringConst(m_mp, str->GetBuffer()), colref);
	}
	else
	{
	...
	CExpression *pexpr =
		GPOS_NEW(m_mp) CExpression(m_mp, popScalarSubquery, pexprLogicalChild, pexprScalarChild);
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprScalarSubqueryQuantified(const CDXLNode *pdxlnSubquery)
{
...
CTranslatorDXLToExpr::PexprScalar(const CDXLNode *pdxlnOp)
{
	...
	{
		return PexprScalarSubqueryExistential(pdxlnOp->GetOperator()->GetDXLOperator(),
											  (*pdxlnOp)[0]);
	}
...
CTranslatorDXLToExpr::PexprCollapseNot(const CDXLNode *pdxlnNotExpr)
{
	...
	if (EdxlopScalarSubqueryAny == edxlopid || EdxlopScalarSubqueryAll == edxlopid)
	{
		...
		Edxlopid edxlopidNew = (EdxlopScalarSubqueryAny == edxlopid) ? EdxlopScalarSubqueryAll
...
CTranslatorDXLToExpr::PexprScalarBoolOp(const CDXLNode *pdxlnBoolExpr)
{
	...
	EdxlBoolExprType edxlbooltype =
		CDXLScalarBoolExpr::Cast(pdxlnBoolExpr->GetOperator())->GetDxlBoolTypeStr();
...
CTranslatorDXLToExpr::PexprScalarNullIf(const CDXLNode *pdxlnNullIf)
{
	...
	return GPOS_NEW(m_mp) CExpression(
		m_mp, GPOS_NEW(m_mp) CScalarNullIf(m_mp, mdid_op, mdid_type), pexprLeft, pexprRight);
}
...
CTranslatorDXLToExpr::PexprScalarFunc(const CDXLNode *pdxlnFunc)
{
	...
	if (CTranslatorDXLToExprUtils::FCastFunc(m_pmda, pdxlnFunc, pmdidInput))
	{
		...
		{
			pop = GPOS_NEW(m_mp)
				CScalarCast(m_mp, mdid_return_type, mdid_func, pmdcast->IsBinaryCoercible());
		}
	}
	else
	{
...
CTranslatorDXLToExpr::PexprWindowFunc(const CDXLNode *pdxlnWindowRef)
{
	...
	CWStringConst *str_name = GPOS_NEW(m_mp)
		CWStringConst(m_mp, CMDAccessorUtils::PstrWindowFuncName(m_pmda, mdid_func)->GetBuffer());
	...
	CScalarWindowFunc *popWindowFunc =
		GPOS_NEW(m_mp) CScalarWindowFunc(m_mp,
										 mdid_func,
										 mdid_return_type,
										 str_name,
										 ews,
										 pdxlopWinref->IsDistinct(),
										 pdxlopWinref->IsStarArg(),
...
CTranslatorDXLToExpr::PexprScalarCoalesce(const CDXLNode *pdxlnCoalesce)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CScalarCoalesce(m_mp, mdid), pdrgpexprChildren);
}
...
CTranslatorDXLToExpr::PexprScalarMinMax(const CDXLNode *pdxlnMinMax)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CScalarMinMax(m_mp, mdid, esmmt), pdrgpexprChildren);
}
	...
	if (0 < pdxlnAggref->Arity())
	{
		...
		for (ULONG ul = 0; ul < pdrgpexprArgs->Size(); ul++)
		{
			...
			CDrvdPropScalar *pdpScalar =
				CDrvdPropScalar::GetDrvdScalarProps(pexprAggrefChild->PdpDerive());
...
CTranslatorDXLToExpr::PexprArray(const CDXLNode *dxlnode)
{
	...
	CScalarArray *popArray = GPOS_NEW(m_mp)
		CScalarArray(m_mp, elem_type_mdid, array_type_mdid, dxl_op->IsMultiDimensional());
...
CTranslatorDXLToExpr::PexprArrayRef(const CDXLNode *dxlnode)
{
	...
	CScalarArrayRef *popArrayref = GPOS_NEW(m_mp) CScalarArrayRef(
		m_mp, elem_type_mdid, dxl_op->TypeModifier(), array_type_mdid, return_type_mdid);
...
CTranslatorDXLToExpr::Eilt(const CDXLScalarArrayRefIndexList::EIndexListBound eilb)
{
	switch (eilb)
	{
...
CTranslatorDXLToExpr::PexprArrayCmp(const CDXLNode *dxlnode)
{
	...
	CScalarArrayCmp *popArrayCmp = GPOS_NEW(m_mp) CScalarArrayCmp(
		m_mp, mdid_op, GPOS_NEW(m_mp) CWStringConst(m_mp, str_opname->GetBuffer()), earrcmpt);
...
CTranslatorDXLToExpr::PexprScalarSwitchCase(const CDXLNode *pdxlnSwitchCase)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CScalarSwitchCase(m_mp), pdrgpexprChildren);
}
...
CTranslatorDXLToExpr::PexprScalarNullTest(const CDXLNode *pdxlnNullTest)
{
	...
	CExpression *pexpr =
		GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarNullTest(m_mp), pexprChild);

	if (!dxl_op->IsNullTest())
	{
		// IS NOT NULL test: add a not expression on top
		pexpr = GPOS_NEW(m_mp)
			CExpression(m_mp, GPOS_NEW(m_mp) CScalarBoolOp(m_mp, CScalarBoolOp::EboolopNot), pexpr);
	}
...
CTranslatorDXLToExpr::PexprScalarBooleanTest(const CDXLNode *pdxlnScBoolTest)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CScalarBooleanTest(m_mp, ebt), pexprChild);
}
...
CTranslatorDXLToExpr::PexprScalarCast(const CDXLNode *pdxlnCast)
{
	...
	else
	{
		pexpr = GPOS_NEW(m_mp) CExpression(
			m_mp, GPOS_NEW(m_mp) CScalarCast(m_mp, mdid_type, mdid_func, fRelabel), pexprChild);
	}
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprScalarArrayCoerceExpr(const CDXLNode *pdxlnArrayCoerceExpr)
{
...
CTranslatorDXLToExpr::PexprScalarSubquery(const CDXLNode *pdxlnSubquery)
{
	...
	CScalarSubquery *popScalarSubquery = GPOS_NEW(m_mp) CScalarSubquery(
		m_mp, colref, false /*fGeneratedByExist*/, false /*fGeneratedByQuantified*/);
...
CTranslatorDXLToExpr::PexprScalarProjElem(const CDXLNode *pdxlnPrEl)
{
	GPOS_ASSERT(NULL != pdxlnPrEl &&
				EdxlopScalarProjectElem == pdxlnPrEl->GetOperator()->GetDXLOperator());
	...
	CExpression *pexprProjElem = GPOS_NEW(m_mp)
		CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectElement(m_mp, colref), pexprChild);
...
CTranslatorDXLToExpr::PexprScalarProjList(const CDXLNode *pdxlnPrL)
{
	GPOS_ASSERT(NULL != pdxlnPrL &&
				EdxlopScalarProjectList == pdxlnPrL->GetOperator()->GetDXLOperator());
	...
	else
	{
		...
		pexprProjList = GPOS_NEW(m_mp)
			CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectList(m_mp), pdrgpexprProjElems);
	}
