...
//
//---------------------------------------------------------------------------
CTranslatorExprToDXL::CTranslatorExprToDXL(IMemoryPool *mp,
									 ...
									 CMDNameArray *pdrgpmdname)
{
	...
	BOOL fDML = false;
	CDXLNode *dxlnode = CreateDXLNode(pexpr,
									  colref_array,
									  pdrgpdsBaseTables,
									  &ulNonGatherMotions,
									  &fDML,
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CDXLNode *pdxlnPrElNew =
			GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, colid, mdname));
	...
	if (0 == ulNonGatherMotions)
	{
		CDrvdPropRelational *pdprel =
			CDrvdPropRelational::GetRelationalProperties(pexpr->Pdp(DrvdPropArray::EptRelational));
		CTranslatorExprToDXLUtils::SetDirectDispatchInfo(
			m_mp, m_pmda, dxlnode, pdprel, pdrgpdsBaseTables);
	}
									...
									BOOL fRoot)
{
	...
	{
		CDXLNode *dxlnode = PdxlnTblScan(pexpr,
										 NULL /*pcrsOutput*/,
										 colref_array,
										 pdrgpdsBaseTables,
	...
	CDXLNode *dxlnode =
		(this->*pf)(pexpr, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML);
								   ...
								   CDXLPhysicalProperties *dxl_properties)
{
	...
	CDistributionSpec *pds =
		CDrvdPropPlan::Pdpplan(pexprTblScan->Pdp(DrvdPropArray::EptPlan))->Pds();
	pds->AddRef();
...
)
{
	...
	CDistributionSpec *pds =
		CDrvdPropPlan::Pdpplan(pexprIndexScan->Pdp(DrvdPropArray::EptPlan))->Pds();
	pds->AddRef();
	pdrgpdsBaseTables->Append(pds);
									 ...
									 CReqdPropPlan *prpp)
{
	...
	CDXLPhysicalIndexScan *dxl_op =
		GPOS_NEW(m_mp) CDXLPhysicalIndexScan(m_mp, table_descr, dxl_index_descr, EdxlisdForward);
	...
	CDXLNode *pdxlnIndexCondList =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarIndexCondList(m_mp));
...
#ifdef GPOS_DEBUG
	pdxlnBitmapIndexProbe->GetOperator()->AssertValid(pdxlnBitmapIndexProbe,
...
)
{
	...
	if (NULL != pexprRecheckCond)
	{
		...
		pcrsAdditional->Include(
			CDrvdPropScalar::GetDrvdScalarProps(pexprRecheckCond->PdpDerive())->PcrsUsed());
	}
										   ...
										   CDXLPhysicalProperties *dxl_properties)
{
	...
	CDXLNode *pdxlnRecheckCondFilter = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarRecheckCondFilter(m_mp), pdxlnRecheckCond);
...
#ifdef GPOS_DEBUG
	pdxlnBitmapTableScan->GetOperator()->AssertValid(pdxlnBitmapTableScan,
													 false /*validate_children*/);
#endif

	CDistributionSpec *pds =
		CDrvdPropPlan::Pdpplan(pexprBitmapTableScan->Pdp(DrvdPropArray::EptPlan))->Pds();
	pds->AddRef();
	pdrgpdsBaseTables->Append(pds);
...
)
{
	...
	return PdxlnDynamicTableScan(
		pexprDTS, colref_array, pdrgpdsBaseTables, pexprScalarCond, dxl_properties);
}
											...
											CDXLPhysicalProperties *dxl_properties)
{
	...
	if (NULL != dxl_properties)
	{
		CWStringDynamic *rows_out_str = GPOS_NEW(m_mp) CWStringDynamic(
			m_mp, dxl_properties->GetDXLOperatorCost()->GetRowsOutStr()->GetBuffer());
		CWStringDynamic *pstrCost = GPOS_NEW(m_mp) CWStringDynamic(
			m_mp, dxl_properties->GetDXLOperatorCost()->GetTotalCostStr()->GetBuffer());
	...
	CDXLPhysicalDynamicTableScan *pdxlopDTS = GPOS_NEW(m_mp) CDXLPhysicalDynamicTableScan(
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnDynamicBitmapTableScan(CExpression *pexprScan,
												  CColRefArray *colref_array,
												  CDistributionSpecArray *pdrgpdsBaseTables,
												  ULONG *,  // pulNonGatherMotions,
												  BOOL *	// pfDML
)
{
	...
	return PdxlnDynamicBitmapTableScan(
		pexprScan, colref_array, pdrgpdsBaseTables, pexprScalar, dxl_properties);
}
											...
											CReqdPropPlan *prpp)
{
	...
	CDXLNode *pdxlnDIS = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
				 GPOS_NEW(m_mp) CDXLPhysicalDynamicIndexScan(m_mp,
															 table_descr,
								  ...
								  CDXLNode *pdxlnScalar)
{
	...
	return PdxlnResult(pexprRelational,
					   colref_array,
					   pdrgpdsBaseTables,
					   pulNonGatherMotions,
					   pfDML,
								  ...
								  CDXLPhysicalProperties *dxl_properties)
{
	...
	CDXLNode *pdxlnRelationalChild = CreateDXLNode(pexprRelational,
												   colref_array,
												   pdrgpdsBaseTables,
												   pulNonGatherMotions,
												   pfDML,
	...
	return PdxlnAddScalarFilterOnRelationalChild(
		pdxlnRelationalChild, pdxlnScalar, dxl_properties, pcrsOutput, colref_array);
}

CDXLNode *
CTranslatorExprToDXL::PdxlnAddScalarFilterOnRelationalChild(CDXLNode *pdxlnRelationalChild,
															...
															CColRefArray *pdrgpcrOrder)
{
	...
	else
	{
		...
		CDXLNode *one_time_filter =
			GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarOneTimeFilter(m_mp));

		return CTranslatorExprToDXLUtils::PdxlnResult(
			m_mp, dxl_properties, pdxlnPrL, filter_dxlnode, one_time_filter, pdxlnRelationalChild);
	}
}
								  ...
								  BOOL *pfDML)
{
	...
	CDXLNode *pdxlnode = PdxlnResult(
		pexprFilter, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML, dxl_properties);
	dxl_properties->Release();
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnIndexScanWithInlinedCondition(CExpression *pexprIndexScan,
														 CExpression *pexprScalarCond,
														 CDXLPhysicalProperties *dxl_properties,
														 CColRefArray *colref_array,
														 CDistributionSpecArray *pdrgpdsBaseTables)
{
		...
		isGist)
	{
		...
		if (2 == pexprIndexScan->Arity())
		{
			...
			pexprNewScalarCond =
				CPredicateUtils::PexprConjunction(m_mp, (*pexprIndexScan)[1], pexprScalarCond);
		}
		pexprIndexLookupCond->AddRef();
		pexprIndexScan->Pop()->AddRef();
		CExpression *pexprNewIndexScan = GPOS_NEW(m_mp)
			CExpression(m_mp, pexprIndexScan->Pop(), pexprIndexLookupCond, pexprNewScalarCond);
		if (COperator::EopPhysicalIndexScan == op_id)
		{
			pdxlnIndexScan = PdxlnIndexScan(
				pexprNewIndexScan, colref_array, dxl_properties, pexprIndexScan->Prpp());
		}
		else
		{
	...
	{
		return PdxlnIndexScan(
			pexprIndexScan, colref_array, pdrgpdsBaseTables, &ulNonGatherMotions, &fDML);
	}

	return PdxlnDynamicIndexScan(
		pexprIndexScan, colref_array, pdrgpdsBaseTables, &ulNonGatherMotions, &fDML);
}
								  ...
								  CDXLPhysicalProperties *dxl_properties)
{
	...
	{
		return CreateDXLNode(pexprRelational,
							 colref_array,
							 pdrgpdsBaseTables,
							 pulNonGatherMotions,
							 pfDML,
	...
	switch (eopidRelational)
	{
		...
		case COperator::EopPhysicalExternalScan:
		{
			...
			return PdxlnTblScan(pexprRelational,
								pcrsOutput,
								NULL /* colref_array */,
								pdrgpdsBaseTables,
								pexprScalar,
		...
		case COperator::EopPhysicalBitmapTableScan:
		{
			...
			return PdxlnBitmapTableScan(pexprRelational,
										pcrsOutput,
										NULL /*colref_array*/,
										pdrgpdsBaseTables,
										pexprScalar,
		...
		case COperator::EopPhysicalDynamicTableScan:
		{
			...
			return PdxlnDynamicTableScan(
				pexprRelational, colref_array, pdrgpdsBaseTables, pexprScalar, dxl_properties);
		}
		...
		case COperator::EopPhysicalDynamicIndexScan:
		{
			...
			return PdxlnIndexScanWithInlinedCondition(
				pexprRelational, pexprScalar, dxl_properties, colref_array, pdrgpdsBaseTables);
		}
		...
		default:
		{
			return PdxlnResultFromFilter(
				pexprFilter, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML);
		}
	}
}
	...
	BOOL *pfDML)
{
	...
	return PdxlnPartitionSelector(pexprRelational,
								  colref_array,
								  pdrgpdsBaseTables,
								  pulNonGatherMotions,
								  pfDML,
											...
											BOOL *pfDML)
{
	...
	// translate relational child expression
	CDXLNode *child_dxlnode = CreateDXLNode(pexprRelational,
											NULL /* colref_array */,
											pdrgpdsBaseTables,
											pulNonGatherMotions,
											pfDML,
	...
	return CTranslatorExprToDXLUtils::PdxlnResult(
		m_mp, dxl_properties, pdxlnPrL, filter_dxlnode, one_time_filter, child_dxlnode);
}
								  ...
								  BOOL *pfDML)
{
	...
	CDXLNode *pdxlnAssert =
		GPOS_NEW(m_mp) CDXLNode(m_mp, pdxlopAssert, pdxlnPrL, pdxlnAssertPredicate, child_dxlnode);
...
)
{
	...
	CDXLNode *pdxlnCTEConsumer = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLPhysicalCTEConsumer(m_mp, ulCTEId, colids), pdxlnPrL);
								  ...
								  BOOL *pfDML)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CDXLNode *pdxlnChildProjected =
			PdxlnRemapOutputColumns(pexprChild,
									child_dxlnode,
...
//---------------------------------------------------------------------------
CColRefArray *
CTranslatorExprToDXL::PdrgpcrMerge(IMemoryPool *mp,
								   CColRefArray *pdrgpcrOrder,
...
)
{
	...
	CDXLPhysicalTVF *dxl_op =
		GPOS_NEW(m_mp) CDXLPhysicalTVF(m_mp, mdid_func, mdid_return_type, pstrFunc);
												   ...
												   CExpression *pexprScalar)
{
	...
	if (0 == ulRows)
	{
		...
		CExpression *pexprFalse =
			CUtils::PexprScalarConstBool(m_mp, false /*value*/, false /*is_null*/);
	...
	if (ulRows > 1)
	{
		...
		CColRefSet *pcrsOutput = CDrvdPropRelational::GetRelationalProperties(
									 pexprCTG->Pdp(DrvdPropArray::EptRelational))
									 ->PcrsOutput();
	...
	else
	{
		...
		pdrgpdatum->Release();
		return CTranslatorExprToDXLUtils::PdxlnResult(m_mp,
									 ...
									 BOOL *pfDML)
{
	...
	GPOS_ASSERT(COperator::EopPhysicalStreamAgg == op_id ||
				COperator::EopPhysicalHashAgg == op_id || COperator::EopPhysicalScalarAgg == op_id);
	...
	return PdxlnAggregate(pexprAgg,
						  colref_array,
						  pdrgpdsBaseTables,
						  pulNonGatherMotions,
						  pfDML,
										  ...
										  BOOL *pfDML)
{
	...
	CDXLNode *pdxlnAgg = PdxlnAggregate(pexprAgg,
										colref_array,
										pdrgpdsBaseTables,
										pulNonGatherMotions,
										pfDML,
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		if (NULL == phmululPL->Find(&colid))
		{
			CDXLNode *pdxlnProjElem =
				CTranslatorExprToDXLUtils::PdxlnProjElem(m_mp, m_phmcrdxln, pcrGroupingCol);
								...
								BOOL *pfDML)
{
	...
	CDXLNode *proj_list_dxlnode = CTranslatorExprToDXLUtils::PdxlnProjListFromChildProjList(
								 ...
								 BOOL *pfDML)
{
	...
	{
		return CreateDXLNode(pexprChild,
							 colref_array,
							 pdrgpdsBaseTables,
							 pulNonGatherMotions,
							 pfDML,
	...
	BOOL *pfDML)
{
	...
	if (EdxlSubPlanTypeScalar == dxl_subplan_type)
	{
		...
		BuildScalarSubplans(pdrgpcrInner,
							pexprInner,
							dxl_colref_array,
							pdrgpdsBaseTables,
							pulNonGatherMotions,
	...
	{
		(void) PdxlnQuantifiedSubplan(pdrgpcrInner,
									  pexprCorrelatedLOJ,
									  dxl_colref_array,
									  pdrgpdsBaseTables,
									  pulNonGatherMotions,
	...
	else
	{
		GPOS_ASSERT(EdxlSubPlanTypeExists == dxl_subplan_type ||
					EdxlSubPlanTypeNotExists == dxl_subplan_type);
		(void) PdxlnExistentialSubplan(pdrgpcrInner,
									   pexprCorrelatedLOJ,
									   dxl_colref_array,
									   pdrgpdsBaseTables,
									   pulNonGatherMotions,
	...
	BOOL *pfDML)
{
	...
	CColRefArray *pdrgpcrInner =
		CPhysicalNLJoin::PopConvert(pexprCorrelatedNLJoin->Pop())->PdrgPcrInner();
	GPOS_ASSERT(NULL != pdrgpcrInner);
	...
	switch (op_id)
	{
		case COperator::EopPhysicalCorrelatedLeftOuterNLJoin:
			BuildSubplansForCorrelatedLOJ(pexprCorrelatedNLJoin,
										  dxl_colref_array,
										  ppdxlnScalar,
										  pdrgpdsBaseTables,
										  pulNonGatherMotions,
		...
		case COperator::EopPhysicalCorrelatedNotInLeftAntiSemiNLJoin:
			pdxlnSubPlan = PdxlnQuantifiedSubplan(pdrgpcrInner,
												  pexprCorrelatedNLJoin,
												  dxl_colref_array,
												  pdrgpdsBaseTables,
												  pulNonGatherMotions,
												  pfDML);
		...
		case COperator::EopPhysicalCorrelatedLeftAntiSemiNLJoin:
			pdxlnSubPlan = PdxlnExistentialSubplan(pdrgpcrInner,
												   pexprCorrelatedNLJoin,
												   dxl_colref_array,
												   pdrgpdsBaseTables,
												   pulNonGatherMotions,
...
CTranslatorExprToDXL::PdxlnRestrictResult(CDXLNode *dxlnode, CColRef *colref)
{
	...
	if (1 < ulPrjElems)
	{
		...
		pdxlnResult->AddChild(GPOS_NEW(m_mp)
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnQuantifiedSubplan(CColRefArray *pdrgpcrInner,
											 CExpression *pexprCorrelatedNLJoin,
											 CDXLColRefArray *dxl_colref_array,
											 CDistributionSpecArray *pdrgpdsBaseTables,
											 ULONG *pulNonGatherMotions,
											 BOOL *pfDML)
{
	...
	CDXLNode *pdxlnInnerChild = CreateDXLNode(pexprInner,
											  NULL /*colref_array*/,
											  pdrgpdsBaseTables,
											  pulNonGatherMotions,
											  pfDML,
	...
	if (fCorrelatedLOJ)
	{
		...
		CColRefSet *pcrsInner = CDrvdPropRelational::GetRelationalProperties(
									pexprInner->Pdp(DrvdPropArray::EptRelational))
									->PcrsOutput();
		CColRefSet *pcrsUsed = GPOS_NEW(m_mp) CColRefSet(
			m_mp,
			*CDrvdPropScalar::GetDrvdScalarProps(pexprScalar->Pdp(DrvdPropArray::EptScalar))
				 ->PcrsUsed());
	...
	{
		GPOS_RAISE(gpopt::ExmaDXL,
				   gpopt::ExmiExpr2DXLUnsupportedFeature,
	...
	CDXLNode *pdxlnSubPlan = GPOS_NEW(m_mp) CDXLNode(
		m_mp,
		GPOS_NEW(m_mp)
			CDXLScalarSubPlan(m_mp, mdid, dxl_colref_array, dxl_subplan_type, dxlnode_test_expr));
...
//---------------------------------------------------------------------------
EdxlSubPlanType
CTranslatorExprToDXL::EdxlsubplantypeCorrelatedLOJ(CExpression *pexprCorrelatedLOJ)
{
	GPOS_ASSERT(NULL != pexprCorrelatedLOJ);
	GPOS_ASSERT(COperator::EopPhysicalCorrelatedLeftOuterNLJoin ==
				pexprCorrelatedLOJ->Pop()->Eopid());
	...
	COperator::EOperatorId eopidSubq =
		CPhysicalCorrelatedLeftOuterNLJoin::PopConvert(pexprCorrelatedLOJ->Pop())
			->EopidOriginSubq();
	switch (eopidSubq)
	{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnExistentialSubplan(CColRefArray *pdrgpcrInner,
											  CExpression *pexprCorrelatedNLJoin,
											  CDXLColRefArray *dxl_colref_array,
											  CDistributionSpecArray *pdrgpdsBaseTables,
											  ULONG *pulNonGatherMotions,
											  BOOL *pfDML)
{
	...
	CDXLNode *pdxlnSubPlan = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
				 GPOS_NEW(m_mp) CDXLScalarSubPlan(
					 m_mp, mdid, dxl_colref_array, dxl_subplan_type, NULL /*dxlnode_test_expr*/));
	pdxlnSubPlan->AddChild(inner_dxlnode);
...
//---------------------------------------------------------------------------
void
CTranslatorExprToDXL::BuildScalarSubplans(CColRefArray *pdrgpcrInner,
...
CTranslatorExprToDXL::PcrsOuterRefsForCorrelatedNLJoin(CExpression *pexpr) const
{
	...
	CDrvdPropRelational *pdprel = CDrvdPropRelational::GetRelationalProperties(
		pexprInnerChild->Pdp(DrvdPropArray::EptRelational));
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnCorrelatedNLJoin(CExpression *pexpr,
											CColRefArray *colref_array,
											CDistributionSpecArray *pdrgpdsBaseTables,
											ULONG *pulNonGatherMotions,
											BOOL *pfDML)
{
	...
	while (crsi.Advance())
	{
		...
		CDXLColRef *dxl_colref =
			GPOS_NEW(m_mp) CDXLColRef(m_mp, mdname, colref->Id(), mdid, colref->TypeModifier());
	...
	{
		BuildSubplans(
			pexpr, dxl_colref_array, &pdxlnCond, pdrgpdsBaseTables, pulNonGatherMotions, pfDML);
	}
	...
	switch (pexprOuterChild->Pop()->Eopid())
	{
		case COperator::EopPhysicalTableScan:
		{
			...
			dxlnode = PdxlnTblScanFromNLJoinOuter(pexprOuterChild,
												  pdxlnCond,
												  colref_array,
												  pdrgpdsBaseTables,
												  pulNonGatherMotions,
		...
		{
			dxl_properties->AddRef();
			dxlnode = PdxlnResultFromNLJoinOuter(pexprOuterChild,
												 pdxlnCond,
												 colref_array,
												 pdrgpdsBaseTables,
												 pulNonGatherMotions,
		...
		default:
		{
			// create a result node over outer child
			dxl_properties->AddRef();
			dxlnode = PdxlnResult(pexprOuterChild,
								  colref_array,
								  pdrgpdsBaseTables,
								  pulNonGatherMotions,
								  pfDML,
									   ...
									   CDXLColRefArray *dxl_colref_array)
{
	...
	CDXLNode *pdxlnSubPlan = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
				 GPOS_NEW(m_mp)
					 CDXLScalarSubPlan(m_mp, mdid, dxl_colref_array, EdxlSubPlanTypeScalar, NULL));
													...
													CDXLColRefArray *dxl_colref_array)
{
	...
	CDXLDatumBool *dxl_datum =
		GPOS_NEW(m_mp) CDXLDatumBool(m_mp, mdid, false /* is_null */, true /* value */);
	CDXLScalarConstValue *pdxlopConstValue = GPOS_NEW(m_mp) CDXLScalarConstValue(m_mp, dxl_datum);
									  ...
									  CDXLNode *dxlnode_right)
{
	CDXLNode *pdxlnBoolExpr =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, boolexptype));
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnTblScanFromNLJoinOuter(CExpression *pexprRelational,
												  CDXLNode *pdxlnCond,
												  CColRefArray *colref_array,
												  CDistributionSpecArray *pdrgpdsBaseTables,
												  ULONG *,  // pulNonGatherMotions,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnResultFromNLJoinOuter(CExpression *pexprRelational,
												 CDXLNode *pdxlnCond,
												 CColRefArray *colref_array,
												 CDistributionSpecArray *pdrgpdsBaseTables,
												 ULONG *pulNonGatherMotions,
												 ...
												 CDXLPhysicalProperties *dxl_properties)
{
	...
	CDXLNode *pdxlnResult = PdxlnResult(pexprRelational,
										colref_array,
										pdrgpdsBaseTables,
										pulNonGatherMotions,
										pfDML,
	...
	switch (edxlopid)
	{
		...
		case EdxlopPhysicalSequence:
		{
			...
			pdxlnResult = PdxlnAddScalarFilterOnRelationalChild(
				pdxlnResult, pdxlnCond, dxl_properties, pcrsOutput, colref_array);
		}
		break;
		default:
			pdxlnCond->Release();
								  ...
								  BOOL *pfDML)
{
	...
	CDrvdPropRelational *pdprelOuter = CDrvdPropRelational::GetRelationalProperties(
		pexprOuterChild->Pdp(DrvdPropArray::EptRelational));

	CDrvdPropRelational *pdprelInner = CDrvdPropRelational::GetRelationalProperties(
		pexprInnerChild->Pdp(DrvdPropArray::EptRelational));
	...
	CDXLNode *pdxlnOuterChild = CreateDXLNode(pexprOuterChild,
											  NULL /*colref_array*/,
											  pdrgpdsBaseTables,
											  pulNonGatherMotions,
											  pfDML,
	...
	CDXLNode *dxlnode_join_filter =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarJoinFilter(m_mp));
	if (NULL != pdxlnCond)
	{
	...
	if (is_index_nlj && GPOS_FTRACE(EopttraceIndexedNLJOuterRefAsParams))
	{
		...
		for (ULONG ul = 0; ul < outer_refs->Size(); ul++)
		{
			...
			mdid->AddRef();
			CDXLColRef *colref_dxl = GPOS_NEW(m_mp)
				CDXLColRef(m_mp, md_name, col_ref->Id(), mdid, col_ref->TypeModifier());
	...
	CDXLPhysicalNLJoin *pdxlopNLJ =
		GPOS_NEW(m_mp) CDXLPhysicalNLJoin(m_mp, join_type, is_index_nlj, nest_params_exists);
									...
									BOOL *pfDML)
{
	...
	CDXLNode *pdxlnHashCondList =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarHashCondList(m_mp));

	// output of outer side
	CColRefSet *outer_refs = CDrvdPropRelational::GetRelationalProperties(
								 pexprOuterChild->Pdp(DrvdPropArray::EptRelational))
								 ->PcrsOutput();
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		if (CPhysicalJoin::FHashJoinCompatible(pexprPred, pexprOuterChild, pexprInnerChild))
		{
			...
			CColRefSet *pcrsPredInner =
				CDrvdPropScalar::GetDrvdScalarProps(pexprPredInner->PdpDerive())->PcrsUsed();
#ifdef GPOS_DEBUG
			CColRefSet *pcrsPredOuter =
				CDrvdPropScalar::GetDrvdScalarProps(pexprPredOuter->PdpDerive())->PcrsUsed();
#endif  // GPOS_DEBUG
			...
			GPOS_ASSERT(outer_refs->ContainsAll(pcrsPredOuter) &&
						pcrsInner->ContainsAll(pcrsPredInner) &&
	...
	{
		CExpression *pexprJoinCond =
			CPredicateUtils::PexprConjunction(m_mp, pdrgpexprRemainingPredicates);
...
//---------------------------------------------------------------------------
void
CTranslatorExprToDXL::CheckValidity(CDXLPhysicalMotion *motion)
	...
	if (EdxlopPhysicalMotionGather == motion->GetDXLOperator())
	{
		if (m_pdrgpiSegments->Size() != motion->GetInputSegIdsArray()->Size())
		{
			GPOS_RAISE(gpdxl::ExmaDXL,
					   ...
					   GPOS_WSZ_LIT("GatherMotion input segments number does not match with the "
	...
	switch (pexprMotion->Pop()->Eopid())
	{
		...
		case COperator::EopPhysicalMotionRoutedDistribute:
		{
			...
			CColRef *pcrSegmentId =
				dynamic_cast<const CDistributionSpecRouted *>(popMotion->Pds())->Pcr();
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnMaterialize(CExpression *pexprSpool,
									   CColRefArray *colref_array,
									   CDistributionSpecArray *pdrgpdsBaseTables,
									   ULONG *pulNonGatherMotions,
									   BOOL *pfDML)
...
CTranslatorExprToDXL::PdxlnPartitionSelector(CExpression *pexpr,
											 ...
											 BOOL *pfDML)
{
	return PdxlnPartitionSelector(pexpr,
								  colref_array,
								  pdrgpdsBaseTables,
								  pulNonGatherMotions,
								  pfDML,
											 ...
											 CDXLPhysicalProperties *dxl_properties)
{
	...
	if (CUtils::FScalarConstTrue(pexprScalar))
	{
		return PdxlnPartitionSelectorExpand(pexpr,
											colref_array,
											pdrgpdsBaseTables,
											pulNonGatherMotions,
											pfDML,
	...
	return PdxlnPartitionSelectorFilter(pexpr,
										colref_array,
										pdrgpdsBaseTables,
										pulNonGatherMotions,
										pfDML,
												...
												BOOL *pfDML)
{
	...
	CDXLNode *pdxlnPrL =
		CTranslatorExprToDXLUtils::PdxlnPrLPartitionSelector(m_mp,
															 m_pmda,
															 m_pcf,
															 m_phmcrdxln,
	...
	TranslatePartitionFilters(
		pexpr, true /*fPassThrough*/, &pdxlnEqFilters, &pdxlnFilters, &pdxlnResidual);
	...
	CDXLNode *pdxlnPropagation =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarConstValue(m_mp, pdxldatumNull));
	...
	CDXLNode *pdxlnSelector =
		CTranslatorExprToDXLUtils::PdxlnPartitionSelector(m_mp,
														  rel_mdid,
														  popSelector->UlPartLevels(),
														  0,  // scan_id
														  GetProperties(pexpr),
...
CTranslatorExprToDXL::PdxlnPartFilterList(CExpression *pexpr, BOOL fEqFilters)
{
	...
	if (fEqFilters)
	{
		pdxlnFilters = GPOS_NEW(m_mp) CDXLNode(
			m_mp, GPOS_NEW(m_mp) CDXLScalarOpList(m_mp, CDXLScalarOpList::EdxloplistEqFilterList));
	}
	else
	{
		pdxlnFilters = GPOS_NEW(m_mp) CDXLNode(
			m_mp, GPOS_NEW(m_mp) CDXLScalarOpList(m_mp, CDXLScalarOpList::EdxloplistFilterList));
	}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnPartitionSelectorExpand(CExpression *pexpr,
												   CColRefArray *colref_array,
												   CDistributionSpecArray *pdrgpdsBaseTables,
												   ULONG *pulNonGatherMotions,
												   BOOL *pfDML,
												   ...
												   CDXLPhysicalProperties *dxl_properties)
{
	...
	GPOS_ASSERT_IMP(NULL != pexprScalarCond,
					(COperator::EopPhysicalDynamicTableScan == pexprChild->Pop()->Eopid() ||
					 ...
					 COperator::EopPhysicalDynamicBitmapTableScan == pexprChild->Pop()->Eopid()) &&
						"Inlining predicates only allowed in DynamicTableScan, DynamicIndexScan "
						"and DynamicBitmapTableScan");

	CPhysicalPartitionSelector *popSelector = CPhysicalPartitionSelector::PopConvert(pexpr->Pop());
	const ULONG ulLevels = popSelector->UlPartLevels();

	// translate child
	CDXLNode *child_dxlnode = PdxlnPartitionSelectorChild(pexprChild,
														  pexprScalarCond,
														  dxl_properties,
														  colref_array,
														  pdrgpdsBaseTables,
	...
	CDXLNode *pdxlnPropagation =
		CTranslatorExprToDXLUtils::PdxlnPropExprPartitionSelector(m_mp,
																  m_pmda,
																  m_pcf,
	...
	CDXLNode *pdxlnPrLSequence = CTranslatorExprToDXLUtils::PdxlnProjListFromChildProjList(
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnPartitionSelectorFilter(CExpression *pexpr,
												   CColRefArray *colref_array,
												   CDistributionSpecArray *pdrgpdsBaseTables,
												   ULONG *pulNonGatherMotions,
												   BOOL *pfDML,
												   ...
												   CDXLPhysicalProperties *dxl_properties)
{
	...
	BOOL fPassThrough =
		FEqPartFiltersAllLevels(pexpr, false /*fCheckGeneralFilters*/) && !fPartialScans;
#ifdef GPOS_DEBUG
	...
	CDrvdPropRelational *pdprel =
		CDrvdPropRelational::GetRelationalProperties(pexprChild->Pdp(DrvdPropArray::EptRelational));
...
//---------------------------------------------------------------------------
BOOL
CTranslatorExprToDXL::FEqPartFiltersAllLevels(CExpression *pexpr, BOOL fCheckGeneralFilters)
{
	GPOS_ASSERT(NULL != pexpr);
	CPhysicalPartitionSelector *popSelector = CPhysicalPartitionSelector::PopConvert(pexpr->Pop());
	const ULONG ulPartLevels = popSelector->UlPartLevels();
	...
	for (ULONG ul = 0; ul < ulPartLevels; ul++)
	{
		...
		if (NULL == pexprEqFilter && NULL != pexprFilter)
		{
			if (!fCheckGeneralFilters ||
				!CPredicateUtils::FConjunctionOfEqComparisons(m_mp, pexprFilter))
			{
				return false;
			}
		}
...
//---------------------------------------------------------------------------
void
CTranslatorExprToDXL::ConstructLevelFilters4PartitionSelector(CExpression *pexprPartSelector,
															  ...
															  CDXLNode **ppdxlnFilters)
{
	...
	*ppdxlnFilters = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLScalarOpList(m_mp, CDXLScalarOpList::EdxloplistFilterList));
	*ppdxlnEqFilters = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLScalarOpList(m_mp, CDXLScalarOpList::EdxloplistEqFilterList));
	...
	for (ULONG ulLevel = 0; ulLevel < ulPartLevels; ulLevel++)
	{
		...
		if (NULL != pexprEqFilter)
		{
			...
			{
				filter_dxlnode =
					CTranslatorExprToDXLUtils::PdxlnRangeFilterEqCmp(m_mp,
																	 m_pmda,
																	 pdxlnEq,
																	 pmdidTypePartKey,
																	 pmdidTypeOther,
			...
			else  // list partition
			{
				...
				filter_dxlnode = CTranslatorExprToDXLUtils::PdxlnListFilterScCmp(m_mp,
																				 m_pmda,
																				 pdxlnPartKey,
																				 pdxlnEq,
																				 pmdidTypePartKey,
		...
		if (NULL != pexprFilter)
		{
			...
			for (ULONG ul = 0; ul < length; ul++)
			{
				CDXLNode *pdxlnScCmp = PdxlnPredOnPartKey((*pdrgpexprConjuncts)[ul],
														  pcrPartKey,
														  pmdidTypePartKey,
														  ulLevel,
														  fRangePart,
		...
		if (NULL != filter_dxlnode && fRangePart)
		{
			CDXLNode *pdxlnDefaultAndOpenEnded =
				CTranslatorExprToDXLUtils::PdxlnRangeFilterDefaultAndOpenEnded(
					m_mp, ulLevel, fLTComparison, fGTComparison, fEQComparison, fDefaultPartition);
		...
		(*ppdxlnEqFilters)
			->AddChild(CTranslatorExprToDXLUtils::PdxlnBoolConst(m_mp, m_pmda, true /*value*/));
	}
...
)
{
	...
	{
		return PdxlnScCmpPartKey(pexprPred,
								 pcrPartKey,
								 pmdidTypePartKey,
								 ulPartLevel,
								 fRangePart,
	...
	if (CUtils::FScalarNullTest(pexprPred))
	{
		...
		GPOS_ASSERT(CUtils::FScalarIdent(pexprChild, pcrPartKey) ||
	...
	{
		return PdxlArrayExprOnPartKey(pexprPred,
									  pcrPartKey,
									  pmdidTypePartKey,
									  ulPartLevel,
									  fRangePart,
	...
	return PdxlnConjDisjOnPartKey(pexprPred,
								  pcrPartKey,
								  pmdidTypePartKey,
								  ulPartLevel,
								  fRangePart,
...
)
{
	...
	CConstraintInterval *pci =
		CConstraintInterval::PcnstrIntervalFromScalarArrayCmp(m_mp, pexprPred, pcrPartKey);
	...
	CDXLNode *dxlnode = PdxlnConjDisjOnPartKey(pexprDisj,
											   pcrPartKey,
											   pmdidTypePartKey,
											   ulPartLevel,
											   fRangePart,
...
)
{
	...
	for (ULONG ul = 0; ul < ulChildren; ul++)
	{
		...
		CDXLNode *child_dxlnode = PdxlnPredOnPartKey(pexprChild,
													 pcrPartKey,
													 pmdidTypePartKey,
													 ulPartLevel,
													 fRangePart,
...
)
{
	...
	CPredicateUtils::ExtractComponents(
		pexprScCmp, pcrPartKey, &pexprPartKey, &pexprOther, &cmp_type);

	*pfLTComparison =
		*pfLTComparison || (IMDType::EcmptL == cmp_type) || (IMDType::EcmptLEq == cmp_type);
	*pfGTComparison =
		*pfGTComparison || (IMDType::EcmptG == cmp_type) || (IMDType::EcmptGEq == cmp_type);
	*pfEQComparison = *pfEQComparison || IMDType::EcmptEq == cmp_type;
	...
	if (fRangePart)  // range partition
	{
		...
		return CTranslatorExprToDXLUtils::PdxlnRangeFilterScCmp(m_mp,
																m_pmda,
																pdxlnOther,
																pmdidTypePartKey,
																pmdidTypeOther,
	...
	else  // list partition
	{
		...
		return CTranslatorExprToDXLUtils::PdxlnListFilterScCmp(m_mp,
															   m_pmda,
															   pdxlnPartKeyExpr,
															   pdxlnOther,
											 ...
											 BOOL is_null)
{
	if (!fRangePart)  // list partition
	{
		CDXLNode *pdxlnPartListNullTest = GPOS_NEW(m_mp)
			CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarPartListNullTest(m_mp, ulPartLevel, is_null));
		CDXLNode *pdxlnDefault =
			GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarPartDefault(m_mp, ulPartLevel));
		return GPOS_NEW(m_mp) CDXLNode(m_mp,
									   GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, Edxlor),
									   pdxlnPartListNullTest,
									   pdxlnDefault);
	}
	...
	CDXLNode *pdxlnPredicateMin = GPOS_NEW(m_mp) CDXLNode(
		...
		GPOS_NEW(m_mp) CDXLNode(m_mp,
								GPOS_NEW(m_mp) CDXLScalarPartBound(
									m_mp, ulPartLevel, pmdidTypePartKey, true /*is_lower_bound*/)));

	pmdidTypePartKey->AddRef();
	CDXLNode *pdxlnPredicateMax = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
				 GPOS_NEW(m_mp) CDXLScalarNullTest(m_mp, is_null),
				 GPOS_NEW(m_mp)
					 CDXLNode(m_mp,
							  GPOS_NEW(m_mp) CDXLScalarPartBound(
								  m_mp, ulPartLevel, pmdidTypePartKey, false /*is_lower_bound*/)));

	// construct the conjunction of the predicate for the lower and upper bounds
	CDXLNode *pdxlnNullTests =
		GPOS_NEW(m_mp) CDXLNode(m_mp,
								GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, Edxland),
								pdxlnPredicateMin,
								pdxlnPredicateMax);

	// AND that with the following: !(default || min_open || max_open)
	CDXLNode *pdxlnDefaultOrOpenEnded =
		CTranslatorExprToDXLUtils::PdxlnRangeFilterDefaultAndOpenEnded(m_mp,
																	   ulPartLevel,
																	   true,   //fLTComparison
																	   true,   //fGTComparison
																	   false,  //fEQComparison
	...
	CDXLNode *pdxlnNotDefaultOrOpenEnded = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, Edxlnot), pdxlnDefaultOrOpenEnded);

	return GPOS_NEW(m_mp) CDXLNode(m_mp,
								   GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, Edxland),
								   pdxlnNotDefaultOrOpenEnded,
								   pdxlnNullTests);
}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnPartitionSelectorChild(CExpression *pexprChild,
												  CExpression *pexprScalarCond,
												  CDXLPhysicalProperties *dxl_properties,
												  CColRefArray *colref_array,
												  CDistributionSpecArray *pdrgpdsBaseTables,
												  ...
												  BOOL *pfDML)
{
	...
	if (NULL == pexprScalarCond)
	{
		return CreateDXLNode(
			pexprChild, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML, true, false);
	}
	...
	switch (pexprChild->Pop()->Eopid())
	{
		case COperator::EopPhysicalDynamicTableScan:
			return PdxlnDynamicTableScan(
				pexprChild, colref_array, pdrgpdsBaseTables, pexprScalarCond, dxl_properties);
		case COperator::EopPhysicalDynamicIndexScan:
			return PdxlnIndexScanWithInlinedCondition(
				pexprChild, pexprScalarCond, dxl_properties, colref_array, pdrgpdsBaseTables);
		default:
							   ...
							   BOOL *pfDML)
{
	...
	*pfDML = false;
	...
	CDXLPhysicalDML *pdxlopDML = GPOS_NEW(m_mp) CDXLPhysicalDML(m_mp,
																dxl_dml_type,
																table_descr,
																pdrgpul,
																action_colid,
								...
								BOOL *pfDML)
{
	...
	for (ULONG ul = 0; ul < ulColumns; ul++)
	{
		...
		CDXLColDescr *pdxlcd = GPOS_NEW(m_mp) CDXLColDescr(m_mp,
														   pmdnameCol,
														   colref->Id(),
														   pcd->AttrNum(),
														   pmdidColType,
...
CTranslatorExprToDXL::GetDXLDirectDispatchInfo(CExpression *pexprDML)
{
	...
	CPropConstraint *ppc = CDrvdPropRelational::GetRelationalProperties(
							   (*pexprDML)[0]->Pdp(DrvdPropArray::EptRelational))
							   ->Ppc();
								 ...
								 BOOL *pfDML)
{
	...
	CDXLPhysicalSplit *pdxlopSplit = GPOS_NEW(m_mp) CDXLPhysicalSplit(m_mp,
																	  delete_colid_array,
																	  insert_colid_array,
																	  action_colid,
																	  ctid_colid,
									  ...
									  BOOL *pfDML)
{
	...
	CDXLPhysicalRowTrigger *pdxlopRowTrigger =
		GPOS_NEW(m_mp) CDXLPhysicalRowTrigger(m_mp, rel_mdid, type, colids_old, colids_new);
...
//---------------------------------------------------------------------------
EdxlDmlType
CTranslatorExprToDXL::Edxldmloptype(const CLogicalDML::EDMLOperator edmlop) const
{
	switch (edmlop)
	{
...
CTranslatorExprToDXL::PdxlnScCmp(CExpression *pexprScCmp)
{
	...
	CDXLNode *pdxlnCmp =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarComp(m_mp, mdid, str_name));
...
CTranslatorExprToDXL::PdxlnScDistinctCmp(CExpression *pexprScDist)
{
	...
	CDXLNode *pdxlnDistCmp =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarDistinctComp(m_mp, mdid));
...
CTranslatorExprToDXL::PdxlnScOp(CExpression *pexprOp)
{
	...
	CDXLNode *pdxlnOpExpr = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarOpExpr(m_mp, mdid_op, return_type_mdid, str_name));
...
CTranslatorExprToDXL::PdxlnScBoolExpr(CExpression *pexprScBoolOp)
{
	...
	CDXLNode *pdxlnBoolExpr =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, edxlbooltype));
...
//---------------------------------------------------------------------------
EdxlBoolExprType
CTranslatorExprToDXL::Edxlbooltype(const CScalarBoolOp::EBoolOperator eboolop) const
{
	switch (eboolop)
	{
...
CTranslatorExprToDXL::PdxlnScFuncExpr(CExpression *pexprFunc)
{
	...
	CDXLNode *pdxlnFuncExpr = GPOS_NEW(m_mp) CDXLNode(
		...
		GPOS_NEW(m_mp) CDXLScalarFuncExpr(
			m_mp, mdid_func, mdid_return_type, popScFunc->TypeModifier(), pmdfunc->ReturnsSet()));
...
CTranslatorExprToDXL::PdxlnScAggref(CExpression *pexprAggFunc)
{
	...
	CDXLScalarAggref *pdxlopAggRef = GPOS_NEW(m_mp) CDXLScalarAggref(
		m_mp, pmdidAggFunc, resolved_rettype, popScAggFunc->IsDistinct(), edxlaggrefstage);
...
CTranslatorExprToDXL::PdxlnScIfStmt(CExpression *pexprIfStmt)
{
	...
	CDXLNode *pdxlnIfStmt =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarIfStmt(m_mp, mdid_type));
...
CTranslatorExprToDXL::PdxlnScSwitch(CExpression *pexprSwitch)
{
	...
	CDXLNode *dxlnode =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarSwitch(m_mp, mdid_type));
...
CTranslatorExprToDXL::PdxlnScNullTest(CExpression *pexprNullTest)
{
	...
	CDXLNode *pdxlnNullTest =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarNullTest(m_mp, true /* is_null */));
...
CTranslatorExprToDXL::PdxlnScCoalesce(CExpression *pexprCoalesce)
{
	...
	CDXLNode *dxlnode =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarCoalesce(m_mp, mdid_type));
...
CTranslatorExprToDXL::PdxlnScMinMax(CExpression *pexprMinMax)
{
	...
	CDXLNode *dxlnode = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarMinMax(m_mp, mdid_type, min_max_type));
...
//---------------------------------------------------------------------------
void
CTranslatorExprToDXL::TranslateScalarChildren(CExpression *pexpr, CDXLNode *dxlnode)
{
...
CTranslatorExprToDXL::PdxlnScCast(CExpression *pexprCast)
{
	...
	CDXLNode *pdxlnCast =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarCast(m_mp, mdid, mdid_func));
...
CTranslatorExprToDXL::PdxlnScCoerceToDomain(CExpression *pexprCoerce)
{
	...
	CDXLNode *pdxlnCoerce = GPOS_NEW(m_mp) CDXLNode(
		...
		GPOS_NEW(m_mp) CDXLScalarCoerceToDomain(
			...
			(EdxlCoercionForm)
				popScCoerce->Ecf(),  // map Coercion Form directly based on position in enum
	...
	CDXLNode *pdxlnCoerce = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
				 GPOS_NEW(m_mp) CDXLScalarCoerceViaIO(
					 ...
					 (EdxlCoercionForm)
						 popScCerce->Ecf(),  // map Coercion Form directly based on position in enum
					 popScCerce->Location()));
...
CTranslatorExprToDXL::PdxlnScArrayCoerceExpr(CExpression *pexprArrayCoerceExpr)
{
	...
	CDXLNode *pdxlnArrayCoerceExpr = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
...
CTranslatorExprToDXL::GetWindowFrame(CWindowFrame *pwf)
{
	...
	CDXLNode *pdxlnLeading = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, true /* fLeading */, edxlfbLeading));
	if (NULL != pwf->PexprLeading())
	{
		pdxlnLeading->AddChild(PdxlnScalar(pwf->PexprLeading()));
	}

	CDXLNode *pdxlnTrailing = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, false /* fLeading */, edxlfbTrailing));
	...
	return GPOS_NEW(m_mp)
		CDXLWindowFrame(m_mp, edxlfs, frame_exc_strategy, pdxlnLeading, pdxlnTrailing);
}
								  ...
								  BOOL *pfDML)
{
	...
	CDXLNode *child_dxlnode = CreateDXLNode((*pexprSeqPrj)[0],
											NULL /* colref_array */,
											pdrgpdsBaseTables,
											pulNonGatherMotions,
											pfDML,
...
CTranslatorExprToDXL::PdxlnArray(CExpression *pexpr)
{
	...
	CDXLNode *pdxlnArray = GPOS_NEW(m_mp) CDXLNode(
		m_mp,
		GPOS_NEW(m_mp)
			CDXLScalarArray(m_mp, elem_type_mdid, array_type_mdid, pop->FMultiDimensional()));
	...
	CDXLNode *pdxlnArrayref = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
				 GPOS_NEW(m_mp) CDXLScalarArrayRef(
					 m_mp, elem_type_mdid, pop->TypeModifier(), array_type_mdid, return_type_mdid));
...
CTranslatorExprToDXL::PdxlnArrayRefIndexList(CExpression *pexpr)
{
	...
	CDXLNode *pdxlnIndexlist = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarArrayRefIndexList(m_mp, Eilb(pop->Eilt())));
...
CTranslatorExprToDXL::PdxlnAssertConstraint(CExpression *pexpr)
{
	...
	CDXLNode *pdxlnAssertConstraint = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarAssertConstraint(m_mp, pstrErrorMsg));
										...
										const CColRefArray *pdrgpcrOutput)
{
	...
	CDXLTableDescr *table_descr =
		GPOS_NEW(m_mp) CDXLTableDescr(m_mp, mdid, pmdnameTbl, ptabdesc->GetExecuteAsUserId());
...
CTranslatorExprToDXL::GetProperties(const CExpression *pexpr)
{
	...
	if (CDistributionSpec::EdtReplicated ==
		CDrvdPropPlan::Pdpplan(pexpr->Pdp(DrvdPropArray::EptPlan))->Pds()->Edt())
	{
	...
	CDXLOperatorCost *cost =
		GPOS_NEW(m_mp) CDXLOperatorCost(pstrStartupcost, pstrTotalcost, rows_out_str, width_str);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnProjList(const CColRefSet *pcrsOutput, CColRefArray *colref_array)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnProjListFromConstTableGet(CColRefArray *pdrgpcrReqOutput,
													 ...
													 IDatumArray *pdrgpdatumValues)
{
	...
	if (NULL != pdrgpcrReqOutput)
	{
		...
		proj_list_dxlnode =
			PdxlnProjListFromConstTableGet(NULL, pdrgpcrReqOutput, pdrgpdatumOrdered);
...
CTranslatorExprToDXL::PdxlnProjElem(const CColRef *colref, CDXLNode *pdxlnValue)
{
	...
	CMDName *mdname = GPOS_NEW(m_mp) CMDName(m_mp, colref->Name().Pstr());
	CDXLNode *pdxlnPrEl = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, colref->Id(), mdname));
...
CTranslatorExprToDXL::GetSortColListDXL(const COrderSpec *pos)
{
	...
	for (ULONG ul = 0; ul < pos->UlSortColumns(); ul++)
	{
		...
		CDXLScalarSortCol *pdxlopSortCol = GPOS_NEW(m_mp)
			CDXLScalarSortCol(m_mp, colref->Id(), sort_op_id, sort_op_name, fSortNullsFirst);
...
CTranslatorExprToDXL::PdxlnHashExprList(const CExpressionArray *pdrgpexpr)
{
	...
	for (ULONG ul = 0; ul < pdrgpexpr->Size(); ul++)
	{
		...
		CDXLNode *pdxlnHashExpr =
			GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarHashExpr(m_mp, mdid_type));
...
//---------------------------------------------------------------------------
ULONG
CTranslatorExprToDXL::UlPosInArray(const CColRef *colref, const CColRefArray *colref_array) const
{
								  ...
								  CDXLNode *child_dxlnode)
{
	...
	CDXLNode *pdxlnScalarOneTimeFilter =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarOneTimeFilter(m_mp));
	...
	return CTranslatorExprToDXLUtils::PdxlnResult(m_mp,
												  ...
												  pdxlnMaterialize ? pdxlnMaterialize
																   : child_dxlnode);
}

CDXLNode *
CTranslatorExprToDXL::PdxlnMaterialize(CDXLNode *dxlnode  // node that needs to be materialized
)
{
	GPOS_ASSERT(NULL != dxlnode);
												   ...
												   CDXLNode *child_dxlnode)
{
	...
	BOOL fPrjListContainsSubplan =
		CTranslatorExprToDXLUtils::FProjListContainsSubplanWithBroadCast(proj_list_dxlnode);
	...
	if (fPrjListContainsSubplan)
	{
		...
		if (pbsScIdentColIds->Size() > 0)
		{
			...
			fMotionHazard = CTranslatorExprToDXLUtils::FMotionHazard(
				m_mp, child_dxlnode, rgeopid, GPOS_ARRAY_SIZE(rgeopid), pbsScIdentColIds);
		}
