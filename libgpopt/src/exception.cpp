...
//---------------------------------------------------------------------------
GPOS_RESULT
gpopt::EresExceptionInit(IMemoryPool *mp)
{
	//---------------------------------------------------------------------------
	...
	CMessage rgmsg[ExmiSentinel] = {
		...
		CMessage(CException(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedCompositePartKey),
				 CException::ExsevNotice,
				 GPOS_WSZ_WSZLEN("Feature not supported by the Pivotal Query Optimizer: composite "
								 "partitioning keys"),
				 0,
				 GPOS_WSZ_WSZLEN("Feature not supported by the Pivotal Query Optimizer: composite "
								 "partitioning keys")),
		...
		CMessage(
			CException(gpopt::ExmaGPOPT, gpopt::ExmiCTEProducerConsumerMisAligned),
			CException::ExsevError,
