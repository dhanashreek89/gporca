...
namespace gpnaucrates
{
	...
	private:
		...
		static void StatsFilterInt4(IMemoryPool *mp,
									ULONG colid,
									INT iLower,
									INT iUpper,
		...
		static void StatsFilterBool(IMemoryPool *mp,
									ULONG colid,
									BOOL fValue,
									CStatsPredPtrArry *pgrgpstatspred);

		// create filter on numeric types
		static void StatsFilterNumeric(IMemoryPool *mp,
									   ULONG colid,
		...
		static void StatsFilterGeneric(IMemoryPool *mp,
									   ULONG colid,
									   OID oid,
									   CWStringDynamic *pstrLowerEncoded,
									   CWStringDynamic *pstrUpperEncoded,
		...
		static CHistogram *PhistExampleInt4Dim(IMemoryPool *mp);

		// helper function that generates an array of ULONG pointers
		static ULongPtrArray *
		Pdrgpul(IMemoryPool *mp, ULONG ul1, ULONG ul2 = gpos::ulong_max)
		{
		...
		static CTableDescriptor *PtabdescTwoColumnSource(IMemoryPool *mp,
	...
	public:
		// example filter
		static CStatsPredPtrArry *Pdrgpstatspred1(IMemoryPool *mp);

		static CStatsPredPtrArry *Pdrgpstatspred2(IMemoryPool *mp);
