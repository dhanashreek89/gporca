...
namespace gpnaucrates
{
	...
	private:
		...
		static GPOS_RESULT EresUnittest_CStatistics(SStatsFilterSTestCase rgstatsdisjtc[],
													ULONG ulTestCases);

		// disjunction filters
		static CStatsPred *PstatspredDisj1(IMemoryPool *mp);

		// disjunction filters
		static CStatsPred *PstatspredDisj2(IMemoryPool *mp);

		// disjunction filters
		static CStatsPred *PstatspredDisj3(IMemoryPool *mp);

		// disjunction filters
		static CStatsPred *PstatspredDisj4(IMemoryPool *mp);

		// disjunction filters
		static CStatsPred *PstatspredDisj5(IMemoryPool *mp);

		// disjunction filters
		static CStatsPred *PstatspredDisj6(IMemoryPool *mp);

		// disjunction filters
		static CStatsPred *PstatspredDisj7(IMemoryPool *mp);

		// disjunction filters
		static CStatsPred *PstatspredDisj8(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredNestedPredDiffCol1(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredNestedPredDiffCol2(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredNestedPredCommonCol1(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredNestedPredCommonCol2(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredNestedSharedCol(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredDisjOverConjSameCol1(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredDisjOverConjSameCol2(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredDisjOverConjSameCol3(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredDisjOverConjSameCol4(IMemoryPool *mp);

		// nested AND and OR predicates
		static CStatsPred *PstatspredDisjOverConjDifferentCol1(IMemoryPool *mp);

		static CStatsPred *PstatspredDisjOverConjMultipleIdenticalCols(IMemoryPool *mp);

		// conjunctive predicates
		static CStatsPred *PstatspredConj(IMemoryPool *mp);

		// generate an array of filter given a column identifier, comparison type, and array of integer point
		static CStatsPredPtrArry *PdrgpstatspredInteger(IMemoryPool *mp,
														ULONG colid,
		...
		static CStatsPredPtrArry *PdrgppredfilterNumeric(IMemoryPool *mp,
														 ULONG colid,
														 SStatsCmpValElem statsCmpValElem);

		// create a filter on a column with null values
		static CStatsPred *PstatspredNullableCols(IMemoryPool *mp);

		// create a point filter where the constant is null
		static CStatsPred *PstatspredWithNullConstant(IMemoryPool *mp);

		// create a 'is not null' point filter
		static CStatsPred *PstatspredNotNull(IMemoryPool *mp);

		// compare the derived statistics with the statistics in the outputfile
		static GPOS_RESULT EresUnittest_CStatisticsCompare(IMemoryPool *mp,
