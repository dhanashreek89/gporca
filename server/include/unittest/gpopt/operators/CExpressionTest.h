...
namespace gpopt
{
	...
	{
	private:
		static CReqdPropPlan *PrppCreateRequiredProperties(IMemoryPool *mp, CColRefSet *pcrs);

		static CExpression *PexprCreateGbyWithColumnFormat(IMemoryPool *mp,
		...
		static GPOS_RESULT EresCheckCachedReqdCols(IMemoryPool *mp,
												   CExpression *pexpr,
												   CReqdPropPlan *prppInput);

		// helper function for the FValidPlan tests
		static void SetupPlanForFValidPlanTest(IMemoryPool *mp,
		...
		static CExpression *PexprComplexJoinTree(IMemoryPool *mp);
