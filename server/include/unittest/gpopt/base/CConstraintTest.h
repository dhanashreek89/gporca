...
namespace gpopt
{
	...
	private:
		...
		static CRangeArray *Pdrgprng(IMemoryPool *mp,
									 IMDId *mdid,
									 const SRangeInfo rgRangeInfo[],
									 ULONG ulRanges);

		static CConstraintInterval *PciFirstInterval(IMemoryPool *mp, IMDId *mdid, CColRef *colref);

		static CConstraintInterval *PciSecondInterval(IMemoryPool *mp,
													  IMDId *mdid,
													  CColRef *colref);

		// interval from scalar comparison
		static GPOS_RESULT EresUnittest_CIntervalFromScalarCmp(IMemoryPool *mp,
		...
		static GPOS_RESULT EresUnittest_CIntervalFromScalarBoolOp(IMemoryPool *mp,
																  CMDAccessor *md_accessor,
																  CColRef *colref);
		...
		// build a conjunction
		static CConstraintConjunction *Pcstconjunction(IMemoryPool *mp,
													   IMDId *mdid,
													   CColRef *colref);

		// build a disjunction
		static CConstraintDisjunction *Pcstdisjunction(IMemoryPool *mp,
													   IMDId *mdid,
													   CColRef *colref);
	...
	public:
		...
		static void PrintEquivClasses(IMemoryPool *mp,
