...
CSystemId CTestUtils::m_sysidDefault(IMDId::EmdidGPDB, GPOS_WSZ_STR_LENGTH("GPDB"));
...
// local memory pool
IMemoryPool *CTestUtils::m_mp = NULL;
...
//---------------------------------------------------------------------------
void
CTestUtils::InitProviderFile(IMemoryPool *mp)
{
...
CTableDescriptor *
CTestUtils::PtabdescPlainWithColNameFormat(IMemoryPool *mp,
										   ULONG num_cols,
										   IMDId *mdid,
...
)
{
	...
	CTableDescriptor *ptabdesc = GPOS_NEW(mp) CTableDescriptor(mp,
															   mdid,
															   nameTable,
	...
	for (ULONG i = 0; i < num_cols; i++)
	{
		...
		CColumnDescriptor *pcoldescInt = GPOS_NEW(mp) CColumnDescriptor(
			mp, pmdtypeint4, default_type_modifier, nameColumnInt, i + 1, is_nullable);
...
//---------------------------------------------------------------------------
CTableDescriptor *
CTestUtils::PtabdescPlain(IMemoryPool *mp,
						  ULONG num_cols,
						  IMDId *mdid,
						  const CName &nameTable,
						  BOOL is_nullable  // define nullable columns
...
CTableDescriptor *
CTestUtils::PtabdescCreate(
	IMemoryPool *mp, ULONG num_cols, IMDId *mdid, const CName &nameTable, BOOL fPartitioned)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGet(IMemoryPool *mp,
							...
							const CWStringConst *pstrTableAlias)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalGet(mp, GPOS_NEW(mp) CName(mp, CName(pstrTableAlias)), ptabdesc));
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGetNullable(IMemoryPool *mp,
									OID oidTable,
									...
									const CWStringConst *pstrTableAlias)
{
	...
	CTableDescriptor *ptabdesc =
		CTestUtils::PtabdescPlain(mp, 3, mdid, CName(&strName), true /*is_nullable*/);
							...
							ULONG ulTableId)
{
	CTableDescriptor *ptabdesc = PtabdescCreate(
		mp, GPOPT_TEST_REL_WIDTH, GPOS_NEW(mp) CMDIdGPDB(ulTableId, 1, 1), CName(str_table_name));
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalExternalGet(IMemoryPool *mp)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalExternalGet(mp, GPOS_NEW(mp) CName(mp, &strAlias), ptabdesc));
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGetPartitioned(IMemoryPool *mp)
{
	...
	CTableDescriptor *ptabdesc =
		PtabdescCreate(mp, ulAttributes, mdid, CName(&strName), true /*fPartitioned*/);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalDynamicGetWithIndexes(IMemoryPool *mp)
{
	ULONG ulAttributes = 2;
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelect(IMemoryPool *mp, CExpression *pexpr)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelect(IMemoryPool *mp,
							   CWStringConst *str_table_name,
							   CWStringConst *pstrTableAlias,
							   ULONG ulTableId)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithContradiction(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectPartitioned(IMemoryPool *mp)
{
	...
	CExpression *pexprScalar = CUtils::PexprScalarEqCmp(
		mp, CUtils::PexprScalarIdent(mp, pcrPartKey), CUtils::PexprScalarConstInt4(mp, val));
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalAssert(IMemoryPool *mp)
{
	CExpression *pexprGet = PexprLogicalGet(mp);

	// get any two columns
	CColRefSet *pcrs =
		CDrvdPropRelational::GetRelationalProperties(pexprGet->PdpDerive())->PcrsOutput();
	CColRef *pcrLeft = pcrs->PcrAny();
	CColRef *pcrRight = pcrs->PcrAny();
	CExpression *pexprPredicate = CUtils::PexprScalarEqCmp(mp, pcrLeft, pcrRight);

	CWStringConst *pstrErrMsg = CXformUtils::PstrErrorMessage(
		mp, gpos::CException::ExmaSQL, gpos::CException::ExmiSQLTest, GPOS_WSZ_LIT("Test msg"));
	CExpression *pexprAssertConstraint = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CScalarAssertConstraint(mp, pstrErrMsg), pexprPredicate);
	...
	CLogicalAssert *popAssert = GPOS_NEW(mp) CLogicalAssert(
		mp, GPOS_NEW(mp) CException(gpos::CException::ExmaSQL, gpos::CException::ExmiSQLTest));
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::Pexpr3WayJoinPartitioned(IMemoryPool *mp)
{
	return PexprLogicalJoin<CLogicalInnerJoin>(
		mp, PexprLogicalGet(mp), PexprJoinPartitionedOuter<CLogicalInnerJoin>(mp));
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::Pexpr4WayJoinPartitioned(IMemoryPool *mp)
{
	return PexprLogicalJoin<CLogicalInnerJoin>(
		mp, PexprLogicalGet(mp), Pexpr3WayJoinPartitioned(mp));
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithNestedAnd(IMemoryPool *mp)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithNestedOr(IMemoryPool *mp)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithEvenNestedNot(IMemoryPool *mp)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprScIdentCmpScIdent(IMemoryPool *mp,
								   ...
								   IMDType::ECmpType cmp_type)
{
	...
	CColRefSet *pcrsLeft =
		CDrvdPropRelational::GetRelationalProperties(pexprLeft->PdpDerive())->PcrsOutput();
	CColRef *pcrLeft = pcrsLeft->PcrAny();

	CColRefSet *pcrsRight =
		CDrvdPropRelational::GetRelationalProperties(pexprRight->PdpDerive())->PcrsOutput();
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprScIdentCmpConst(IMemoryPool *mp,
								 CExpression *pexpr,
								 IMDType::ECmpType cmp_type,
								 ULONG ulVal)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectCmpToConst(IMemoryPool *mp)
{
	// generate a get expression
	CExpression *pexpr = PexprLogicalGet(mp);
	CExpression *pexprPred =
		PexprScIdentCmpConst(mp, pexpr, IMDType::EcmptEq /* cmp_type */, 10 /* ulVal */);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectArrayCmp(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithOddNestedNot(IMemoryPool *mp)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithNestedAndOrNot(IMemoryPool *mp)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSubqueryWithConstTableGet(IMemoryPool *mp, COperator::EOperatorId op_id)
{
	GPOS_ASSERT(NULL != mp);
	...
	CColRefSet *pcrs =
		CDrvdPropRelational::GetRelationalProperties(pexprConstTableGet->PdpDerive())->PcrsOutput();
	...
	if (COperator::EopScalarSubqueryAny == op_id)
	{
		...
		pexprSubquery = GPOS_NEW(mp) CExpression(
			mp,
			GPOS_NEW(mp)
				CScalarSubqueryAny(mp, GPOS_NEW(mp) CMDIdGPDB(GPDB_INT4_EQ_OP), str, pcrInner),
			pexprConstTableGet,
	...
	else
	{
		...
		pexprSubquery = GPOS_NEW(mp) CExpression(
			mp,
			GPOS_NEW(mp)
				CScalarSubqueryAll(mp, GPOS_NEW(mp) CMDIdGPDB(GPDB_INT4_EQ_OP), str, pcrInner),
			pexprConstTableGet,
			CUtils::PexprScalarIdent(mp, pcrOuter));
	}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithConstAnySubquery(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithConstAllSubquery(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectCorrelated(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectOnOuterJoin(IMemoryPool *mp)
{
	...
	const CColRef *pcrOuter = CDrvdPropRelational::GetRelationalProperties(pexprOuter->PdpDerive())
								  ->PcrsOutput()
								  ->PcrAny();
	const CColRef *pcrInner = CDrvdPropRelational::GetRelationalProperties(pexprInner->PdpDerive())
								  ->PcrsOutput()
								  ->PcrAny();
	CExpression *pexprOuterJoinPred = CUtils::PexprScalarEqCmp(mp, pcrOuter, pcrInner);
	CExpression *pexprOuterJoin = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), pexprOuter, pexprInner, pexprOuterJoinPred);
	CExpression *pexprPred =
		CUtils::PexprScalarEqCmp(mp, pcrInner, CUtils::PexprScalarConstInt4(mp, 5 /*val*/));
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectCorrelated(IMemoryPool *mp, CColRefSet *outer_refs, ULONG ulLevel)
{
	...
	CExpression *pexprResult =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexpr, pexprCorrelation);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalProject(IMemoryPool *mp,
								CExpression *pexpr,
								CColRef *colref,
								CColRef *new_colref)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalProjectGbAggCorrelated(IMemoryPool *mp)
{
	...
	CExpression *pexprProject =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalProject(mp), pexprGbAgg, pexprPrjList);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalJoinCorrelated(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalJoinWithPartitionedAndIndexedInnerChild(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalNAryJoin(IMemoryPool *mp, CExpressionArray *pdrgpexpr)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalNAryJoin(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLeftOuterJoinOnNAryJoin(IMemoryPool *mp)
{
	...
	CColRef *pcrLeft1 =
		CDrvdPropRelational::GetRelationalProperties(pexprLOJInnerChild->PdpDerive())
			->PcrsOutput()
			->PcrAny();

	// get a random column from NAry join second child
	CColRef *pcrLeft2 =
		CDrvdPropRelational::GetRelationalProperties((*pexprNAryJoin)[1]->PdpDerive())
			->PcrsOutput()
			->PcrAny();

	// get a random column from NAry join output
	CColRef *pcrRight = CDrvdPropRelational::GetRelationalProperties(pexprNAryJoin->PdpDerive())
							->PcrsOutput()
							->PcrAny();
	...
	return GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), pexprNAryJoin, pexprLOJInnerChild, pexprPred);
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprNAryJoinOnLeftOuterJoin(IMemoryPool *mp)
{
	CExpression *pexprNAryJoin = PexprLogicalNAryJoin(mp);
	CColRef *pcrNAryJoin =
		CDrvdPropRelational::GetRelationalProperties((*pexprNAryJoin)[0]->PdpDerive())
			->PcrsOutput()
			->PcrAny();
	...
	CColRef *pcrLeft = CDrvdPropRelational::GetRelationalProperties(pexprLOJOuterChild->PdpDerive())
						   ->PcrsOutput()
						   ->PcrAny();
	...
	CExpression *pexprLOJ = GPOS_NEW(mp) CExpression(mp,
...
CExpression *
CTestUtils::PexprLogicalLimit(
	IMemoryPool *mp, CExpression *pexpr, LINT iStart, LINT iRows, BOOL fGlobal, BOOL fHasCount)
{
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalLimit(IMemoryPool *mp)
{
	return PexprLogicalLimit(mp,
							 PexprLogicalGet(mp),
							 0,		// iStart
							 100,   // iRows
							 true,  // fGlobal
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGbAggWithSum(IMemoryPool *mp)
{
	CExpression *pexprGb = PexprLogicalGbAgg(mp);

	// get a random column from Gb child
	CColRef *colref = CDrvdPropRelational::GetRelationalProperties((*pexprGb)[0]->PdpDerive())
						  ->PcrsOutput()
						  ->PcrAny();
	...
	colref_array->AddRef();
	CExpression *pexprGbWithSum =
		CUtils::PexprLogicalGbAggGlobal(mp, colref_array, (*pexprGb)[0], pexprPrjList);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGbAggWithInput(IMemoryPool *mp, CExpression *pexprInput)
{
	// get the first few columns
	CDrvdPropRelational *pdprel =
		CDrvdPropRelational::GetRelationalProperties(pexprInput->PdpDerive());
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGbAgg(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprPrjElemWithSum(IMemoryPool *mp, CColRef *colref)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	...
	CExpression *pexprScalarAgg = CUtils::PexprAggFunc(mp,
													   pmdidSumAgg,
													   pstrAggFunc,
													   colref,
													   false /*is_distinct*/,
	...
	CName name(mp, &str);
	CColRef *pcrComputed =
		COptCtxt::PoctxtFromTLS()->Pcf()->PcrCreate(pmdtype, pop->TypeModifier(), name);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGbAggOverJoin(IMemoryPool *mp)
{
	...
	CColRefArray *colref_array = GPOS_NEW(mp) CColRefArray(mp);
	CColRef *colref = CDrvdPropRelational::GetRelationalProperties(pexprJoin->PdpDerive())
						  ->PcrsOutput()
						  ->PcrAny();
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGbAggDedupOverInnerJoin(IMemoryPool *mp)
{
	// generate a join expression
	CExpression *pexprJoin = PexprLogicalJoin<CLogicalInnerJoin>(mp);
	...
	return GPOS_NEW(mp)
		CExpression(mp,
					GPOS_NEW(mp) CLogicalGbAggDeduplicate(
						mp, pdrgpcrGrp, COperator::EgbaggtypeGlobal /*egbaggtype*/, pdrgpcrKeys),
					pexprJoin,
					GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalGbAggCorrelated(IMemoryPool *mp)
{
	...
	CColRefArray *colref_array = GPOS_NEW(mp) CColRefArray(mp);
	CColRef *colref =
		CDrvdPropRelational::GetRelationalProperties(pexpr->PdpDerive())->PcrsOutput()->PcrAny();
	colref_array->Append(colref);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprConstTableGet(IMemoryPool *mp, ULONG ulElements)
{
	...
	CName name(&strName);
	CColumnDescriptor *pcoldescInt = GPOS_NEW(mp) CColumnDescriptor(
		mp, pmdtypeint4, default_type_modifier, name, 1 /* attno */, false /*IsNullable*/
	);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprConstTableGet5(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalInsert(IMemoryPool *mp)
{
	CExpression *pexprCTG = PexprConstTableGet(mp, 1 /* ulElements */);

	CColRefSet *pcrs =
		CDrvdPropRelational::GetRelationalProperties(pexprCTG->PdpDerive())->PcrsOutput();
	...
	return GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalInsert(mp, ptabdesc, colref_array), pexprCTG);
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalDelete(IMemoryPool *mp)
{
	CWStringConst strName(GPOS_WSZ_LIT("BaseTable"));
	CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 1, 1);
	CTableDescriptor *ptabdesc = PtabdescCreate(mp, 1, mdid, CName(&strName));
	...
	return GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalDelete(mp, ptabdesc, colref_array, colref, colref), pexprGet);
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalUpdate(IMemoryPool *mp)
{
	CWStringConst strName(GPOS_WSZ_LIT("BaseTable"));
	CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 1, 1);
	CTableDescriptor *ptabdesc = PtabdescCreate(mp, 1, mdid, CName(&strName));
	...
	return GPOS_NEW(mp) CExpression(
		...
		GPOS_NEW(mp) CLogicalUpdate(
			mp, ptabdesc, pdrgpcrDelete, pdrgpcrInsert, colref, colref, NULL /*pcrTupleOid*/),
		pexprGet);
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalDynamicGet(IMemoryPool *mp,
								   CTableDescriptor *ptabdesc,
								   const CWStringConst *pstrTableAlias,
								   ULONG ulPartIndex)
{
	...
	return GPOS_NEW(mp)
		CExpression(mp,
					GPOS_NEW(mp) CLogicalDynamicGet(
						mp, GPOS_NEW(mp) CName(mp, CName(pstrTableAlias)), ptabdesc, ulPartIndex));
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithEqPredicateOverDynamicGet(IMemoryPool *mp)
{
	CWStringConst strName(GPOS_WSZ_LIT("PartTable"));
	CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID_PARTITIONED, 1, 1);
	CTableDescriptor *ptabdesc =
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSelectWithLTPredicateOverDynamicGet(IMemoryPool *mp)
{
	CWStringConst strName(GPOS_WSZ_LIT("PartTable"));
	CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID_PARTITIONED, 1, 1);
	CTableDescriptor *ptabdesc =
	...
	CExpression *pexprScalar =
		CUtils::PexprScalarCmp(mp, pexprScalarIdent, pexprConst, IMDType::EcmptL);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalTVFTwoArgs(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalTVFThreeArgs(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalTVFNoArgs(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalTVF(IMemoryPool *mp, ULONG ulArgs)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	...
	CColumnDescriptor *pcoldescInt = GPOS_NEW(mp) CColumnDescriptor(
		mp, pmdtypeint8, default_type_modifier, name, 1 /* attno */, false /*IsNullable*/);
	...
	CLogicalTVF *popTVF =
		GPOS_NEW(mp) CLogicalTVF(mp, mdid, mdid_return_type, str_func_name, pdrgpcoldesc);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalCTEProducerOverSelect(IMemoryPool *mp, ULONG ulCTEId)
{
	CExpression *pexprSelect = CTestUtils::PexprLogicalSelect(mp);
	CColRefArray *colref_array =
		CDrvdPropRelational::GetRelationalProperties(pexprSelect->PdpDerive())
			->PcrsOutput()
			->Pdrgpcr(mp);

	return GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalCTEProducer(mp, ulCTEId, colref_array), pexprSelect);
}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalCTEProducerOverSelect(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprCTETree(IMemoryPool *mp)
{
	ULONG ulCTEId = 0;
	CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSequence(IMemoryPool *mp, CExpressionArray *pdrgpexpr)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSequence(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalUnion(IMemoryPool *mp, ULONG ulDepth)
{
	...
	else
	{
		...
		pexpr = GPOS_NEW(mp) CExpression(
			mp, GPOS_NEW(mp) CLogicalUnion(mp, pdrgpcrOutput, pdrgpdrgpcrInput), pdrgpexprInput);
	}
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprLogicalSequenceProject(IMemoryPool *mp, OID oidFunc, CExpression *pexprInput)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();
	...
	CExpression *pexprWinFunc = GPOS_NEW(mp)
		CExpression(mp,
	...
	CColRef *pcrComputed = col_factory->PcrCreate(
		md_accessor->RetrieveType(pmdfunc->GetResultTypeMdid()), default_type_modifier);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprOneWindowFunction(IMemoryPool *mp)
{
	CExpression *pexprGet = PexprLogicalGet(mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprTwoWindowFunctions(IMemoryPool *mp)
{
...
CTestUtils::PdrgpexprJoins(IMemoryPool *mp,
...
CExpression *
CTestUtils::PexprLogicalNAryJoin(
	IMemoryPool *mp, CWStringConst *pstrRel, ULONG *pulRel, ULONG ulRels, BOOL fCrossProduct)
{
	CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
	for (ULONG ul = 0; ul < ulRels; ul++)
	{
	...
	if (!fCrossProduct)
	{
		...
		for (ULONG ul = 0; ul < ulRels - 1; ul++)
		{
			...
			CColRef *pcrLeft = CDrvdPropRelational::GetRelationalProperties(pexprLeft->PdpDerive())
								   ->PcrsOutput()
								   ->PcrAny();
			CColRef *pcrRight =
...
//---------------------------------------------------------------------------
CQueryContext *
CTestUtils::PqcGenerate(IMemoryPool *mp, CExpression *pexpr, CColRefArray *colref_array)
{
	...
	return GPOS_NEW(mp)
		CQueryContext(mp, pexpr, prpp, colref_array, pdrgpmdname, true /*fDeriveStats*/);
}
...
//---------------------------------------------------------------------------
CQueryContext *
CTestUtils::PqcGenerate(IMemoryPool *mp, CExpression *pexpr)
{
	CColRefSet *pcrs = GPOS_NEW(mp) CColRefSet(mp);
	...
	COrderSpec *pos = GPOS_NEW(mp) COrderSpec(mp);
	pos->Append(
		GPOS_NEW(mp) CMDIdGPDB(GPDB_INT4_LT_OP), pcrsOutput->PcrAny(), COrderSpec::EntFirst);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprScalarNestedPreds(IMemoryPool *mp,
								   CExpression *pexpr,
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprFindFirstExpressionWithOpId(CExpression *pexpr, COperator::EOperatorId op_id)
{
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
void
CTestUtils::EqualityPredicate(IMemoryPool *mp,
							  CColRefSet *pcrsLeft,
							  CColRefSet *pcrsRight,
...
//---------------------------------------------------------------------------
CPoint *
CTestUtils::PpointInt2(IMemoryPool *mp, INT i)
{
...
//---------------------------------------------------------------------------
CPoint *
CTestUtils::PpointInt4(IMemoryPool *mp, INT i)
{
...
//---------------------------------------------------------------------------
CPoint *
CTestUtils::PpointInt4NullVal(IMemoryPool *mp)
{
...
//---------------------------------------------------------------------------
CPoint *
CTestUtils::PpointInt8(IMemoryPool *mp, INT i)
{
...
//---------------------------------------------------------------------------
CPoint *
CTestUtils::PpointBool(IMemoryPool *mp, BOOL value)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprReadQuery(IMemoryPool *mp, const CHAR *szQueryFileName)
{
	...
	CExpression *pexprQuery = trdxl2expr.PexprTranslateQuery(ptroutput->CreateDXLNode(),
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresTranslate(IMemoryPool *mp,
						  ...
						  BOOL fIgnoreMismatch)
{
	...
	CExpression *pexprQuery = ptrdxl2expr.PexprTranslateQuery(ptroutput->CreateDXLNode(),
															  ptroutput->GetOutputColumnsDXLArray(),
															  ptroutput->GetCTEProducerDXLArray());

	CQueryContext *pqc = CQueryContext::PqcGenerate(mp,
													pexprQuery,
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresCompare(IOstream &os,
...
//---------------------------------------------------------------------------
BOOL
CTestUtils::FPlanMatch(IMemoryPool *mp,
					   IOstream &os,
...
//---------------------------------------------------------------------------
BOOL
CTestUtils::FPlanCompare(IMemoryPool *mp,
						 IOstream &os,
						 const CDXLNode *pdxlnActual,
						 ULLONG ullPlanIdActual,
						 ULLONG ullPlanSpaceSizeActual,
						 ...
						 INT iCmpSpaceSize)
{
	...
	if ((0 == iCmpSpaceSize &&
		 ullPlanSpaceSizeActual != ullPlanSpaceSizeExpected) ||  // required comparison is equality
		(-1 == iCmpSpaceSize &&
		 ullPlanSpaceSizeActual >
...
//---------------------------------------------------------------------------
IntPtrArray *
CTestUtils::PdrgpiSegments(IMemoryPool *mp)
{
...
CTestUtils::FFaultSimulation()
{
	return GPOS_FTRACE(EtraceSimulateAbort) || GPOS_FTRACE(EtraceSimulateIOError) ||
		   GPOS_FTRACE(EtraceSimulateOOM) || IWorker::m_enforce_time_slices;
}
#endif  // GPOS_DEBUG
...
//---------------------------------------------------------------------------
CHAR *
CTestUtils::SzMinidumpFileName(IMemoryPool *mp, const CHAR *file_name)
{
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresRunMinidump(IMemoryPool *mp,
							...
							IConstExprEvaluator *pceeval)
{
	...
	pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(mp,
													   md_accessor,
													   pdxlmd,
													   szMinidumpFileName,
													   ulSegments,
							 ...
							 IConstExprEvaluator *pceeval)
{
	...
	for (ULONG ul = *pulTestCounter; ul < ulTests; ul++)
	{
		...
		if (fMatchPlans)
		{
			...
			eres = EresRunMinidumpsUsingOneMDFile(mp,
												  rgszFileNames[ul],
												  &rgszFileNames[ul],
												  pulTestCounter,
												  ulSessionId,
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresRunMinidumpsUsingOneMDFile(IMemoryPool *mp,
										   const CHAR *szMDFilePath,
										   const CHAR *rgszFileNames[],
										   ULONG *pulTestCounter,
										   ULONG ulSessionId,
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresSamplePlans(const CHAR *rgszFileNames[],
							ULONG ulTests,
							ULONG *pulTestCounter,
							ULONG ulSessionId,
							ULONG ulCmdId)
	...
	for (ULONG ul = *pulTestCounter; ul < ulTests; ul++)
	{
...

		{
...

			{
				...
				at.Os() << "Generated " << optimizer_config->GetEnumeratorCfg()->UlCreatedSamples()
						<< " samples ... " << std::endl;
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresCheckPlans(const CHAR *rgszFileNames[],
						   ULONG ulTests,
						   ULONG *pulTestCounter,
						   ULONG ulSessionId,
						   ULONG ulCmdId,
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresCheckOptimizedPlan(const CHAR *rgszFileNames[],
								   ULONG ulTests,
								   ULONG *pulTestCounter,
								   ULONG ulSessionId,
								   ULONG ulCmdId,
...
//---------------------------------------------------------------------------
IDatum *
CTestUtils::CreateGenericDatum(IMemoryPool *mp,
							   CMDAccessor *md_accessor,
							   IMDId *mdid_type,
							   ...
							   LINT value)
{
	...
	if (CMDTypeGenericGPDB::IsTimeRelatedType(mdid_type))
	{
		dxl_datum = GPOS_NEW(mp)
			CDXLDatumStatsDoubleMappable(mp,
										 mdid_type,
	...
	{
		dxl_datum =
			GPOS_NEW(mp) CDXLDatumStatsLintMappable(mp,
													mdid_type,
													default_type_modifier,
													pmdtype->IsPassedByValue() /*is_const_by_val*/,
													false /*is_const_null*/,
	...
	{
		dxl_datum = GPOS_NEW(mp) CDXLDatumGeneric(mp,
												  mdid_type,
												  default_type_modifier,
												  pmdtype->IsPassedByValue() /*is_const_by_val*/,
												  false /*is_const_null*/,
...
//---------------------------------------------------------------------------
CConstraintInterval *
CTestUtils::PciGenericInterval(IMemoryPool *mp,
							   CMDAccessor *md_accessor,
							   ...
							   CRange::ERangeInclusion eriRight)
{
	...
	CRange *prange = GPOS_NEW(mp) CRange(
		mdid, COptCtxt::PoctxtFromTLS()->Pcomp(), pdatumLower, eriLeft, pdatumUpper, eriRight);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprScalarCmpIdentToConstant(IMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprExistsSubquery(IMemoryPool *mp, CExpression *pexprOuter)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexpSubqueryAll(IMemoryPool *mp, CExpression *pexprOuter)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != pexprOuter);
	...
	CColRefSet *pcrsInner =
		CDrvdPropRelational::GetRelationalProperties(pexprInner->PdpDerive())->PcrsOutput();
	const CColRef *pcrInner = pcrsInner->PcrAny();

	return GPOS_NEW(mp)
		CExpression(mp,
					GPOS_NEW(mp) CScalarSubqueryAll(mp,
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexpSubqueryAny(IMemoryPool *mp, CExpression *pexprOuter)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != pexprOuter);
	...
	return GPOS_NEW(mp)
		CExpression(mp,
					GPOS_NEW(mp) CScalarSubqueryAny(mp,
													GPOS_NEW(mp) CMDIdGPDB(GPDB_INT4_EQ_OP),
													GPOS_NEW(mp) CWStringConst(GPOS_WSZ_LIT("=")),
													pcrInner),
					pexprInner,
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprNotExistsSubquery(IMemoryPool *mp, CExpression *pexprOuter)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != pexprOuter);
...
//---------------------------------------------------------------------------
const CExpression *
CTestUtils::PexprFirst(const CExpression *pexpr, const COperator::EOperatorId op_id)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprAnd(IMemoryPool *mp, CExpression *pexprActual, CExpression *pexprExpected)
{
...
//---------------------------------------------------------------------------
CExpression *
CTestUtils::PexprOr(IMemoryPool *mp, CExpression *pexprActual, CExpression *pexprExpected)
{
	GPOS_ASSERT(NULL != pexprActual);
	GPOS_ASSERT(NULL != pexprExpected);
...
GPOS_RESULT
CTestUtils::EresUnittest_RunTestsWithoutAdditionalTraceFlags(const CHAR **rgszFileNames,
															 ULONG *pulTestCounter,
															 ULONG ulTests,
															 BOOL fMatchPlans,
															 BOOL fTestSpacePruning)
{
	...
	GPOS_RESULT eres = CTestUtils::EresRunMinidumps(mp,
													rgszFileNames,
													ulTests,
													pulTestCounter,
													1,  // ulSessionId
