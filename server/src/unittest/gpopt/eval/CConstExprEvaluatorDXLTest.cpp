...
)
{
	...
	CDXLDatumInt4 *dxl_datum =
		GPOS_NEW(m_mp) CDXLDatumInt4(m_mp, pmdtypeint4->MDId(), false /*is_const_null*/, m_val);
...
{
	{
		CUnittest rgut[] = {
			...
			GPOS_UNITTEST_FUNC_THROW(CConstExprEvaluatorDXLTest::EresUnittest_NonScalar,
									 gpdxl::ExmaGPOPT,
									 gpdxl::ExmiEvalUnsupportedScalarExpr),
			GPOS_UNITTEST_FUNC_THROW(CConstExprEvaluatorDXLTest::EresUnittest_NestedSubquery,
									 gpdxl::ExmaGPOPT,
									 gpdxl::ExmiEvalUnsupportedScalarExpr),
...
CConstExprEvaluatorDXLTest::EresUnittest_ScalarContainingVariables()
{
	...
	CExpression *pexprFunCall =
		CUtils::PexprScalarEqCmp(testsetup.Pmp(),
