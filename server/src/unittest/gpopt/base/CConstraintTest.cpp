...
CConstraintTest::EresUnittest_CConjunction()
{
	...
	CColRefSet *pcrs1 =
		CDrvdPropRelational::GetRelationalProperties(pexprGet1->PdpDerive())->PcrsOutput();
	CColRef *pcr1 = pcrs1->PcrAny();

	CExpression *pexprGet2 = CTestUtils::PexprLogicalGet(mp);
	CColRefSet *pcrs2 =
		CDrvdPropRelational::GetRelationalProperties(pexprGet2->PdpDerive())->PcrsOutput();
...
CConstraintTest::Pcstconjunction(IMemoryPool *mp, IMDId *mdid, CColRef *colref)
{
	// first interval
	CConstraintInterval *pciFirst = PciFirstInterval(mp, mdid, colref);
...
CConstraintTest::Pcstdisjunction(IMemoryPool *mp, IMDId *mdid, CColRef *colref)
{
	// first interval
	CConstraintInterval *pciFirst = PciFirstInterval(mp, mdid, colref);
...
CConstraintTest::EresUnittest_CConstraintIntervalFromArrayExpr()
{
	...
	CConstraintInterval *pcnstIn =
		CConstraintInterval::PciIntervalFromScalarExpr(mp, pexprArrayComp, colref);
	GPOS_ASSERT(CConstraint::EctInterval == pcnstIn->Ect());
	GPOS_ASSERT(pcnstIn->Pdrgprng()->Size() ==
	...
	CExpression *pexprNotIn =
		CTestUtils::PexprLogicalSelectArrayCmp(mp, CScalarArrayCmp::EarrcmpAll, IMDType::EcmptNEq);
	CExpression *pexprArrayNotInComp = (*pexprNotIn->PdrgPexpr())[1];
	CColRef *pcrNot = CDrvdPropRelational::GetRelationalProperties(pexprNotIn->PdpDerive())
						  ->PcrsOutput()
						  ->PcrAny();
	CConstraintInterval *pcnstNotIn =
		CConstraintInterval::PciIntervalFromScalarExpr(mp, pexprArrayNotInComp, pcrNot);
	...
	CColRef *pcrInRepeats =
		CDrvdPropRelational::GetRelationalProperties(pexprInRepeatsSelect->PdpDerive())
			->PcrsOutput()
			->PcrAny();
	...
	CExpression *pexprNotInRepeatsSelect = CTestUtils::PexprLogicalSelectArrayCmp(
		mp, CScalarArrayCmp::EarrcmpAll, IMDType::EcmptNEq, pdrgpi);
	CColRef *pcrNotInRepeats =
		CDrvdPropRelational::GetRelationalProperties(pexprNotInRepeatsSelect->PdpDerive())
			->PcrsOutput()
			->PcrAny();
	CExpression *pexprNotInArrayCmpRepeats = (*pexprNotInRepeatsSelect->PdrgPexpr())[1];
	CConstraintInterval *pcnstNotInRepeats = CConstraintInterval::PciIntervalFromScalarExpr(
...
EresUnittest_CConstraintIntervalFromArrayExprIncludesNull()
{
	...
	CColRef *colref =
		CDrvdPropRelational::GetRelationalProperties(pexprIn->PdpDerive())->PcrsOutput()->PcrAny();
	CConstraintInterval *pci =
		CConstraintInterval::PciIntervalFromScalarExpr(mp, (*pexprIn)[1], colref);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CConstraintTest::EresUnittest_CIntervalFromScalarCmp(IMemoryPool *mp,
													 ...
													 CColRef *colref)
{
	...
	for (ULONG ul = 0; ul < GPOS_ARRAY_SIZE(rgecmpt); ul++)
	{
		...
		CConstraintInterval *pci =
			CConstraintInterval::PciIntervalFromScalarExpr(mp, pexprScCmp, colref);
		PrintConstraint(mp, pci);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CConstraintTest::EresUnittest_CIntervalFromScalarBoolOp(IMemoryPool *mp,
														CMDAccessor *md_accessor,
														CColRef *colref)
{
	// AND
...
//---------------------------------------------------------------------------
CExpression *
CConstraintTest::PexprScalarCmp(IMemoryPool *mp,
								CMDAccessor *md_accessor,
								CColRef *colref,
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintTest::PciFirstInterval(IMemoryPool *mp, IMDId *mdid, CColRef *colref)
{
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintTest::PciSecondInterval(IMemoryPool *mp, IMDId *mdid, CColRef *colref)
{
	const SRangeInfo rgRangeInfo[] = {
...
//---------------------------------------------------------------------------
CRangeArray *
CConstraintTest::Pdrgprng(IMemoryPool *mp,
						  IMDId *mdid,
						  const SRangeInfo rgRangeInfo[],
						  ULONG ulRanges)
{
...
//---------------------------------------------------------------------------
void
CConstraintTest::PrintConstraint(IMemoryPool *mp, CConstraint *pcnstr)
{
...
//---------------------------------------------------------------------------
void
CConstraintTest::PrintEquivClasses(IMemoryPool *mp, CColRefSetArray *pdrgpcrs, BOOL fExpected)
{
	// debug print
	CAutoTrace at(mp);
	at.Os() << std::endl;
...
CConstraintTest::EresUnittest_NegativeTests()
{
	...
	CAutoP<CWStringDynamic> pstrLower1(GPOS_NEW(mp)
										   CWStringDynamic(mp, GPOS_WSZ_LIT("AAAAB2Jheg==")));
	CAutoP<CWStringDynamic> pstrUpper1(GPOS_NEW(mp)
										   CWStringDynamic(mp, GPOS_WSZ_LIT("AAAACmZvb2Jhcg==")));
	...
	CConstraintInterval *pciFirst = CTestUtils::PciGenericInterval(mp,
																   &mda,
