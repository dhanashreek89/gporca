...
//
//---------------------------------------------------------------------------
CTreeMapTest::CNode::CNode(IMemoryPool *,  // mp
						   ULONG *pulData,
						   CNodeArray *pdrgpnd)
...
//---------------------------------------------------------------------------
CTreeMapTest::CNode *
CTreeMapTest::Pnd(IMemoryPool *mp, ULONG *pul, CNodeArray *pdrgpnd, BOOL *fTestTrue)
{
...
CTreeMapTest::PtmapLoad(IMemoryPool *mp)
{
	...
	CExpression *pexprPlan = NULL;
	{
		...
		peng->Trace();
		{
			...
			for (ULONG ulRank = 0; ulRank < ullCount; ulRank++)
			{
				...
				{
					if (!GPOS_MATCH_EX(
							ex, gpopt::ExmaGPOPT, gpopt::ExmiUnsatisfiedRequiredProperties))
					{
						GPOS_RETHROW(ex);
					}
...
CTreeMapTest::EresUnittest_FailedPlanEnumerationTests()
{
	...
	for (ULONG ul = 0; eres == GPOS_OK && ul < ulTests; ul++)
	{
		GPOS_TRY
		{
			eres = CTestUtils::EresRunMinidumps(mp,
