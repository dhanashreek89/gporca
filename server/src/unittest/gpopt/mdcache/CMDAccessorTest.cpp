...
CMDAccessorTest::EresUnittest()
{
	CUnittest rgut[] = {
		...
		GPOS_UNITTEST_FUNC_THROW(
			CMDAccessorTest::EresUnittest_Negative, gpdxl::ExmaMD, gpdxl::ExmiMDCacheEntryNotFound),
...
CMDAccessorTest::EresUnittest_Basic()
{
	...
	CMDIdGPDB *pmdidObject1 = GPOS_NEW(mp)
		CMDIdGPDB(GPOPT_MDCACHE_TEST_OID /* OID */, 1 /* major version */, 1 /* minor version */);
	CMDIdGPDB *pmdidObject2 = GPOS_NEW(mp)
		CMDIdGPDB(GPOPT_MDCACHE_TEST_OID /* OID */, 12 /* version */, 1 /* minor version */);
...
CMDAccessorTest::EresUnittest_Navigate()
{
	...
	CMDIdGPDB *mdid_func = GPOS_NEW(mp)
		CMDIdGPDB(GPDB_FUNC_TIMEOFDAY /* OID */, 1 /* major version */, 0 /* minor version */);
...
CMDAccessorTest::EresUnittest_Indexes()
{
	...
	CMDIdGPDB *rel_mdid =
		GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 1 /* major */, 1 /* minor version */);
...
CMDAccessorTest::EresUnittest_CheckConstraint()
{
	...
	// lookup a relation in the MD cache
	CMDIdGPDB *rel_mdid =
		GPOS_NEW(mp) CMDIdGPDB(GPOPT_TEST_REL_OID21, 1 /* major */, 1 /* minor version */);
...
CMDAccessorTest::EresUnittest_Negative()
{
	...
	CMDIdGPDB *pmdidNonExistingObject = GPOS_NEW(mp)
		CMDIdGPDB(GPOPT_MDCACHE_TEST_OID /* OID */, 15 /* version */, 1 /* minor version */);
...
CMDAccessorTest::EresUnittest_ConcurrentAccessMultipleMDA()
{
	...
	GPOS_TRY
	{
		...
		for (ULONG i = 0; i < GPOS_ARRAY_SIZE(rgPtsk); i++)
		{
			rgPtsk[i] = atp.Create(PvInitMDAAndLookup,
...
CMDAccessorTest::PvLookupMultipleObj(void *pv)
{
	...
	for (ULONG ul = 0; ul < ulNumberOfObjects; ul++)
	{
		...
		CMDIdGPDB *mdid = GPOS_NEW(pmdtaskparams->m_mp)
			CMDIdGPDB(GPOPT_MDCACHE_TEST_OID /*OID*/, 1 /*major*/, ul + 1 /*minor*/);
		(void) md_accessor->RetrieveRel(mdid);
		mdid->Release();
	}
