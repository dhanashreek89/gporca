...
CMiniDumperDXLTest::EresUnittest_Basic()
{
	...
	GPOS_TRY
	{
		...
		CSerializableQuery serQuery(mp,
		...
		CDXLNode *pdxlnPlan =
			ptrexprtodxl.PdxlnTranslate(pexprPlan, pqc->PdrgPcr(), pqc->Pdrgpmdname());
	...
	GPOS_CATCH_EX(ex)
	{
		...
		GPOS_ASSERT_IMP(!GPOS_FTRACE(EtraceSimulateAbort) && !GPOS_FTRACE(EtraceSimulateIOError) &&
		...
		CMinidumperUtils::GenerateMinidumpFileName(file_name,
												   GPOS_FILE_NAME_BUF_SIZE,
												   ulSessionId,
												   ulCommandId,
