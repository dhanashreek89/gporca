...
//---------------------------------------------------------------------------
void
CCostTest::TestParams(IMemoryPool *mp, BOOL fCalibrated)
{
	CAutoTrace at(mp);
	IOstream &os(at.Os());
	...
	{
		pcp = ((CCostModelGPDBLegacy *) COptCtxt::PoctxtFromTLS()->GetCostModel())
				  ->GetCostModelParams();

		dSeqIOBandwidth = pcp->PcpLookup(CCostModelParamsGPDBLegacy::EcpSeqIOBandwidth)->Get();
		dRandomIOBandwidth =
			pcp->PcpLookup(CCostModelParamsGPDBLegacy::EcpRandomIOBandwidth)->Get();
...
CCostTest::EresUnittest_Parsing()
{
	...
	CParseHandlerDXL *pphDXL =
		CDXLUtils::GetParseHandlerForDXLFile(mp, "../data/dxl/cost/cost0.xml", NULL);
...
CCostTest::EresUnittest_ParsingWithException()
{
	...
	IMemoryPool *mp = amp.Pmp();
	CParseHandlerDXL *pphDXL =
		CDXLUtils::GetParseHandlerForDXLFile(mp, "../data/dxl/cost/wrong-cost.xml", NULL);
	GPOS_DELETE(pphDXL);
...
CCostTest::EresUnittest_SetParams()
{
	...
	CExpression *pexpr =
		CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprOuter, pexprInner, pexprPred);
	...
	ICostModelParams::SCostParam *pcp =
		pcm->GetCostModelParams()->PcpLookup(CCostModelParamsGPDB::EcpNLJFactor);
