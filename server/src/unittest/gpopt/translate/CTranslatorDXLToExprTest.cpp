...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExprTest::Pexpr(IMemoryPool *mp, const CHAR *dxl_filename)
{
	...
	CQueryToDXLResult *ptroutput =
		CDXLUtils::ParseQueryToQueryDXLTree(mp, szDXLExpected, NULL /*xsd_file_path*/);
	...
	CExpression *pexprTranslated =
		pdxltr->PexprTranslateQuery(ptroutput->CreateDXLNode(),
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTranslatorDXLToExprTest::EresTranslateAndCheck(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CWStringDynamic *
CTranslatorDXLToExprTest::Pstr(IMemoryPool *mp, CExpression *pexpr)
{
...
namespace
{
	...
	public:
		GetBuilder(IMemoryPool *mp, CWStringConst strTableName, OID oidTableOid)
			: m_mp(mp), m_strTableName(strTableName)
		{
			...
			m_ptabdesc = GPOS_NEW(mp) CTableDescriptor(mp,
													   mdid,
													   CName(&strTableName),
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExprTest::PexprGet(IMemoryPool *mp)
{
...
CTranslatorDXLToExprTest::EresUnittest_SelectQueryWithConstInList()
{
	...
	// manually create the Expr Tree
	{
		...
		CExpression *pexprScalarArrayCmp = CUtils::PexprScalarArrayCmp(
			mp, CScalarArrayCmp::EarrcmpAny, IMDType::EcmptEq, pexprScalarChildren, colref);
		...
		CExpression *pexprExpected =
			CUtils::PexprLogicalSelect(mp, pexprLgGet, pexprScalarArrayCmpCollapsed);
		pstrExpected = Pstr(mp, pexprExpected);
...
CTranslatorDXLToExprTest::EresUnittest_SelectQueryWithBoolExpr()
{
	...
	// manually create the Expr Tree
	{
		...
		pdrgpexprInput->Append(pexprScCmp);
		CExpression *pexprScBool =
			CUtils::PexprScalarBoolOp(mp, CScalarBoolOp::EboolopAnd, pdrgpexprInput);
...
CTranslatorDXLToExprTest::EresUnittest_SelectQueryWithScalarOp()
{
	...
	// manually create the Expr Tree
	{
		...
		CExpression *pexprScCmp = CUtils::PexprScalarCmp(mp,
														 pcrRight,
														 pexprScOp,
														 CWStringConst(GPOS_WSZ_LIT(">")),
