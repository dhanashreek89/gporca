...
CMDProviderTest::EresUnittest()
{
	CUnittest rgut[] = {
		...
		GPOS_UNITTEST_FUNC_THROW(
			CMDProviderTest::EresUnittest_Negative, gpdxl::ExmaMD, gpdxl::ExmiMDCacheEntryNotFound),
	};
...
//---------------------------------------------------------------------------
void
CMDProviderTest::TestMDLookup(IMemoryPool *mp, IMDProvider *pmdp)
{
	CAutoMDAccessor amda(mp, pmdp, CTestUtils::m_sysidDefault, CMDCache::Pcache());

	// lookup different objects
	CMDIdGPDB *pmdid1 = GPOS_NEW(mp)
		CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 1 /* major version */, 1 /* minor version */);
	CMDIdGPDB *pmdid2 =
		GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 12 /* version */, 1 /* minor version */);
...
CMDProviderTest::EresUnittest_Stats()
{
...

	{
		...
		CMDIdRelStats *rel_stats_mdid =
			GPOS_NEW(mp) CMDIdRelStats(GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 1, 1));
		...
		CMDIdColStats *mdid_col_stats = GPOS_NEW(mp)
			CMDIdColStats(GPOS_NEW(mp) CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 1, 1), 1 /* attno */);
...
CMDProviderTest::EresUnittest_Negative()
{
...

	{
		...
		CMDIdGPDB *mdid = GPOS_NEW(mp)
			CMDIdGPDB(GPOPT_MDCACHE_TEST_OID, 15 /* major version */, 1 /* minor version */);
