...
CJoinCardinalityTest::EresUnittest_JoinNDVRemain()
{
	...
	for (ULONG ul2 = 0; ul2 < ulTestCases && (GPOS_FAILED != eres); ul2++)
	{
		...
		CHistogram *join_histogram =
			histogram1->MakeJoinHistogram(mp, CStatsPred::EstatscmptEq, histogram2);
		...
		CDouble dDiffNDVJoin(fabs((dNDVBucketsJoin - CStatisticsUtils::GetNumDistinct(
														 join_histogram->ParseDXLToBucketsArray()))
									  .Get()));
...
CJoinCardinalityTest::EresUnittest_Join()
{
	...
	SStatsJoinSTestCase rgstatsjointc[] = {
		...
		{"../data/dxl/statistics/Join-Statistics-DDistinct-Input.xml",
		 "../data/dxl/statistics/Join-Statistics-DDistinct-Output.xml",
		 false,
		 PdrgpstatspredjoinSingleJoinPredicate},
		{"../data/dxl/statistics/Join-Statistics-Text-Input.xml",
		 "../data/dxl/statistics/Join-Statistics-Text-Output.xml",
		 false,
		 PdrgpstatspredjoinSingleJoinPredicate},
	};
...
CStatsPredJoinArray *
CJoinCardinalityTest::PdrgpstatspredjoinSingleJoinPredicate(IMemoryPool *mp)
{
...
CStatsPredJoinArray *
CJoinCardinalityTest::PdrgpstatspredjoinMultiplePredicates(IMemoryPool *mp)
{
	CStatsPredJoinArray *join_preds_stats = GPOS_NEW(mp) CStatsPredJoinArray(mp);
...
CStatsPredJoinArray *
CJoinCardinalityTest::PdrgpstatspredjoinNullableCols(IMemoryPool *mp)
{
	CStatsPredJoinArray *join_preds_stats = GPOS_NEW(mp) CStatsPredJoinArray(mp);
