...
CStatisticsTest::EresUnittest_GbAggWithRepeatedGbCols()
{
	...
	CExpression *pexprPrj =
		CUtils::PexprAddProjection(mp, pexpr, CUtils::PexprScalarIdent(mp, colrefs->PcrFirst()));
...
CHistogram *
CStatisticsTest::PhistExampleInt4Dim(IMemoryPool *mp)
{
...
CTableDescriptor *
CStatisticsTest::PtabdescTwoColumnSource(IMemoryPool *mp,
										 ...
										 const CWStringConst &strColB)
{
	CTableDescriptor *ptabdesc =
		GPOS_NEW(mp) CTableDescriptor(mp,
	...
	for (ULONG i = 0; i < 2; i++)
	{
		...
		CColumnDescriptor *pcoldesc = GPOS_NEW(mp) CColumnDescriptor(
			mp, pmdtype, default_type_modifier, nameColumn, i + 1, false /*is_nullable*/
		);
...
CStatisticsTest::EresUnittest_CStatisticsBasic()
{
	...
	if (NULL == col_factory->LookupColRef(1 /*id*/))
	{
		...
		(void) col_factory->PcrCreate(pmdtypeint4,
	...
	CStatistics *pstats1 =
		CFilterStatsProcessor::MakeStatsFilter(mp, stats, pred_stats, true /* do_cap_NDVs */);
	pstats1->Rows();
	...
	CStatistics *pstats2 = GPOS_NEW(mp)
		CStatistics(mp, phmulhist2, phmuldoubleWidth2, 100.0 /* rows */, false /* is_empty */);
	...
	CStatistics *pstats4 =
		CGroupByStatsProcessor::CalcGroupByStats(mp, stats, GCs, aggs, NULL /*keys*/);
	...
	CStatistics *pstats6 =
		CUnionAllStatsProcessor::CreateStatsForUnionAll(mp, stats, stats, colids, colids, colids);
...
CStatsPredPtrArry *
CStatisticsTest::Pdrgpstatspred1(IMemoryPool *mp)
{
...
// create a filter clause
CStatsPredPtrArry *
CStatisticsTest::Pdrgpstatspred2(IMemoryPool *mp)
{
	// contain for filters
	CStatsPredPtrArry *pdrgpstatspred = GPOS_NEW(mp) CStatsPredPtrArry(mp);
	...
	StatsFilterNumeric(
		mp, 3, pstrLowerNumeric, pstrUpperNumeric, CDouble(1.0), CDouble(2.0), pdrgpstatspred);
	...
	StatsFilterGeneric(
		mp, 4, GPDB_DATE, pstrLowerDate, pstrUpperDate, lLowerDate, lUpperDate, pdrgpstatspred);
	...
	StatsFilterGeneric(
		mp, 5, GPDB_TIMESTAMP, pstrLowerTS, pstrUpperTS, lLowerTS, lUpperTS, pdrgpstatspred);
...
CStatisticsTest::StatsFilterInt4(
	IMemoryPool *mp, ULONG colid, INT iLower, INT iUpper, CStatsPredPtrArry *pdrgpstatspred)
{
	CStatsPredPoint *pstatspred1 = GPOS_NEW(mp)
		CStatsPredPoint(colid, CStatsPred::EstatscmptGEq, CTestUtils::PpointInt4(mp, iLower));

	CStatsPredPoint *pstatspred2 = GPOS_NEW(mp)
		CStatsPredPoint(colid, CStatsPred::EstatscmptL, CTestUtils::PpointInt4(mp, iUpper));
...
void
CStatisticsTest::StatsFilterBool(IMemoryPool *mp,
								 ULONG colid,
								 BOOL fValue,
								 CStatsPredPtrArry *pdrgpstatspred)
{
	CStatsPredPoint *pstatspred1 = GPOS_NEW(mp)
		CStatsPredPoint(colid, CStatsPred::EstatscmptEq, CTestUtils::PpointBool(mp, fValue));
...
void
CStatisticsTest::StatsFilterNumeric(IMemoryPool *mp,
									ULONG colid,
...
void
CStatisticsTest::StatsFilterGeneric(IMemoryPool *mp,
									ULONG colid,
									OID oid,
									CWStringDynamic *pstrLowerEncoded,
									CWStringDynamic *pstrUpperEncoded,
