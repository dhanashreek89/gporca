...
//---------------------------------------------------------------------------
void
CSubqueryTestUtils::GenerateGetExpressions(IMemoryPool *mp,
										   ...
										   CExpression **ppexprInner)
{
	...
	CTableDescriptor *ptabdescR =
		CTestUtils::PtabdescCreate(mp, 3 /*num_cols*/, pmdidR, CName(&strNameR));
	*ppexprOuter = CTestUtils::PexprLogicalGet(mp, ptabdescR, &strNameR);
	...
	CTableDescriptor *ptabdescS =
		CTestUtils::PtabdescCreate(mp, 3 /*num_cols*/, pmdidS, CName(&strNameS));
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprJoinWithAggSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
	...
	CExpression *pexpr = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalInnerJoin(mp), (*pexprSelect)[0], pexprRight, (*pexprSelect)[1]);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubquery(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubqueryConstComparison(IMemoryPool *mp,
															  CExpression *pexprOuter,
															  CExpression *pexprInner,
															  BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithAggSubquery(IMemoryPool *mp,
												CExpression *pexprOuter,
												CExpression *pexprInner,
												BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubqueryConstComparison(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithAggSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubqueryOverJoin(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
	...
	CTableDescriptor *ptabdescT =
		CTestUtils::PtabdescCreate(mp, 3 /*num_cols*/, pmdidT, CName(&strNameT));
	CExpression *pexprT = CTestUtils::PexprLogicalGet(mp, ptabdescT, &strNameT);
	CColRef *pcrInner =
		CDrvdPropRelational::GetRelationalProperties(pexprR->PdpDerive())->PcrsOutput()->PcrAny();
	CColRef *pcrOuter =
		CDrvdPropRelational::GetRelationalProperties(pexprT->PdpDerive())->PcrsOutput()->PcrAny();
	...
	CExpression *pexprSubq = GPOS_NEW(mp) CExpression(
		...
		GPOS_NEW(mp) CScalarSubquery(
			mp, pcrInner, false /*fGeneratedByExist*/, false /*fGeneratedByQuantified*/),
		pexprJoin);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAnySubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAnySubqueryOverWindow(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryQuantifiedOverWindow(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAllSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAllSubqueryOverWindow(IMemoryPool *mp, BOOL fCorrelated)
{
	return PexprSelectWithSubqueryQuantifiedOverWindow(
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAnyAggSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAllAggSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithAnySubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
	GenerateGetExpressions(mp, &pexprOuter, &pexprInner);
	...
	return PexprProjectWithSubqueryQuantified(
		mp, pexprOuter, pexprInner, COperator::EopScalarSubqueryAny, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithAllSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
	GenerateGetExpressions(mp, &pexprOuter, &pexprInner);
	...
	return PexprProjectWithSubqueryQuantified(
		mp, pexprOuter, pexprInner, COperator::EopScalarSubqueryAll, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueriesInDifferentContexts(IMemoryPool *mp, BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
	GenerateGetExpressions(mp, &pexprOuter, &pexprInner);
	...
	return PexprProjectWithSubqueryQuantified(
		mp, pexprSelect, pexprGet, COperator::EopScalarSubqueryAny, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueriesInNullTestContext(IMemoryPool *mp, BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
	GenerateGetExpressions(mp, &pexprOuter, &pexprInner);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithExistsSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNotExistsSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryDisjuncts(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
	...
	return PexprSelectWithSubqueryBoolOp(
		mp, pexprOuter, pexprInner, fCorrelated, CScalarBoolOp::EboolopOr);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithTrimmableExists(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithTrimmableNotExists(IMemoryPool *mp, BOOL fCorrelated)
{
	return PexprSelectWithTrimmableExistentialSubquery(
		mp, COperator::EopScalarSubqueryNotExists, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithExistsSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithNotExistsSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedCmpSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	...
	CExpression *pexprPredicate1 = CUtils::PexprScalarEqCmp(
		mp, CUtils::PexprScalarConstBool(mp, true /*value*/), pexprSubqueryPred);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithCmpSubqueries(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	CExpression *pexprInner = PexprSelectWithAggSubquery(mp, fCorrelated);
	CColRef *pcrInner = CDrvdPropRelational::GetRelationalProperties(pexprInner->PdpDerive())
							->PcrsOutput()
							->PcrAny();

	CExpression *pexprOuter = CTestUtils::PexprLogicalGet(mp);
	CColRef *pcrOuter = CDrvdPropRelational::GetRelationalProperties(pexprOuter->PdpDerive())
							->PcrsOutput()
							->PcrAny();
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedQuantifiedSubqueries(IMemoryPool *mp,
															  COperator::EOperatorId op_id,
															  BOOL fCorrelated)
{
	GPOS_ASSERT(COperator::EopScalarSubqueryAny == op_id ||
	...
	CExpression *pexprOuter1 =
		CTestUtils::PexprLogicalGetNullable(mp, GPOPT_TEST_REL_OID1, &strName1, &strAlias1);

	CWStringConst strName2(GPOS_WSZ_LIT("Rel2"));
	CWStringConst strAlias2(GPOS_WSZ_LIT("Rel2Alias"));
	CExpression *pexprOuter2 =
		CTestUtils::PexprLogicalGetNullable(mp, GPOPT_TEST_REL_OID2, &strName2, &strAlias2);

	CWStringConst strName3(GPOS_WSZ_LIT("Rel3"));
	CWStringConst strAlias3(GPOS_WSZ_LIT("Rel3Alias"));
	CExpression *pexprOuter3 =
		CTestUtils::PexprLogicalGetNullable(mp, GPOPT_TEST_REL_OID3, &strName3, &strAlias3);

	CWStringConst strName4(GPOS_WSZ_LIT("Rel4"));
	CWStringConst strAlias4(GPOS_WSZ_LIT("Rel4Alias"));
	CExpression *pexprInner =
		CTestUtils::PexprLogicalGetNullable(mp, GPOPT_TEST_REL_OID4, &strName4, &strAlias4);

	CExpression *pexprSubqueryQuantified1 =
		PexprSubqueryQuantified(mp, op_id, pexprOuter3, pexprInner, fCorrelated);
	CExpression *pexprSelect1 =
		CUtils::PexprLogicalSelect(mp, pexprOuter3, pexprSubqueryQuantified1);
	CExpression *pexprSubqueryQuantified2 =
		PexprSubqueryQuantified(mp, op_id, pexprOuter2, pexprSelect1, fCorrelated);
	CExpression *pexprSelect2 =
		CUtils::PexprLogicalSelect(mp, pexprOuter2, pexprSubqueryQuantified2);
	CExpression *pexprSubqueryQuantified3 =
		PexprSubqueryQuantified(mp, op_id, pexprOuter1, pexprSelect2, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedAnySubqueries(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedAllSubqueries(IMemoryPool *mp, BOOL fCorrelated)
{
	return PexprSelectWithNestedQuantifiedSubqueries(
		mp, COperator::EopScalarSubqueryAll, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWith2LevelsCorrSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	...
	if (fCorrelated)
	{
		...
		CColRef *pcrOuter = CDrvdPropRelational::GetRelationalProperties(pexpr->PdpDerive())
								->PcrsOutput()
								->PcrAny();
		CColRef *pcrInner =
			CDrvdPropRelational::GetRelationalProperties(pexprInnerSelect->PdpDerive())
				->PcrsOutput()
				->PcrAny();
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryConjuncts(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithSubqueries(IMemoryPool *mp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
...
)
{
	...
	CExpression *pexprNonCorrelated =
...
)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp)
			CScalarSubqueryAll(mp, GPOS_NEW(mp) CMDIdGPDB(GPDB_INT4_NEQ_OP), str, pcrInner),
		pexprSelect,
		CUtils::PexprScalarIdent(mp, pcrOuter));
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableSubquery(IMemoryPool *mp,
												  COperator::EOperatorId op_id,
												  BOOL fCorrelated)
{
	GPOS_ASSERT(COperator::EopScalarSubqueryAny == op_id ||
	...
	CWStringConst strNameR(GPOS_WSZ_LIT("Rel1"));
	CMDIdGPDB *pmdidR =
		GPOS_NEW(mp) CMDIdGPDB(GPOPT_TEST_REL_OID1, 1 /*version_major*/, 1 /*version_minor*/);
	CTableDescriptor *ptabdescR =
	...
	switch (op_id)
	{
		...
		case COperator::EopScalarSubqueryNotExists:
			pexprSubquery =
				PexprSubqueryExistential(mp, op_id, pexprOuter, pexprInner, fCorrelated);
			break;
	...
	CExpression *pexprPred =
		CUtils::PexprScalarEqCmp(mp, pcrOuter, CUtils::PexprScalarConstInt4(mp, 5 /*val*/));
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableAnySubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableAllSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableExistsSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableNotExistsSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableScalarSubquery(IMemoryPool *mp, BOOL fCorrelated)
{
	CExpression *pexprOuter = CTestUtils::PexprLogicalGet(mp);
	CExpression *pexprInner = CTestUtils::PexprLogicalGet(mp);
	CExpression *pexprSelect = PexprSubquery(mp, pexprOuter, pexprInner, fCorrelated);
...
)
{
	...
	CExpression *pexprLogicalGbAgg =
		CUtils::PexprLogicalGbAggGlobal(mp, colref_array, pexprSelect, pexprProjList);

	// return a subquery expression on top of group by
	return GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp) CScalarSubquery(
			mp, pcrComputed, false /*fGeneratedByExist*/, false /*fGeneratedByQuantified*/),
		pexprLogicalGbAgg);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryBoolOp(IMemoryPool *mp,
												  CExpression *pexprOuter,
												  CExpression *pexprInner,
												  BOOL fCorrelated,
												  CScalarBoolOp::EBoolOperator eboolop)
{
	...
	// generate a regular predicate
	CExpression *pexprPred2 =
		CUtils::PexprScalarEqCmp(mp, pcrLeft, CUtils::PexprScalarConstInt4(mp, 5 /*val*/));
	...
	CExpression *pexprSubqueryExists = PexprSubqueryExistential(
		mp, COperator::EopScalarSubqueryExists, pexprOuter, pexprGet2, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithSubqueries(IMemoryPool *mp,
											   CExpression *pexprOuter,
											   CExpression *pexprInner,
											   BOOL fCorrelated)
{
	...
	pexprGet = CTestUtils::PexprLogicalGet(mp);
	CExpression *pexprSubqueryExists = PexprSubqueryExistential(
		mp, COperator::EopScalarSubqueryExists, pexprOuter, pexprGet, fCorrelated);
	pcrComputed = col_factory->PcrCreate(pmdtypebool, default_type_modifier);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryQuantified(IMemoryPool *mp,
													  COperator::EOperatorId op_id,
													  BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithQuantifiedAggSubquery(IMemoryPool *mp,
														 COperator::EOperatorId op_id,
														 BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
	...
	CExpression *pexprSubq =
		PexprSubqueryAgg(mp, pexprOuter, CTestUtils::PexprLogicalGet(mp), fCorrelated);
	CExpression *pexprGb = (*pexprSubq)[0];
	pexprGb->AddRef();
	pexprSubq->Release();

	CColRef *pcrInner =
		CDrvdPropRelational::GetRelationalProperties(pexprGb->PdpDerive())->PcrsOutput()->PcrAny();
	CColRef *pcrOuter = CDrvdPropRelational::GetRelationalProperties(pexprOuter->PdpDerive())
							->PcrsOutput()
							->PcrAny();
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithSubqueryQuantified(IMemoryPool *mp,
													   CExpression *pexprOuter,
													   CExpression *pexprInner,
													   COperator::EOperatorId op_id,
													   BOOL fCorrelated)
{
	...
	CExpression *pexprPrjElem =
		CUtils::PexprScalarProjectElement(mp, pcrComputed, pexprSubqueryQuantified);
	CExpression *pexprPrjList =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprPrjElem);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryExistential(IMemoryPool *mp,
													   COperator::EOperatorId op_id,
													   BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
	...
	CExpression *pexprInner = NULL;
	GenerateGetExpressions(mp, &pexprOuter, &pexprInner);
	CExpression *pexprSubqueryExistential =
		PexprSubqueryExistential(mp, op_id, pexprOuter, pexprInner, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithTrimmableExistentialSubquery(IMemoryPool *mp,
																COperator::EOperatorId op_id,
																BOOL  // fCorrelated
)
{
	...
	if (COperator::EopScalarSubqueryExists == op_id)
	{
		pexprSubqueryExistential =
			GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarSubqueryExists(mp), pexprGbAgg);
	}
	else
	{
		pexprSubqueryExistential =
			GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarSubqueryNotExists(mp), pexprGbAgg);
	}

	// generate a regular predicate
	CColRefSet *pcrs =
		CDrvdPropRelational::GetRelationalProperties(pexprOuter->PdpDerive())->PcrsOutput();
	CExpression *pexprEqPred =
		CUtils::PexprScalarEqCmp(mp, pcrs->PcrAny(), CUtils::PexprScalarConstInt4(mp, 5 /*val*/));
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithSubqueryExistential(IMemoryPool *mp,
														COperator::EOperatorId op_id,
														BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != mp);
	...
	// generate a scalar project list
	CExpression *pexprPrjElem =
		CUtils::PexprScalarProjectElement(mp, pcrComputed, pexprSubqueryExistential);
	CExpression *pexprPrjList =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprPrjElem);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueryWithConstTableGet(IMemoryPool *mp, COperator::EOperatorId op_id)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(COperator::EopScalarSubqueryAny == op_id ||
				COperator::EopScalarSubqueryAll == op_id);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueryWithDisjunction(IMemoryPool *mp)
{
	GPOS_ASSERT(NULL != mp);
