...
//
//---------------------------------------------------------------------------
CCostModelGPDB::CCostModelGPDB(IMemoryPool *mp, ULONG ulSegments, CCostModelParamsGPDB *pcp)
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostTupleProcessing(DOUBLE rows, DOUBLE width, ICostModelParams *pcp)
{
	GPOS_ASSERT(NULL != pcp);
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostScanOutput(IMemoryPool *,  // mp
							   DOUBLE rows,
							   DOUBLE width,
							   DOUBLE num_rebinds,
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostUnary(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostSpooling(IMemoryPool *mp,
							 CExpressionHandle &exprhdl,
							 const SCostingInfo *pci,
							 ICostModelParams *pcp)
{
...
CCostModelGPDB::FUnary(COperator::EOperatorId op_id)
{
	return COperator::EopPhysicalAssert == op_id || COperator::EopPhysicalComputeScalar == op_id ||
		   ...
		   COperator::EopPhysicalRowTrigger == op_id || COperator::EopPhysicalSplit == op_id ||
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostChildren(IMemoryPool *mp,
							 CExpressionHandle &exprhdl,
							 const SCostingInfo *pci,
							 ICostModelParams *pcp)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		if (NULL != popChild && CUtils::FPhysicalScan(popChild))
		{
			...
			dCostChild =
				dCostChild +
				CostScanOutput(mp, dScanRows, pci->GetWidth()[ul], pci->PdRebinds()[ul], pcp).Get();
		}
...
CCost
CCostModelGPDB::CostMaxChild(IMemoryPool *,
							 CExpressionHandle &,
							 const SCostingInfo *pci,
							 ICostModelParams *)
{
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostCTEProducer(IMemoryPool *mp,
								CExpressionHandle &exprhdl,
								...
								const SCostingInfo *pci)
{
	...
	CCost costSpooling =
		CCost(pci->NumRebinds() * (pci->Rows() * pci->Width() * dMaterializeCostUnit));
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostCTEConsumer(IMemoryPool *,  // mp
								CExpressionHandle &
#ifdef GPOS_DEBUG
									exprhdl
#endif  // GPOS_DEBUG
								...
								const SCostingInfo *pci)
{
	...
	const CDouble dInitScan =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDB::EcpInitScanFactor)->Get();
	...
	return CCost(pci->NumRebinds() * (dInitScan + pci->Rows() * pci->Width() *
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostConstTableGet(IMemoryPool *,  // mp
								  CExpressionHandle &
#ifdef GPOS_DEBUG
									  exprhdl
#endif  // GPOS_DEBUG
								  ...
								  const SCostingInfo *pci)
{
	...
	return CCost(
		pci->NumRebinds() *
		CostTupleProcessing(pci->Rows(), pci->Width(), pcmgpdb->GetCostModelParams()).Get());
}
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostDML(IMemoryPool *mp,
						CExpressionHandle &exprhdl,
						const CCostModelGPDB *pcmgpdb,
						const SCostingInfo *pci)
{
	...
	CCost costLocal =
		CCost(pci->NumRebinds() * (num_rows_outer * dWidthOuter) / dTupUpdateBandwidth);
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostScalarAgg(IMemoryPool *mp,
							  CExpressionHandle &exprhdl,
							  const CCostModelGPDB *pcmgpdb,
							  const SCostingInfo *pci)
{
	...
	CCost costLocal = CCost(pci->NumRebinds() * (num_rows_outer * dWidthOuter * ulAggCols *
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostStreamAgg(IMemoryPool *mp,
							  CExpressionHandle &exprhdl,
							  const CCostModelGPDB *pcmgpdb,
							  const SCostingInfo *pci)
{
	...
	CCost costLocal =
		CCost(pci->NumRebinds() * (num_rows_outer * dWidthOuter * dTupDefaultProcCostUnit +
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostSequence(IMemoryPool *mp,
							 CExpressionHandle &exprhdl,
							 const CCostModelGPDB *pcmgpdb,
							 const SCostingInfo *pci)
{
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostSort(IMemoryPool *mp,
						 CExpressionHandle &exprhdl,
						 const CCostModelGPDB *pcmgpdb,
						 const SCostingInfo *pci)
{
	...
	const CDouble dSortTupWidthCost = pcmgpdb->GetCostModelParams()
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostTVF(IMemoryPool *,  // mp
						CExpressionHandle &
#ifdef GPOS_DEBUG
							exprhdl
#endif  // GPOS_DEBUG
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostUnionAll(IMemoryPool *mp,
							 CExpressionHandle &exprhdl,
							 const CCostModelGPDB *pcmgpdb,
							 const SCostingInfo *pci)
{
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostHashAgg(IMemoryPool *mp,
							CExpressionHandle &exprhdl,
							const CCostModelGPDB *pcmgpdb,
							const SCostingInfo *pci)
{
	...
	const ULONG ulGrpCols =
		CPhysicalHashAgg::PopConvert(exprhdl.Pop())->PdrgpcrGroupingCols()->Size();
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostHashJoin(IMemoryPool *mp,
							 CExpressionHandle &exprhdl,
							 const CCostModelGPDB *pcmgpdb,
							 const SCostingInfo *pci)
{
	...
	const CDouble dHJFeedingTupColumnSpillingCostUnit =
		pcmgpdb->GetCostModelParams()
			->PcpLookup(CCostModelParamsGPDB::EcpHJFeedingTupColumnSpillingCostUnit)
			->Get();
	const CDouble dHJFeedingTupWidthSpillingCostUnit =
		pcmgpdb->GetCostModelParams()
			->PcpLookup(CCostModelParamsGPDB::EcpHJFeedingTupWidthSpillingCostUnit)
			->Get();
	...
	CColRefSet *pcrsUsed =
		CDrvdPropScalar::GetDrvdScalarProps(pexprJoinCond->PdpDerive())->PcrsUsed();
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostIndexNLJoin(IMemoryPool *mp,
								CExpressionHandle &exprhdl,
								const CCostModelGPDB *pcmgpdb,
								const SCostingInfo *pci)
{
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostNLJoin(IMemoryPool *mp,
						   CExpressionHandle &exprhdl,
						   const CCostModelGPDB *pcmgpdb,
						   const SCostingInfo *pci)
{
	...
	const CDouble dNLJFactor =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDB::EcpNLJFactor)->Get();
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostMotion(IMemoryPool *mp,
						   CExpressionHandle &exprhdl,
						   const CCostModelGPDB *pcmgpdb,
						   const SCostingInfo *pci)
{
	...
	if (COperator::EopPhysicalMotionBroadcast == op_id)
	{
		...
		dSendCostUnit = pcmgpdb->GetCostModelParams()
							->PcpLookup(CCostModelParamsGPDB::EcpBroadcastSendCostUnit)
							->Get();
		dRecvCostUnit = pcmgpdb->GetCostModelParams()
	...
	costLocal =
		CCost(pci->NumRebinds() * (num_rows_outer * dWidthOuter * dSendCostUnit + recvCost));
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostSequenceProject(IMemoryPool *mp,
									CExpressionHandle &exprhdl,
									const CCostModelGPDB *pcmgpdb,
									const SCostingInfo *pci)
{
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostIndexScan(IMemoryPool *,  // mp
							  CExpressionHandle &exprhdl,
							  const CCostModelGPDB *pcmgpdb,
							  const SCostingInfo *pci)
{
	...
	return CCost(pci->NumRebinds() *
				 (dRowsIndex * (ulIndexKeys * dIndexFilterCostUnit +
								dTableWidth * dIndexScanTupCostUnit + dIndexScanTupRandomFactor)));
}
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostBitmapTableScan(IMemoryPool *,  // mp,
									CExpressionHandle &exprhdl,
									const CCostModelGPDB *pcmgpdb,
									const SCostingInfo *pci)
{
	...
	CColRefSet *pcrsUsed =
		CDrvdPropScalar::GetDrvdScalarProps(pexprIndexCond->PdpDerive())->PcrsUsed();

	if (COperator::EopScalarBitmapIndexProbe != pexprIndexCond->Pop()->Eopid() ||
		1 < pcrsUsed->Size())
	{
		// child is Bitmap AND/OR, or we use Multi column index
		const CDouble dInitScan = pcmgpdb->GetCostModelParams()
		...
		return CCost(pci->NumRebinds() *
					 (pci->Rows() * pci->Width() * dIndexFilterCostUnit + dInitScan));
	}
	...
	CDouble dNDVThreshold = pcmgpdb->GetCostModelParams()
								   ...
								   CDouble dNDV)
{
	...
	CDouble dBitmapIO = pcmgpdb->GetCostModelParams()
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostScan(IMemoryPool *,  // mp
						 CExpressionHandle &exprhdl,
						 const CCostModelGPDB *pcmgpdb,
						 const SCostingInfo *pci)
{
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostFilter(IMemoryPool *mp,
						   CExpressionHandle &exprhdl,
						   const CCostModelGPDB *pcmgpdb,
						   const SCostingInfo *pci)
{
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::Cost(CExpressionHandle &exprhdl,  // handle gives access to expression properties
