...
namespace gpopt
{
	...
	public:
		// ctor
		explicit CCostModelParamsGPDBLegacy(IMemoryPool *mp);
		...
		// set param by id
		virtual void SetParam(ULONG id, CDouble dVal, CDouble dLowerBound, CDouble dUpperBound);

		// set param by name
		virtual void SetParam(const CHAR *szName,
							  CDouble dVal,
							  CDouble dLowerBound,
							  CDouble dUpperBound);
